//! Perf-baseline reflow 1000 узлов (FR-068 W0; контракт §8 FR-068,
//! docs/change-requests/fr-068-ui-refactoring-long-term.md).
//!
//! # Что измеряется
//!
//! Один полный immediate-mode reflow синтетического UI-графа от корня
//! чистыми функциями `canvas_ui::layout` (PRD-0009 §7.4 V-5): `stack` →
//! `constrain` → `pad` → `Column::lay_out` (10 контейнерных строк) →
//! 9 × `Row::lay_out` + `grid_cells`. Метрика — МЕДИАННАЯ длительность
//! одного reflow в μs по `ITERATIONS` замерам (`Instant`), после
//! `WARMUP` прогревочных прогонов (не замеряются).
//!
//! # Определение синтетического графа (зафиксировано, детерминировано)
//!
//! Viewport 1280×800 (canonical G4-окно), stage 1200×760 (`constrain` +
//! центрирование `stack`), внутренний слот = stage − pad 16 px.
//! «Узел» = ЛИСТОВОЙ rect, вычисленный за один reflow; итого РОВНО
//! 1000 узлов (`RowCase` × 10 строк × 100 листьев):
//!
...
//!   сумма ≫ слота, хвост сжимается до нулевой ширины (именованная
//!   деградация F-17);
...
//!
//! 10 контейнерных rect'ов строк (выход `Column::lay_out`), stage и
//! внутренний слот в 1000 НЕ входят (это контейнеры, не узлы).
//! Все размеры — формула от глобального индекса (без RNG; seeded-RNG в
//! std нет, формулы дают тот же детерминизм) — см. константы в
...
//! debug). Baseline-файл `tests/perf_baseline.txt` — референс dev-машины
//! и коммитится. Сравнение при запуске: регрессия > 20% от записанного
//! — fail (контракт §8 FR-068), улучшение > 20% — предупреждение в
//! stdout (не fail; baseline можно обновить осознанно).
//!
//! # Бюджеты
//!
//! - Абсолютный (любая машина): медиана < 1000 μs = 1 мс на 1000 узлов
//!   (§8 FR-068). 60 fps на 5000 нод — GPU-бюджет рендера SPEC §6.3,
//!   вне headless-замера; данный тест закрывает CPU reflow-компонент
//!   этого гейта.
...
//!
//! ```text
//! cargo test -p canvas-ui --test perf_baseline -- --ignored
//! cargo test -p canvas-ui -- --ignored perf_baseline      # как в FR-068
//! CANVAS_UI_UPDATE_PERF=1 cargo test -p canvas-ui --test perf_baseline -- --ignored
//! ```
...
use canvas_ui::layout::{
    constrain, grid_cells, pad, stack, Child, Column, CrossAlign, HAlign, MainAlign, Row,
    RowPolicy, VAlign,
};
...
/// Один полный reflow от корня (то, что замеряется): stack → constrain
/// → pad → Column → строки/grid. Кладёт в `out` РОВНО 1000 листовых
/// rect'ов (контейнерные rect'ы строк существуют внутри, но не
/// добавляются — определение «узла» см. в модульной доке).
fn reflow(graph: &Graph, out: &mut Vec<UiRect>) {
    ...
    let row_slots = graph.column.lay_out(root_slot, &graph.column_children);
    for (case, &slot) in graph.rows.iter().zip(&row_slots) {
        match case {
            RowCase::Linear { row, items } => out.extend(row.lay_out(slot, items)),
            RowCase::Grid {
                cols,
                rows,
                row_h,
                gap,
            } => out.extend(grid_cells(slot, cols, *rows, *row_h, *gap)),
        }
    }
}
...
/// машинно-зависимость) + одна строка `median_us=<число>`.
fn baseline_text(median_us: f64) -> String {
    let secs = SystemTime::now()
        ...
        .unwrap_or(0);
    format!(
        "# CanvasDesk FR-068 W0 — perf baseline reflow (crates/canvas-ui/tests/perf_baseline.rs)\n\
         # Граф: viewport 1280×800 → constrain/stack (stage 1200×760) → pad(16) → Column(10) →\n\
         # 5×Row Fit + 2×Row Wrap + 2×Row SqueezeTail (по 100 Child) + grid_cells 10×10 = РОВНО 1000 листовых rect'ов за reflow;\n\
         # размеры детерминированы формулами от индекса (без RNG). Контейнерные rect'ы строк в 1000 не входят.\n\
...
/// Perf-гейт FR-068 W0 (контракт §8): reflow 1000 узлов — медиана < 1 мс;
/// против baseline `tests/perf_baseline.txt` — регрессия > 20% — fail,
/// улучшение > 20% — предупреждение. Замер машинно-зависим: baseline —
/// референс dev-машины; на другой машине возможен ложный fail по дрейфу —
/// тогда обнови baseline осознанно (`CANVAS_UI_UPDATE_PERF=1`).
#[test]
#[ignore = "perf-замер (машинно-зависим): cargo test -p canvas-ui --test perf_baseline -- --ignored"]
fn perf_reflow_1000() {
    ...
    assert_eq!(out, again, "reflow недетерминирован");
    black_box(&out);
    ...
    black_box(med);
    println!(
        "reflow {NODES} узлов: median = {med:.1} μs ({ITERATIONS} итераций после {WARMUP} прогревочных; бюджет §8 < {BUDGET_US:.0} μs)"
    );

    // Абсолютный бюджет контракта §8 FR-068 — на любой машине.
    assert!(
        med < BUDGET_US,
        "reflow 1000 узлов: median {med:.1} μs ≥ бюджет {BUDGET_US:.0} μs \
         (FR-068 W0 §8: reflow < 1 мс; 60 fps на 5000 нод — SPEC §6.3). \
    ...
    let path = PathBuf::from(env!("CARGO_MANIFEST_DIR"))
        .join("tests")
        .join("perf_baseline.txt");
    if std::env::var("CANVAS_UI_UPDATE_PERF").is_ok_and(|v| v == "1") {
        fs::write(&path, baseline_text(med)).expect("запись tests/perf_baseline.txt");
    ...
    match recorded {
        Some(baseline) => {
            ...
            assert!(
                ...
                "регрессия perf: baseline {baseline:.1} μs → текущая {med:.1} μs \
                 (+{drift_pct:.0}% > +20%). FR-068 W0 §8: регрессия > 20% — фиксируй осознанно: \
            ...
            if med < baseline * (1.0 - DRIFT) {
                println!(
                    "WARN: улучшение > 20% против baseline ({baseline:.1} μs → {med:.1} μs, {drift_pct:.0}%) — не fail; \
                     рассмотри осознанное обновление baseline: CANVAS_UI_UPDATE_PERF=1"
                );
            }
        ...
        None => println!(
            "WARN: {} отсутствует или без строки median_us= — сравнение с baseline пропущено \
             (абсолютный бюджет §8 проверен). Создай baseline: \
             CANVAS_UI_UPDATE_PERF=1 cargo test -p canvas-ui --test perf_baseline -- --ignored",
            path.display()
        ),
    }
