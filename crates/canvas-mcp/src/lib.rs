...
const TOOLS: &[ToolSpec] = &[
    ...
    ToolSpec {
        name: "node_edit",
        description: "Редактировать ноду одним вызовом: обновляет ТОЛЬКО переданные поля (text, label, color, expr, x, y, width, height); label/color/expr = null — сброс; expr — Numi-style формула, результат рендерится под текстом ноды (невалидная формула — ошибка, нода не меняется); возвращает обновлённую ноду. Многострочный text — реальными переводами строк (в JSON LF экранируется как \\n); последовательность \\n как два символа тоже принимается как перевод строки; литеральный обратный слеш — \\\\",
        required: &["id"],
        properties: &[
            ("id", STR),
            ("text", STR),
