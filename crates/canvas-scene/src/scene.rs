...
impl SceneState {
    ...
    pub fn recompute_flow(&mut self) {
        ...
        {
            // FR-042 (багфикс паники bundles.rs «index out of bounds»):
            // инвариант «хвост recompute_flow перестраивает пучки» обязан
            // действовать и на воркер-ветке. Ранняя отсрочка выводки НЕ
            // отменяет мутацию топологии: до прихода FlowReady кадр
            // рендерится с индексами пучков от ПРЕЖНЕГО канваса — после
            // удаления ребра `dominant_edge` читал `canvas.edges[index]`
            // вне диапазона (index == len) и ронял приложение. Перестройка
            // синхронно здесь — O(edges), тяжёлый propagate по-прежнему
            // уходит воркеру.
    ...
    /// FR-075 (дефект найден верификацией витрины 2026-09-26): догоняющий
    /// refit высот после наполнения `template_descs` (`App::sync_template_descs`
    /// при построении App). Загрузочный пересчёт (`with_storage`) исполняется
    /// ДО того, как сцена узнаёт описания манифестов, — подгон высоты шла
    /// без зоны описания, а рендер описание показывал (расхождение I-2:
    /// мера ≠ рендер): ряды таблицы и футер «ИТОГ» выезжали за силуэт
    /// шаблонной карточки из сохранённого/внешнего `.canvas`. Хеш-гейт
    /// (`refit_node_to_content_if_changed`): появление описания меняет
    /// `content_height_hash` — реальный замер (рост и усадка) исполняется
    /// только у таких нод, у остальных вызов no-op; метод дёшев и
    /// идемпотентен (образец хвоста `recompute_flow`).
    pub fn refit_after_template_descs(&mut self) {
        self.apply_result_reserve();
        for index in 0..self.canvas.nodes.len() {
            self.refit_node_to_content_if_changed(index);
        }
