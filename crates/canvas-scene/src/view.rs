...
pub struct SpillView {
    ...
    /// FR-050 Н9-2 (этап D): квалифицированный путь источника
    /// «Объект.Поле» (имена — единая точка сборки dataref, коллизия —
    /// «Имя (node_id)») — для тултипа «пролито: {path} = {value}».
    pub path: String,
    /// FR-050 Н9-2 (этап D): локальный литерал RHS строки присваивания
    /// («rps = 500 rps» → «500 rps») — часть тултипа «(локально было: …)»;
    /// None — локальной строки/литерала нет.
