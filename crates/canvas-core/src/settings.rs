...
/// FR-061 этап C (D-7, Н-3 пре-PRD PRD-0004): порог T режима тела —
/// при числе строк данных (параметры + расчёт; авто-строки не в счёт —
/// диагностика) БОЛЬШЕ T тело переходит от сквозного листа (Н-1) к
/// блок-ведомости (Н-2: заголовок «▸ расчёт · N строк» + Σ на направляющей
/// чисел). Дефолт 4 — рекомендация анализа §9 (Q2, вопрос владельца
/// открыт: «предсказуемость — добавил 5-ю строку, блок появился»);
/// вынос в config.toml — по решению владельца.
pub const NODE_BODY_BLOCK_THRESHOLD: usize = 4;
