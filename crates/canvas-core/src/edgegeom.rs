...
pub fn retarget_edge(
    ...
    match end {
        EdgeEnd::From => {
            ...
            edge.from_line = None;
            // FR-029: именованный выход истока тоже сбрасывается — у новой
            // ноды такого выхода может не быть (деградация до узлового
            // значения, семантика как у from_line)
            edge.from_output = None;
        }
        EdgeEnd::To => {
            edge.to_node = target;
