...
mod tests {
    ...
    fn analyze_skips_nodes_without_formula() {
        ...
        let solutions = flow::propagate_with_lines(&canvas, &HashMap::new()).unwrap();
    ...
    fn analyze_utilization_critical_from_named_output() {
        let mut solutions = flow::propagate_with_lines(&queue_canvas(), &HashMap::new()).unwrap();
    ...
    fn analyze_utilization_named_over_one_is_overload() {
        ...
        let mut solutions = flow::propagate_with_lines(&canvas, &HashMap::new()).unwrap();
        solutions.named.insert(
    ...
    fn has_risk_detects_any_severity() {
        ...
        let solutions = flow::propagate_with_lines(&healthy, &HashMap::new()).unwrap();
