...
/// пустого канваса (раскладка/состояние — чистые модули; отрисовка в app).
pub mod scheme_gallery_ui;

/// FR-038 / T-038.2: snap-движок выравнивания — чистая геометрия (сетка,
/// направляющие, majority, равные интервалы, collision-avoidance) без
/// winit/wgpu; детерминизм п.20. Рендер линий — T-038.3, интеграция в drag —
/// T-038.4.
pub mod snap;
...
/// модель (геометрия окна поверх канваса, tidy-лейаут дерева, машина
...
/// FR-070: UI-админпанель (консоль дизайн-системы) — сайдбар секций +
/// демо-зона: матрица состояний, уровни наполнения, сущности канваса (ST4),
/// каталог design-токенов с live-правкой слотов палитры. Паттерн — kit_ui.
pub mod admin_ui;

/// FR-055 (этап U4 PRD-0009, F-10): DebugOverlay — рамки hit-rect'ов по
/// слоям, имя под курсором, подсветка пересечений (G6). Тогл — F9 /
/// `?ui=debug` (web). Модель чистая, отрисовка — полоса UiLayer::Debug.
...
pub mod ui {
    ...
    /// (FR-016), `whatif_on` — what-if режим: для пунктов-переключателей
    /// рисуется ✓-галочка (FR-004.1 — Hotkeys, T15 — DesktopMode,
    /// FR-016 — BottleneckOverlay, FR-017 — WhatIf).
