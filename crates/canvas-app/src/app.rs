...
impl App {
    fn on_key(&mut self, event: &KeyEvent) {
        ...
        // PRD-0007 (X2): открытое окно проверки — Esc закрывает (§6.4:
        // Ready/Stale → Closed); канвас-хоткеи не получают
    ...
    fn on_left_button(&mut self, state: ElementState) {
        ...
        match state {
            ElementState::Pressed => {
                ...
                // PRD-0007 (X2): модальное окно проверки цепочки — клики
                // элементов окна (✕/чип/крошки/узлы), мимо окна — закрытие
                // (канвас клик не получает, паттерн main stage Q6)
                if self.explain.is_some() {
