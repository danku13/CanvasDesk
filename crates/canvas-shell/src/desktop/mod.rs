...
#[cfg(windows)]
pub mod monitor;
// T15-relaunch: single-instance handoff — ОС-мьютекс эксклюзивности +
// exit-событие. Повторный запуск (в т.ч. перезапуск на --desktop из меню
// канваса — in-place встройка невозможна: Vulkan не презентует в ребёнка
// Progman) сигналит работающему инстансу штатный выход и ждёт смерти
// предыдущего владельца мьютекса до старта.
#[cfg(windows)]
