...
mod tests {
    ...
    /// f305972) — все 9 search-тестов упали по таймауту 2 с, при зелёных
    /// macOS/Linux и зелёных watcher-тестах того же бинарника (их порог
    /// первого события — 5 с): медленный раннер отдаёт событие позже 2 с.
    /// Таймаут защищает от зависания, а не гейтит производительность.
