...
fn headless_gfm_body_quads_draw() {
    ...
    // Область тела: origin world (-180,-108) → screen (20,42).
    let (ox, oy) = world_to_screen(origin);
    assert_eq!((ox, oy), (20, 42));
    ...
    // x 20..34; у правого края ноды их быть не должно.
    let stray: Vec<usize> = (34..370)
        .filter(|&x| {
            let p = px(x, oy + 33);
    ...
    // Страйк: body-local y = 96+11 = 107 → screen 149, слово с x = 20.
    // Линия тонкая (1.2px): в полосе 148..152 между x 22..80 есть серые
    // пиксели зачёркивания (текст ярче ~235, карточка темнее ~108).
    let strike_hits = (148..152)
