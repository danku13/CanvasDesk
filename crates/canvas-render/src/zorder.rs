...
pub struct ZPlan {
    ...
    /// `max_text_groups`, а также HUD (добавляет рендерер). Screen-тексты
    /// панелей рисует отдельная группа за пределами z-плана — см.
    /// `TextSystem::overlay_group`.
...
pub fn plan_z_order(
    ...
    // поверх всего кадра рисуется ещё HUD.
