...
pub fn template_icon_quads(icon: &str, rect: [f32; 4], tint: [f32; 4]) -> Vec<CardInstance> {
    ...
    match icon {
        ...
        // Аудит шаблонов 2026-09-25: clock — циферблат (рамка-круг) со
        // стрелками (минутная вверх, часовая вправо). Используется
        // pa-session-duration/pa-ttfv — прежде падали в custom-фолбэк.
        "clock" => {
            // Рамка-циферблат.
            push([x + 1.5 * u, y + 1.5 * u], [13.0 * u, 13.0 * u], fill, true);
            // Минутная стрелка (вверх от центра).
            push([x + 7.2 * u, y + 4.0 * u], [1.6 * u, 4.5 * u], fill, false);
            // Часовая стрелка (вправо от центра).
            push([x + 8.8 * u, y + 7.2 * u], [3.5 * u, 1.6 * u], fill, false);
        }
