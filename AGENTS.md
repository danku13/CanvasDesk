...
- `docs/change-requests/` — FR/CR-документы по шаблону `cr-template.md` (индекс —
  `index-cr-fr.md`). Реализация — только по документу; по завершении агент обновляет
  статус («реализовано»), Changelog документа и `docs/ACCEPTANCE.md`.
...
2. **Пользовательская документация** (`user-docs/`, 7 страниц; вшита в бинарь
   просмотрщиком FR-027 — `include_str!`, источник `crates/canvas-app/src/docs_ui.rs`):
