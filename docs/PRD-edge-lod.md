...
- **Статус:** в анализе
- **Тип:** PRD (документ требований продукта; реализация оформляется отдельным `FR-040` по шаблону `docs/change-requests/cr-template.md`)
- **Приоритет:** важно
...
- **Q1.** Вес: считать только рёбра (PoC) или коннекты с учётом `fromLine` (1 ребро с 5 строками = 5 коннектов) и `toParam`-спиллов? Предпочтение владельца зафиксировать в FR-040; влияет на формулу толщины.
...
| Двойной клик по пучку: конфликт с детектором лейбла | UX | В LOD-0 double-click на пучке = main stage (лейбл-редактор у агрегированной линии неопределён — лейблы индивидуальных рёбер показываются в main stage); зафиксировать в FR-040 |
...
|---|---|---|
| E0 | FR-040 по шаблону `cr-template.md` (фиксация решений §10-Q1/Q2/Q3) | Разрешение на реализацию |
...
- `docs/change-requests/fr-040-edge-lod-main-stage.md` — реализационный FR (создаётся на E0).
