...
- **Статус:** предложено → переходное → реализовано W1 FR-068 2026-09-25 → **выполнено и ЗАКРЫТО W4 FR-068 2026-09-25** (taffy вырезан целиком: `TaffyBackend`, фича, кит-пути, parity/perf-гейты; `FlexLayoutEngine` — единственный движок; конечное состояние ADR-0015 достигнуто) (trait LayoutBackend + NativeBackend 1:1 + TaffyBackend за фичей + pilot + 10 HTML5 demos + kit taffy-пути — гейты зелёные; wasm-дельта фактическая +283.9 КБ raw post-opt ниже оценки +376 КБ) (см. ADR-0015: taffy как временное решение, вырезается к W4 FR-068; `FlexLayoutEngine` W2 заменяет на совместимых политиках, W4 — taffy вырезается полностью, если Grid не используется). Критический разбор ADR-0015 выявил 9 слабостей (C1–C9: двойная поддержка навсегда, retained-конфликт, `SqueezeTail` ≠ `flex_shrink`, замер wasm без маржинальности, профиль кадра не замерен, 4-я тяжёлая UI-зависимость, нет конечной цели «своя UI-библиотека», 10 HTML5 demos не описаны, G4 расширение не специфицировано) — все закрываются волнами W0–W4 FR-068. Настоящий ADR — основа для W1 (taffy opt-in); W2 (`FlexLayoutEngine`) и W4 (вырезание taffy) — выполнен в ADR-0015 (2026-09-25).
- **Дата:** 2026-09-24
