...
> **Статус: предложение (не реализовано).** Формат описан как задел;
> реализация не начата — в README ей соответствует T25 «SDK BYOK-виджетов —
> опционально, не выполнено». Документ — проект спецификации, а не описание
...
MCP-сервер-посредник (stdio ↔ named pipe `\\.\pipe\canvasdesk` приложения)
слушает команды в формате JSON-RPC и возвращает `.canvas`-совместимый JSON.
Транспортные гарантии — FR-034/ADR-0009: handshake успешен независимо от
состояния приложения (недоступность CanvasDesk → isError в вызовах, а не
краш сервера), поддержаны batch-запросы и протокол 2025-06-18, мост сам
переподключается к pipe, когда приложение появится. Чистота канала —
FR-035/ADR-0010: stdout моста содержит только JSON-RPC (автоспавн поднимает
сервис с изолированным stdio, логи GUI — в stderr), посторонний текст в
протокольном канале исключён.

### Подключение AI-клиента (один exe, FR-008)

Один бинарник покрывает весь стек: `canvasdesk.exe` — GUI-сервис,
`canvasdesk.exe mcp` — MCP-сервер. Конфиг MCP-клиента (Claude Desktop,
Hermes Agent и т.п.): `command` = путь к `canvasdesk.exe`, `args` = `["mcp"]`.
Если сервис не запущен, посредник поднимет его сам (автостарт; `--no-spawn`
отключает) — весь стек стартует одной командой. Отдельный
`canvasdesk-mcp.exe` сохраняется для совместимости старых конфигов.
