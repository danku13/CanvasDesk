...
## 11. Постскриптум верификации (2026-09-26): находка координатного смещения панели stage

Ручная верификация M2 в браузере (wasm-сборка, деплой Pages) выявила:
панель «Как считается» на stage отрисовывает ТОЛЬКО фон-каркас — заголовки
колонок и строки отсутствуют. Диагностика (tracing в кадре stage) показала
двойное смещение: площади `CalcPanelLayout` (`vars_title`/`formulas_title`/
`vars_area`/`formulas_area`/строки) — stage-локальные координаты, а код
отрисовки прибавлял их к `px/py = rect.x/y + panel.rect[0/1]` — смещение
`panel.rect` применялось дважды, контент рисовался за нижней границей stage.

Находка ДО-Табличная: смещение жило с оригинального FR-044 (994e68e) и
пережило оба рефакторинга (FR-059 stack, M2 Table v2) — оракул T7 сравнивал
Table-путь с ручным kit-циклом на ОДНОЙ базе координат (самосогласованность
при общей ошибке), а hit-тесты кликов в input.rs были на верной
stage-локальной базе (расхождение баз никто не сверял, т.к. невидимая
панель не получала ни кликов, ни колеса).

Фикс (stage.rs): заголовки/вьюпорты колонок/бегунки — от начала stage
(`ox/oy = rect.x/rect.y`); `paint_calc_panel_rows` принимает семантику
«начало stage»; колесо над колонками — курсор переводится в stage-локальные
(`rel = cursor − rect.x/y`, паритет с input.rs). Оракул T7 обновлён на ту же
базу (равенство Table ≡ ручной цикл сохранено); клик-хит-тесты (Р-5) не
тронуты — они были верны. Гейты: workspace 2089/0, clippy/fmt/wasm — зелёные;
визуальная верификация в браузере: строки/заголовки панели на месте,
выравнивание направляющих совпадает с веером.

Урок для протокола I-1: «бит-в-бит» оракулы валидируют равенство двух путей,
но не корректность базы координат — при миграции screen-space UI нужна хотя
бы одна скриншот-проверка на реальном рендере (пиксельная зона), что теперь
и сделано для панели stage.
