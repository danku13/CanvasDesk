...
- **Статус:** дизайн на ревью; реализация — следующая очередь среза W3
  (этапы M1–M4 §8; этап M1 additive, без потребителей).
- **Дата:** 2026-09-26
...
(Ok/Err/Unmapped-текст), юнит/бейдж пустые → `unit_w=badge_w=0`,
`value_right = vars_right − gap` — ровно текущая геометрия
...
1. **Основной (потребительский) слой** — методы `*_with` с внешним
   ...
   `guides_with(&mut m, &mut fs, viewport_w)`, `visible_rows(viewport)`
   (замера не требует), `row_layout_with(&mut m, &mut fs, slot, i)`,
   `paint_with(&mut p, &mut m, &mut fs, viewport)`. ЭТИ методы используют
   продакшн-потребители (stage M2) — бит-в-бит замера с текущим кодом.
...
1. **Бит-в-бит.** «Переменные» stage: юнит/бейдж пустые у всех строк, но
   структурные зазоры живой колонки значения должны сохраниться
   (`value_right = right − gap`). Явный `columns{unit: off, badge: off}`
   при исключении колонки из цепочки зазоров СДВИНУЛ бы числа на `gap`
...
| **M5** | overlays (2) | `row_style`+zebra циклы → Table | низкий |
| **M6** | docs | каталог §9.3.5/§9.5 → «исполнено»; ACCEPTANCE FR-068 — запись; surface-registry — компонент Table | — |
