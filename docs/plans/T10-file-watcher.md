...
## 8. Отступления при реализации (сессия T10)

Отражают фактическую реализацию (коммит см. в TASKS); семантика §1–§7 сохранена.

1. **`WatchService::sync_dirs(&[PathBuf])` вместо `sync_dirs(&Canvas, &Path)`**
   (§3). Желаемый набор директорий считает чистая `canvas_core::watched_dirs`
   (тестируемая, сортировка + dedup), shell-сервис — только механика notify.
   Тестам shell не нужен Canvas.
2. **Один `RecommendedWatcher` с `watch`/`unwatch`** вместо
   `HashMap<PathBuf, RecommendedWatcher>` (§3). notify поддерживает
   множественные наблюдения на одном инстансе; меньше потоков и хендлов.
   Снятое наблюдение при ошибке unwatch покидает учёт — пере-добавится
   при следующем sync.
3. **`SceneState::resolve_file_path` делегирует `canvas_core::resolve_node_path`**
   — алгоритм перенесён в core единым источником (дубль логики устранён).
4. **notify 8.2** (план допускал 6+): MSRV 1.77 ≤ workspace 1.80. По
   исходникам выяснено: `Event::new(kind)` + `.add_path()`
   (конструктора с путями нет); Windows RDCW отдаёт rename парой
   `Name(From)`+`Name(To)`, не `Both` — склейка в `collapse` покрывает
   оба бэкенда, cfg-веток в тестах не понадобилось.
5. **`collapse` — чистая функция** (notify::Event → FileEvent): пары
   `From`+`To` и `Remove`→(позже)`Create` склеиваются в Rename; Create ДО
   Remove не склеивается (файл родился и умер); после Rename(a,b) дублёвые
   Create(b)/Remove(a) подавляются; дедуп (kind, path). Отсюда тесты
   ассертят итоговые FileEvent — кроссплатформенно.
6. **Rename-матч снимает `brokenLink`** (файл существует по `to`);
   **atomic-save детект**: `Rename(tmp, file)` при ноде на `file` →
   `ThumbStale` (контент изменился, путь нет).
7. **Модификация → полный `invalidate_node_caches`** (как в §4): ключ
   атласа — индекс ноды, точечного удаления нет; SQLite промахнётся по
   mtime сам (ключ — путь+mtime). Побочный эффект — перезапрос видимых
   тамбнейлов, дедуп в ThumbService удерживает шквал.
8. **Дроп/удаление нод синхронизируют вотчер** (`sync_watch_dirs` после
   Drop из T9, `delete_selected`, после Rename-событий) — §5 шаг 4.
9. **Тултип битой ноды** — `ScreenText` у курсора, клип-ширина 380 px,
   текст «Файл недоступен: <путь>» (§4), screen-space — константный
