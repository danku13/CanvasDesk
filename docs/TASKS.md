...
## Пост-v1 расширения (вне T0–T22)

### T23. Динамическая подсветка связей (brainstorm-focus)

**Промпт:**
```
Режим фокуса для брейншторма по плану docs/plans/T23-focus-highlight.md.
При включении (хоткей F / ПКМ-меню «Фокус на связях» / строка панели
настроек, config.toml) hover или выделение ноды подсвечивает все
инцидентные связи и ноды-соседи (акцентный цвет, утолщение, «дыхание»
один цикл ~1.6 с), остальная сцена плавно притемняется до ~35% яркости
(fade 150 мс). Окрестность — 1-hop (canvas-core focus_set, чистая функция
+ тесты). Затемнение — карточки/связи/текст (альфа инстансов и
TextArea.default_color); миникарта, порты и screen-space оверлеи не
затемняются. Формат .canvas не меняется, состояние рантаймное.
Критерий: на графе из 50+ нод контекст идеи читается мгновенно при
наведении; 5000 нод — fps не проседает (cpu_ms +<1 мс); работает в
оконном режиме и --desktop.
```

**Приёмка:** docs/plans/T23-focus-highlight.md §9.
