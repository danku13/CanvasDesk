...
## M6. Расширения и интеллект (BYOK / агентская интеграция)

> Новый milestone — выход за рамки базовой спецификации M1–M5. Две задачи: динамическая подсветка связей (помощь при мозговом штурме) и BYOK-формат / MCP-сервер для работы Hermes Agent (генерация Mindmap и структурированных заметок из любого текста).
...
**Приёмка:** `F3` или клавиша `B` включает режим фокуса; подсветка связей работает с модификаторами; затемнение регулируется в `config.toml`; производительность не падает на 5000 нод.

---

### T24. BYOK-формат и MCP-сервер для Hermes Agent
...
**Приёмка:** файл `docs/BYOK.md` описывает формат; `cargo build -p canvas-mcp` проходит; команда CLI `--byok-generate docs/sample.md --mode mindmap` создаёт `.canvas` с деревом нод; вставка в текущий канвас через контекстное меню работает; неизвестные поля из .canvas сохраняются (совместимость). MCP-сервер отвечает на запрос Hermes Agent в формате JSON-RPC.

---

### T25. (Опционально) SDK для BYOK-виджетов

**Цель:** шаблон виджета, использующий BYOK-формат для генерации динамических заметок или карт знаний из локального текста.
