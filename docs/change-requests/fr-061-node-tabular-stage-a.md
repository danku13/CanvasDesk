...
- **Статус:** этапы A+B+C+D выполнены (2026-09-23). Хвосты (2026-09-23, сессия web-85edad2d, решения владельца по 15 вопросам): **этап E** — kit-Row (D-15) поставлен, реализация следующим коммитом; **hit-хвосты реализованы** — свёрнутость блока Н-2 (runtime v1, Q4: состояние в памяти сцены, дефолт — развёрнут, сброс при загрузке), клик по заголовку (вся строка + chevron-ховер Pointer), экспандер описания «⋯ целиком ▾»/«▴ свернуть» («Раскрыть+авто»: автосворачивание при клике вне ноды/начале правки), Q3 «desc→манифест→проза» (`canvas_core::expr::first_prose_paragraph` — общая функция сцены и рендера, I-2). Q7: drag-разворот — только клик в v1 (drag после редизайна портов). Отложено на этап E/коммит 3: ellipsis формулы, VLM-ревью T9, Q6 (360–400 тяжёлым шаблонам), Q8 (алиасы). Порты свёрнутых строк: построчные точки исчезают вместе со строками (рёбра geometry — сторона-к-стороне, не зависят); пере-якорение слотами заголовка — v2 (нужно распределение полосы, D-7). Высота: growth-only (I-6) — свёрнутость не усаживает карточку (цена как у FR-050 Р-4).
- **Тип:** FR
- **Приоритет:** высоко
