...
- **Статус:** этапы A+B+C+D выполнены (2026-09-23; этап D — полировка: токены table.*/motion.*, i18n блока-заголовка RU/EN, DebugOverlay-направляющие Q9, rich-формулы O-5 с фиксом italic-маркера, D-8 описание+кламп). ОТЛОЖЕНО на отдельное согласование (hit-зоны app.rs, зона FR-060 не начата): свёрнутость блока/клик по заголовку, экспандер описания «⋯ целиком ▾»; также отложены ellipsis формулы (патологически узкие ноды, документированное поведение) и VLM-ревью (визуальная приёмка владельцем). Этап E (kit-Row) — постановка
- **Тип:** FR
- **Приоритет:** высоко
