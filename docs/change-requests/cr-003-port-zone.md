...
- `2026-09-16` — агент (аудит реализации всех CR/FR, main `984ca6b`): реализация подтверждена по коду (документ отставал: статус был «в анализе», код готов): настройка `port_zone_px` с пресетами `PORT_ZONE_PRESETS=[10,14,20,28,40]` (`settings.rs:169-193`, кламп при загрузке), UI-строка «Зона портов: N px» (`lib.rs:147-148`, клик-цикл `main.rs:3504`), зона — экранные px с пересчётом world/zoom (`port_at(..., tolerance/zoom)`), проброс в порты (`main.rs:4964`) и хэндлы рёбер (`main.rs:1966`), рост кружков (`cards.rs:278-280`, clamp 10..26); тесты `settings` (10), `test_port_hitzone_at_extreme_zooms`. Приёмка §13 (CR-003.1–2) — принята 2026-09-14. Статус `выполнено`.
