...
- [x] **W3.2** — 2026-09-25: миграция потребителей завершена (settings_ui 12, scheme_gallery_ui 10, template_ui 5, search_ui 3 → `MeasuredItem`; НОВЫЙ `Column::lay_out_measured/_with` — вертикальный F-13-симметричный API, +2 оракула бит-в-бит): потребители canvas-app/canvas-render 32 → 1 (демо kit_ui F-14 — решение W3.3 за владельца); workspace 2061/0, fmt/clippy -D warnings/wasm_gate --check зелёные; находка: `Spacer` в Column места по высоте не занимает — «распорки» скелета галереи зазоров не давали (статус-кво сохранено, решение за владельцем). Детали — docs/plans/fr-068-w3-consumer-migration.md §7.
...
- [x] Замер wasm-бандла (§Контракт-7) — исполнен 2026-09-26 (свежая среда, канонический `web_bundle.sh`: trunk 0.21.14 + wasm-bindgen 0.2.127 + wasm-opt -Oz): бандл **11.18 МиБ raw / 2.51 МиБ brotli** (wasm 11,516,622 raw / 2,595,466 brotli; gzip 4.66 МиБ — оценка сверху); загрузка ~2 с @25 Мбит/с (§8 ≤5 с ✓). Бюджет §8.8: **raw ≤8 ПРЕВЫШЕН** (+3.18 МиБ), brotli ≤4 — OK. A/B-атрибуция (wasm post-opt, binaryen 125 = версия CI, идентичная среда; метод верифицирован воспроизведением задокументированного замера W2 с точностью 0.08%): W2-снапшот 9,152,760 → +212 КиБ (48 коммитов фич: W3 Component-слой, Table v2, FR-073, FR-040 v2, FR-028 v2, icons, схемы 6→10) → **+2,124,689 байт — WebGL2-фолбэк (FR-WASM-02, `wgpu features=["webgl"]`)** → +37 КиБ (FR-074 + W4, net: taffy −) → main 11,532,033 (ae0a806) / 11,516,622 (93e553c + W3.3/tour). Binaryen 125 vs 133 — 8 КиБ (шум). Превышение raw — явное решение владельца (варианты: пересмотр бюджета — brotli/загрузка в норме; webgl за opt-in-фичей; статус-кво).
...
- `2026-09-26` — агент (сессия web-49ae985b, запрос владельца «замерь место, если влезет — проверим»): **§Контракт-7 закрыт — замер wasm-бандла исполнен** после двух волн отложек «диск среды 100%». Среда: свежая песочница 9.9 ГБ диск / 3.9 ГБ RAM / 2 ядра, тулчейн установлен с нуля (rustup minimal 1.98.1 stable + wasm32-unknown-unknown + trunk 0.21.14 + binaryen 125/133; rustflags `web_sys_unstable_apis` из .cargo/config.toml). Канонический `web_bundle.sh` на main 93e553c: **бандл 11,729,302 байт raw (11.18 МиБ) / 2,629,954 brotli (2.51 МиБ) / 4,891,998 gzip (4.66 — оценка сверху)**; wasm 11,516,622 raw / 2,595,466 brotli (−25% от wasm-opt); js-glue 141,333; tour.js 48,821 (FR-028 v2); загрузка ~2 с @25 Мбит/с ✓. Вердикт §8.8: **raw ≤8 ПРЕВЫШЕН** (+3.18 МиБ; превышен ДО W1 на default 8.72 и рос дальше), **brotli ≤4 OK**, загрузка OK. A/B-атрибуция роста W2→main (+2.27 МиБ wasm post-opt) — снапшоты ed0c0c4/772ab9c/eef589c/ae0a806 в идентичной среде (метод верифицирован: W2-снапшот 9,152,760 ≈ задокументированным агентам 9,159,641, 0.08%): **доминанта — WebGL2-фолбэк (FR-WASM-02, wgpu `webgl`) +2.03 МиБ**; 48 коммитов фич (W3, Table v2, FR-073, FR-040 v2, FR-028 v2, icons, схемы 6→10) — суммарно +212 КиБ; FR-074+W4 (taffy −) — net +37 КиБ; W3.3+tour — −15 КиБ. Binaryen 125 vs 133 — 8 КиБ шума. Попутные находки: (а) прод-деплой GitHub Pages `/app` — 404 на все ассеты (приложение в проде недоступно — проверка деплоя вне этого замера); (б) `web_bundle.sh` без brotli-CLI вердиктит по gzip-оценке «FAIL-compressed», тогда как реальный brotli в бюджете — CI с brotli вердиктит корректно; (в) история main переписана в линейную (59d77cb в предках), tooltip-фикс 0a77d89 и хотфикс W3.2 a11f6c9 — в main. Решение по превышению raw — за владельцем: (1) пересмотреть бюджет (brotli и загрузка в норме — сырой лимит 8 МиБ писался до webgl-фолбэка), (2) webgl за opt-in-фичей/ленивым выбором бекенда, (3) статус-кво с фиксацией в ACCEPTANCE. Гейты замера: сборка release зелёная, wasm-opt/wasm-bindgen 0.2.127 по канону; код не менялся — только замер + доки. Доки: FR-068 (статус/чеклист/changelog), ACCEPTANCE FR-068.7, worklog.

- `2026-09-26` — агент (лид + 3 параллельных агента-исполнителя, git-worktree, последовательный merge; сессия web-3e2a9c55, запрос владельца «заменяй и доделывай остаток бэклога»): **W3.3 исполнено — финал W3**. (1) Фундамент canvas-ui (лид, additive, коммиты 4f89415/aa23181): пад-семантика F-13 — `MeasuredItem::Text += pad_x` (суммарный горизонтальный пад, бит-в-бит ≡ проводке `Fixed{width_of+pad}`) и `+= h: Option<f32>` (явная высота дизайн-константой; находка: чип whatif `CHIP_HEIGHT 26` ≠ измеренной `16.9` — без `h` миграция не бит-в-бит); `TextMeasurer::wrap` — жадный перенос по словам (семантика прецедента `admin_ui::wrap_text`); +4 оракула (полный rect бит-в-бит, кнопочная проводка max(min_w,w+pad) на уровне выхода движка — round_px on freeze, wrap ≡ ручному оракулу RU/EN). (2) 2-a: витрина кита — секция Row v1 УДАЛЕНА (решение владельца по открытому вопросу M6), Table — единственная табличная секция (b5b5958); отрисовка не тронута (row_rows generic); оракул «Row ≡ Table» сохранён тестом (row-путь — только тест); ключ i18n kit.section.row удалён RU/EN. (3) 2-b: провода width_of→Fixed → `Text{pad_x,h}` — whatif бар (чипы/кнопки, rect-тесты зелёные без правки значений; chip_width/btn_width — только первый проход ширины бара), template чип категорий; `admin_ui::wrap_text` → делегат `TextMeasurer::wrap` (f095b22). (4) 2-c: аудит кандидатов FR-074 (2c435e9, без кода): z-index — не кандидат на band-пути хромировки (Painter::z_group ≠ полосы UiLayer; кандидат при переводе хромировки на Painter); Grid Auto/minmax — карточек тем 2 фиксированные (не оправдан), template сеток нет; Sticky — потребителей 0 (фича — решение владельца); статус-абзацы 6–8 в §9.3 каталога. Классификация остатка width_of (§9.3.1 закрыт): замеры контейнеров/screen-текст/канвас-домен — не проводки; docs_ui span-перенос — контентный движок вьюера (остаётся). (5) Гейты: workspace **2089 passed / 0 failed** (база 2072); clippy --workspace --all-targets -D warnings; fmt --check; wasm_gate --check. Попутный фикс main: tour_signal.rs (FR-028 v2, параллельная сессия) — doc-отступы clippy + fmt-дрейф. (6) Доки: каталог §9.3.6–8/§9.5, ui-kit §4/§5, ACCEPTANCE FR-068.10, worklog.
