...
- **Статус:** в работе (W0 выполнено 2026-09-25 — гейты зелёные; W1 выполнено 2026-09-25 — гейты зелёные, taffy opt-in за фичей, wasm +283.9 КБ raw post-opt; W2 выполнено 2026-09-25 — гейты зелёные: FlexLayoutEngine побитово-паритетен taffy (1000/1000 деревьев, 15/15 demos на общем golden), Shaper trait изолировал cosmic-text, wasm дельта +692 байта raw; **W3 выполнено 2026-09-25** — гейты зелёные: kit.rs 3541→58 строк (фасад-реэкспорт), `Component` trait + 7 модулей component/* (button/panel/dropdown/modal/text_field/list/row — Props/WidgetState/paint/hit_test, 6+row), лид + 4 параллельных агента (worktree, безконфликтный merge), workspace 2001 passed default / 373+2 taffy (2 фейла whatif_ui — ПРЕДСУЩЕСТВУЮЩИЕ на main, семейство документированных taffy-округлений), W3.1 пилот-миграция whatif_ui → measured-API (Child::fixed в canvas-app 42→32), каталог миграции docs/plans/fr-068-w3-consumer-migration.md; решение владельца: гейт «−30–50% Child::fixed» — перефиксировать на «0 Child::fixed в canvas-app/render» (оценка плана ~497 завышена ~9×: фактически 42 у потребителей, 142 из 184 глобально — оракулы/фикстуры); retained-state НЕ введён (perf W2 0.26 мс < 1 мс — KISS); wasm-замер размера (§Контракт-7) отложен — диск среды 100% (release-сборка не помещается), wasm-gate --check зелёный, дельта ожидается ≤ ~10 КБ raw (0 новых deps, код-мувы); ADR-0015 принят владельцем; W4 — по плану; замечание владельцу: абсолютный wasm-лимит raw 8 МБ превышен ДО W1 на default-бандле (8.72→8.83 МБ с web-sys 0.3.104 + unstable-API — вне рамок W2) — решение по §Контракту-7 за владельцем)
