...
- **Статус:** закрыто (2026-09-25) — гибрид реализован в W1 FR-068 (`trait LayoutBackend` + `NativeBackend` 1:1 + `TaffyBackend` за фичей + pilots + 10 HTML5 demos, гейты зелёные), затем W4 FR-068 вырезал taffy целиком — `FlexLayoutEngine` единственный движок (стратегия ADR-0015, конечное состояние достигнуто). Настоящий документ сохраняет историческую ценность постановки P1–P3 и адаптера taffy; актуальная карта вёрстки — `docs/ui-kit.md`.
