...
- [ ] Контраст-тесты покрывают все новые слоты (обе темы + пресеты); список известных исключений в этом документе (если вскроются).
- [ ] G1: `scripts/token_lint.sh` зелёный; grep-аудит цветовых литералов вне tokens/theme/тестов = 0.
- [ ] G4: `ThemeInfo.accent` приходит из палитры (проверка: смена `accent` в теме меняет хром виджетов); select-рамка/порты/каретка — от слота.
- [ ] G5: `HEADER_HEIGHT`/`CORNER_RADIUS`/типографика — из токенов; значения == прежним (тест равенства).
- [ ] G2: добавление пресета-файла не меняет `.rs` рендера (дифф-проверка T-046.4).
- [ ] Демо-сценарий PRD-0006 §6: смена темы → осмотр канваса (карточки, рёбра, гида, wheel, минимапа, тосты, диалоги, what-if, фокус-режим) — неперекрашенных поверхностей нет; перезапуск сохраняет выбор (десктоп; web — после F-9).
- [ ] Round-trip существующих `.canvas` не изменён (тесты canvas-core).
