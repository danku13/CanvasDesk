...
- `2026-09-16` — агент (аудит реализации всех CR/FR, main `984ca6b`): аудит: реализация подтверждена и расширена — инструмент `node_edit` (`canvas-mcp/lib.rs:218-233`; required `id`, поля `text/label/color/expr/x/y/width/height`), диспетчер (`main.rs:3970-4055`): только переданные поля, `null` — сброс, валидация (пресеты «1..6», width/height > 0, парсинг expr до undo), ответ — полная сводка ноды; тесты `mcp_node_edit_updates_only_given_fields`, `mcp_node_edit_validation`, schema-тест в canvas-mcp. Поле `expr` добавлено сверх плана (FR-013). Приёмка MCP-канала (§13 FR-005.1–3) — тест-долг владельца. Статус `выполнено`.
...
- `crates/canvas-mcp/src/lib.rs:175-320` — `TOOLS` (20 инструментов).
