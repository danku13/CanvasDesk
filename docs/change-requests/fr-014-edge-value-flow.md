...
- `2026-09-15` — агент: правка 3 (дефект UI-переключателя типа потока,
  воспроизведён на headless-стенде: ПКМ по связи → палитра → «Поток» →
  «Значение» — модель не менялась, в сохранённом файле
  `canvasdesk.flow.kind` не появлялся, downstream не пересчитывался).
  Причина: `PaletteAction::EdgeFlowKind` мутировал клон-снимок
  (`snapshot.edges.get_mut`) вместо живого канваса; в undo при этом уходил
  уже изменённый снимок (второй дефект того же кода). MCP-путь
  (`flow_set_kind`) мутировал канвас напрямую — потому MCP-тесты проходили,
  а интерфейс нет. Исправление: логика тогла вынесена в
  `SceneState::toggle_edge_flow` — мутация живого канваса, undo-снимок
  «до» (FR-006), `mark_dirty`, живой пересчёт; возвращает
  `Ok(applied)` / `Err(участники цикла)`; палитра при цикле показывает
  toast. Тесты: 4 регрессионных в `main.rs` (`palette_flow_toggle_*`):
  применение к живому канвасу + расчёт downstream (1680/3 = 560) +
  undo-снимок «до», обратный тогл с удалением поля и `MissingInbound`,
  no-op-случаи без накопления шагов, отклонение цикла с участниками.
  Проверка на стенде (Xvfb + XTEST): после тогла файл содержит
  `flow.kind: "value"`, на ребре лейбл «1680», у получателя — «560».
