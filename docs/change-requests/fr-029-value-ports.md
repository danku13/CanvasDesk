...
- `2026-09-22` (2) — агент (перекрёстная проверка, раунд 3): открытый вопрос
  FR-029 «проливание vs ручная правка параметра» **закрыт окончательно** —
  решение владельца **Р-5 FR-050**: пролитая строка не редактируется;
  ручная правка = удалить связь и ввести локальное значение; сценарий
  «быстро посмотреть» — `Ctrl+Z` (возврат связи) или what-if; состояния
  «перекрыта вручную» не вводится. Раунд также закрыл Н1–Н9 FR-050, в том
  числе Н8 → **Р-6** (именованный синтаксис выражений «Объект.Поле» — сразу;
  см. FR-044 Q1). Единственный оставшийся вопрос FR-050 — Н10 (природа
  авто-строки: «проекция» vs «настоящая строка», переформулирован простыми
  словами).
