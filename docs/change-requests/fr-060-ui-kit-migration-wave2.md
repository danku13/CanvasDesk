...
- **Статус:** ✅ выполнено (волна 2, 2026-09-23: autolink — kit::modal/list_rows+ScrollState/stack, palette — dropdown_menu (якорь-строка 6=10−4, якорь-зона бара ±1: зазоры 3 = −1+4) + list_rows, explain — modal (слот с полями) + take_while/match вместо break-клампов; app.rs: dialog — kit::modal+button_size измеренный (фикс h=150: 440×150 → измеренные ширина/высота, кнопки от текста, ellipsis вместо молчаливого клипа), menu/hotkeys — list_rows + WidgetState + Painter (панели с тенью — квады: params.w вне PaintItem), autolink_frame/palette_overlay — Painter+WidgetState; G5-аудит 3 модулей — 0 take(n)/break/truncate (Option::take — перенос состояния, не срез); G4-линт: +3 состояния (autolink/explain/palette через test_viewport); замер wasm: +1 229 Б ≈ 1,2 КБ ≤ 100 КБ (raw cdylib release, обе стороны одной процедурой); explain_frame — отрисовка осталась на screen_rect_quad (без теней, эквивалент Painter-конверсии; ~600 строк механики при нулевом визуальном эффекте — остаток волны))
- **Тип:** FR
- **Приоритет:** важно
- **Владелец:** агент сессии 2026-09-23 (реализация)
...
- **Создан:** 2026-09-23
- **Обновлён:** 2026-09-23 (реализация)
...
- `2026-09-23` — агент: **выполнено** (реализация волны 2; детали в строке статуса). Ключевые решения реализации: (1) min-маржа «viewport−20» dialog_rect autolink — мёртвый код при всех вьюпортах (доказательство в parity-тесте), устранена; (2) `kit::modal` для explain — слот = вьюпорт с полями `WIN_MARGIN` (симметричный инсет сохраняет и кламп-отступ, и центр — parity-тест ≡ прежней формуле); (3) строка с верхом ровно на кромке тела (0 видимых px) — окно кита исключает (рендер клиповал в ноль — видимой разницы нет; попутно убрано пересечение невидимой hit-зоны с футером); (4) flip бара палитры у нижнего края (прежний кламп перекрывал выделение) и прижим колонки к низу в вырожденном случае — системная политика кита (как у колонок/подсказок FR-059); (5) «Painter» меню/хоткеев — панели с тенью (params.w=0) остаются квадами (флаг вне контракта PaintItem), строки/состояния — кит; (6) explain_frame — отрисовка не переведена (screen_rect_quad — тот же band-конвейер без теней; ~600 строк механики при нулевом визуальном эффекте) — остаток волны; (7) G4-линт: +3 состояния (autolink_review/explain/palette — палитра через `test_viewport` оверрайд, клампы также модельными тестами palette.rs); (8) замер wasm — raw cdylib release cargo (trunk/wasm-bindgen в среде недоступны), обе стороны одной процедурой: 12 010 850 → 12 012 079 Б (+1 229 Б ≈ 1,2 КБ ≤ 100 КБ); (9) права app.rs — перечень зафиксирован в worklog при старте (dialog/menu/hotkeys + функции отрисовки мигрируемых поверхностей, трактовка задокументирована).
