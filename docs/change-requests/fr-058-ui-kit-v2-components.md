...
- **Тип:** FR
- **Приоритет:** важно
- **Владелец:** Super Z (сессия web-bea0078b)
...
- `2026-09-23` — Super Z (сессия web-bea0078b): реализованы все 5 компонентов v2 (TextField, ScrollState+list_rows+scroll_bar, Switch, Card, Icon) в `crates/canvas-ui/src/kit.rs` (только добавление — существующие сигнатуры/константы не менялись); 37 юнит-тестов (вставка/удаление/селекция/движение каретки на emoji+кириллице, clamp/max_offset/needs_scroll, list_rows с частичными строками, switch/card геометрия, icon_glyph полный маппинг); `docs/ui-kit.md` §7.2 (таблица компонентов + инвариант каретки); гейты fmt/clippy -D warnings/test зелёные; 0 правок canvas-app/canvas-render (проверка диффом). Статус → «реализовано».
