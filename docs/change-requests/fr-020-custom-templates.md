...
- `2026-09-16` — агент (аудит реализации всех CR/FR, main `984ca6b`): реализация не начата — `save_template`/`custom_template`/конвертация Numi-expr в params отсутствуют, пункта «Сохранить как шаблон» в палитре нет, каталог пользовательских шаблонов не заведён. Статус `выявлено` сохранён.
- `2026-09-16` — агент: **реализация v1 выполнена** (отдельный коммит FR-020).
  Реализовано: хранение custom — `~/.canvasdesk/templates/<id>/template.json`
  (решение владельца — единый корень с виджетами/кэшем; НЕ
  %APPDATA%/canvasdesk): `templates.rs` — `custom(root)` (скан, невалидные —
  warn+skip), `save_custom` (создание папок, пометка Custom), `delete_custom`
  (идемпотентно), `validate_custom_id` (3..64, `[a-z0-9.-]`, без `..` — защита
  от выхода за корень), `TemplateRegistry::all_with_custom(root)` (custom
  перекрывает built-in при совпадении id — override, инвариант 3);
  `TemplateSource {Builtin, Custom}` — поле манифеста, MCP `template_list`
  отдаёт `source`; `params_from_text` — общая точка извлечения параметров
  из Numi-листа (синхронизация FR-018 + сохранение FR-020).
  UI: пункт «Сохранить как шаблон» в группе «Действия» палитры выделения
  (только у шаблонных нод; замена ПКМ-меню — палитра заменила контекстные
  меню в FR-009/010) → сохранение снимка template-ссылки в манифест:
  id = slug имени ноды с транслитерацией кириллицы + суффикс `-2` при
  конфликте, категория `custom`, версия 1.0.0, типы параметров выводятся
  по единицам (`infer_param_type`); реестр перечитается сразу — шаблон
  доступен в палитре/wheel/MCP без перезапуска; тост с путём.
  Тесты: core ×5 (round-trip tempdir, override built-in, валидация id,
  идемпотентное удаление, пустой корень), app ×3 (slugify/транслит,
  infer_param_type, unique_custom_id), palette — пункт только у шаблонных
  нод; MCP `source` в тесте template_list.
  **Отклонения от исходного плана (осознанные, v1):**
  - Диалог сохранения (имя/категория/описание/чекбоксы параметров) —
    отложен: v1 сохраняет сразу с консистентными дефолтами (имя — подпись
    ноды, категория custom, все параметры шаблона становятся params).
    Шаблон правится вручную в `template.json` (файл — источник истины).
  - `extract_variables`/`parametrize` для НЕ-шаблонных expr-нод — v2:
    требует рендер `Expr` в текст (перевод Var→$Param в произвольной
    формуле); v1 сохраняет шаблонные ноды, где формула уже в $param-виде.
  - Иконка custom — квад-иконка `custom` (решение владельца FR-018 — без
    SVG); замена иконки пользователем — v2.
  - MCP `template_create_from_node` — v2 (по доке).
  Статус `выполнено (v1)`.
