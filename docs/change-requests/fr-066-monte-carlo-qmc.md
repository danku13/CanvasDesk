...
- **Статус:** выполнено (P1/P2/P3, 2026-09-24)
- **Тип:** FR
- **Приоритет:** важно
...
- `2026-09-24` — агент: реализовано (статус `выполнено`). Три фазы одним
  слоем: **P1** `expr/mc.rs` (McConfig/McMode/Distribution, resolve →
  построчные подмены, ChaCha8-сэмплы, чанки 256/задача rayon `par_chunks`
  + однопоточный фолбэк) + sibling `flow::propagate_monte_carlo`
  (`propagate_with_lines` НЕ тронута — контракт §5.1); **P2** QMC
  (`sobol_burley` 0.5, default-features=false — SIMD выключен; сэмпл
  [0,1) → clamp ε → inverse-CDF statrs; Пуассон — бинарный поиск по
  DiscreteCDF, λ ≤ 1000), collect-then-reduce квантили (sort по
  run-индексу → builtin `percentile_inc`), синтетический
  `FlowSolutions` (P-метки P50/P90/P99 в named), engine-метаданные
  §5.7.4 (raw JSON, паттерн whatif.rs; stale при рассинхроне версии);
  **P3** MCP `monte_carlo_run` (ToolSpec native-only §5.8 + dispatch arm
  за фичей `qmc` canvas-scene; анализ на хвостовом P90-квантиле через
  синтетический снимок — `analyze.rs` без правок) + `skills/` (v3:
  модель-verify, каталог, README 41, CHANGELOG — тем же коммитом).
  Отступления от плана (согласованы с контрактами документа): (1) сид
  §5.7.2 — буквальный `hash ⊕ seed ⊕ run_idx` коллапсится при N = 2^k
  (XOR поглощает scenario_seed < 2^k — множества рёнов совпадают,
  побитово одинаковые квантили у разных сидов; ловится тестом
  дисперсии) — тройка смешивается FNV-1a 64 поверх LE-байтов (детерминизм
  сохранён, контракт «тот же seed → тот же результат» выполняется
  дословно); (2) `Distribution::LogNormal { mean, sd }` — параметры в
  НАТУРАЛЬНОМ пространстве (демо-числа FR «LogNormal(μ=120, σ=15)»
  осмысленны; FR-063-функции остаются в лог-пространстве); (3) params →
  построчные подмены `line_exprs` (не `node_values`: несколько
  параметров одной ноды и шаблонные листы — паттерн whatif_set_param),
  единица придаётся формулой-потребителем (FR-050 Н5); (4) ядро
  деградирует тихо (skipped_params/failed_runs), MCP валидирует строго.
  Гейты: `cargo test -p canvas-core --features qmc` — 399+/399 (12
  юнит-тестов mc + 7 интеграционных гейтов mc_engine.rs:
  побитовая воспроизводимость, квантили эталона №5 ±1 %,
  QMC-дисперсия < MC, severity на P90, stale, деградация, бюджет);
  `cargo test -p canvas-scene --features qmc` — 5 e2e
  monte_carlo_run; canvas-mcp/headless — счётчики 41 native / 40 wasm;
  skills_sync зелёный; wasm-гейты --check зелёные (qmc не собирается
  на wasm32 — реестр без monte_carlo_run).
