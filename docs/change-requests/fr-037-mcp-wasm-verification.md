...
|----|-------|---------------------------|
| MW1 | M | **Крейт `canvas-scene`.** Перенос SceneState (модельные поля + методы, `main.rs:551+`), модуль `mcp` (диспетчер + хелперы + `next_free_id` из `canvas-app/src/lib.rs:359`, реэкспорт для UI), `Viewport` + синк в `on_mcp_wake`; UI-поля `selected/selected_nodes/dragging` → поля App (~99 механических правок `self.scene.selected` → `self.selected`); перенос ~40 MCP-тестов (~2970 строк). Приёмка: нативные гейты зелёные (тестов ≥ 1034, те же имена/ассерты); `cargo check --target wasm32-unknown-unknown -p canvas-scene` ✓; `RUST_TEST_THREADS=1 cargo test --target wasm32-wasip1 -p canvas-scene` ✓ (~40 тестов в wasmtime); `cargo test -p canvas-shell` (pipe round-trip) ✓ **Выполнено 2026-09-19:** крейт `canvas-scene` (scene/measure/mcp/view + tests.rs, ноль новых внешних зависимостей); main.rs 15026→11171 (после ребейса на волну CP4/CP6; вырезано ~4,7 тыс. строк); UI-поля — поля App (73 замены); viewport-зеркало + синк в `on_mcp_wake`; инъекция уровня 2 измерений в `main()`. Приёмка: fmt/clippy чисто; workspace 1077/0 (≥1034, включая 42 теста CP6 и 50 перенесённых MCP-тестов — имена/ассерты без изменений); check wasm32-unknown-unknown ✓; wasip1 53/53 в wasmtime (oracle-гейты эталонов CP1/CP3/CP5); canvas-shell в составе workspace ✓. Вью-типы SpillView/WhatIfNode — в canvas-scene/src/view.rs, canvas-render реэкспортирует (слои core → scene → render → app) |
...
| MW3 | M | **`canvas-mcp-headless`.** `HeadlessSession` (SceneState + `TemplateRegistry::builtin()`) с impl `AppTransport`; bin `canvasdesk-mcp-headless` = `run_stdio_with_transport(Some(session))`; lib-тесты протокольного цикла: initialize (эхо версии 2025-06-18) → tools/list (27) → tools/call → graph_apply мини-эталон №1 → flow oracle ±1% → analyze_bottlenecks (ρ-гейт CP5) → негативные ветки (isError неизвестного инструмента, −32601, −32700, batch, notification-тишина). Приёмка: lib-тесты под wasip1 ✓; ручная сессия: `wasmtime run target/wasm32-wasip1/debug/canvasdesk-mcp-headless.wasm` отвечает на initialize/tools_list через stdio **Выполнено 2026-09-19:** крейт `canvas-mcp-headless` (лист в DAG, ноль новых внешних зависимостей: canvas-mcp/canvas-scene/canvas-core/serde_json/anyhow); `HeadlessSession` за `AppTransport` — `send_line` воспроизводит конверт `on_mcp_wake` буквально (parse_envelope → id → mcp_unwrap_call → mcp_dispatch → build_result | build_call_error; notification — тишина; битый конверт — build_error null-id; viewport-зеркало уже в SceneState — синк не нужен); bin `canvasdesk-mcp-headless` = `run_stdio_with_transport` (reconnect — no-op, in-process). 12 lib-тестов протокольного цикла: initialize-эхо 2025-06-18, tools/list 36, tools/call roundtrip (text+structuredContent), CP3-гейт graph_apply мини-эталон №1 с oracle ±1 % (те же числа, что canvas-scene), CP5-гейт ρ-лестница (0.417 none → ×2 warn 0.833 → ×5.35 overload 2.229, бейджи), негативные ветки (isError неизвестного инструмента, isError ошибки инструмента, −32601, −32700, batch, notification-тишина), персистентность состояния. Приёмка: нативно 12/12; wasip1 12/12 в wasmtime; check wasm32-unknown-unknown ✓; ручная сессия в wasmtime: initialize (эхо 2025-06-18, serverInfo canvasdesk) → tools/list (36) → tools/call canvas_info (structuredContent, isError=false), EOF — штатный выход (exit 0) |
| MW4 | M | **Гейт, CI, документация.** `scripts/mcp_wasm_e2e.py` (драйвер сессии: build wasip1 → wasmtime → сценарий MW3 + сохранение лога сессии для разбора падений) + `scripts/mcp_wasm_gate.sh`; CI `wasm-check` += `-p canvas-mcp -p canvas-scene -p canvas-mcp-headless` (компиляция; исполнение — локально, прецедент ADR-0011); AGENTS (структура workspace, «Сборка и тесты»: wasm-гейт теперь покрывает MCP-слой), SPEC §13/§3, ACCEPTANCE § «MCP-WASM-верификация» (сценарий 5 минут), wasm-port.md — примечание (уже добавлено этим документом). Приёмка: `scripts/mcp_wasm_gate.sh` — полный зелёный прогон в чистом контейнере; CI зелёный; документация синхронна **Выполнено 2026-09-19:** `scripts/mcp_wasm_e2e.py` (драйвер: build wasip1 → wasmtime → initialize/tools/list 36 → graph_apply oracle ±1 % → ρ-лестница CP5 → isError/−32601/batch/notification-тишина → EOF exit 0; лог target/tmp/mcp_wasm_session.log) + `scripts/mcp_wasm_gate.sh` (3 ступени: check → wasip1-тесты scene 53 + мост 13 + headless 12 = 78 в wasmtime → e2e-сессия); CI `wasm-check` += canvas-scene/canvas-mcp-headless; AGENTS (структура + MCP-wasm-гейт), SPEC §3/§13, ACCEPTANCE §29, ADR-0012 → «принято», wasm-port.md — примечание актуализировано (W2-вынос исполнен). Гейт: полный зелёный прогон, exit 0 |
| MW5 (опция) | S | **Инспектор-сессия владельца.** `scripts/mcp_wasm_inspector.sh`: обёртка для `npx @modelcontextprotocol/inspector` поверх wasmtime-запуска headless-сервера — живая ручная проверка MCP без Windows (устраняет зависимость ручной приёмки MCP от Windows-машины). Приёмка: инспектор подключается, tools/list отображается, graph_apply из UI инспектора сходится с oracle **Выполнено 2026-09-19:** `scripts/mcp_wasm_inspector.sh` — два режима: web UI (по умолчанию, сервер вписан позиционной целью: `--web wasmtime run …canvasdesk-mcp-headless.wasm`, браузер → 127.0.0.1:6274, URL с токеном печатает инспектор) и `--check` — автоматическая приёмка инспектором как MCP-клиентом: initialize → tools/list (36, graph_apply в списке) → tools/call graph_apply мини-эталон №1 → oracle ±1 % (числа/батч импортируются из `scripts/mcp_wasm_e2e.py` — один источник истины). Нюансы найдены разведкой: версия 2.7.0 запинена (`INSPECTOR_PACKAGE`, воспроизводимость), аргументы инструментов с массивами — только через недокументированный в help-хвосте `--tool-args-json` (значения дословно, без key=value-коэрции: `--tool-arg` роняет массив operations — «отсутствует параметр»); schema-portability предупреждения инспектора идут в stderr, stdout — чистый JSON. Первый запуск npx требует npm registry (node 18+), дальше — кэш. Гейты не тронуты (npx — внешняя зависимость, инспектор — ручная способность владельца, не CI). Приёмка: `--check` зелёный (36 инструментов, 5 чисел oracle ±1 %, exit 0); web UI — HTTP 200, сервер предподключён |
...
- 2026-09-19 — **MW5 реализован** (сессия агента, приказ «Бери в работу MW5 (S)»):
  `scripts/mcp_wasm_inspector.sh` — обёртка официального инспектора
  `@modelcontextprotocol/inspector@2.7.0` (запинен, `INSPECTOR_PACKAGE`)
  поверх wasmtime-запуска headless-сервера. Режимы: web UI (интерактивная
  сессия владельца, сервер предподключён, токен-URL печатает инспектор)
  и `--check` — автоприёмка инспектором-клиентом (initialize → tools/list
  36 → graph_apply мини-эталон №1, oracle ±1 % из mcp_wasm_e2e.py;
  артефакты — target/tmp/mcp_inspector_*.json). Нюанс CLI 2.7.0:
  массивные аргументы — только `--tool-args-json` (`--tool-arg key=value`
  коэрцитует в строку → «отсутствует параметр operations»). Приёмка
  MW5 зелёная: инспектор подключается, tools/list отображается (36),
  graph_apply из инспектора сходится с oracle ±1 % (exit 0); web UI —
  HTTP 200. Гейты/CI не менялись (npx — внешняя зависимость — не для
  автономных гейтов; Rust-код не тронут). Докам: AGENTS («Сборка и
  тесты»), SPEC §13, ACCEPTANCE §29 (FR-037.10 — конкретный сценарий),
  статус FR-037 → MW1–MW5.
- 2026-09-19 — **MW4 реализован — FR-037 закрыт (MW1–MW4)** (сессия
  агента): `scripts/mcp_wasm_e2e.py` — драйвер реальной MCP-сессии в
  wasmtime (полный сценарий гейтов CP3/CP5 + негативные ветки, лог
  сессии для разбора падений); `scripts/mcp_wasm_gate.sh` — гейт одной
  командой (check → 78 wasip1-тестов → e2e); CI `wasm-check`
  расширен (scene/headless); AGENTS/SPEC §3+§13/ACCEPTANCE §29
  синхронизированы; ADR-0012 → «принято» (приказ владельца «работай
  сам» = исполнение плана). Полный прогон гейта — зелёный (exit 0);
  нативный регресс — ноль (workspace 1089/0). MW5 (инспектор)/MW6
  (файловый режим) — опции, по решению владельца.
- 2026-09-19 — **MW3 реализован** (сессия агента, приказ «работай сам»):
  крейт `canvas-mcp-headless` (лист в DAG) — `HeadlessSession` за
  `AppTransport`: `send_line` воспроизводит конверт `on_mcp_wake`
  буквально (parse_envelope → id → mcp_unwrap_call → mcp_dispatch →
  build_result | build_call_error; notification — тишина; битый конверт
  — build_error null-id); bin `canvasdesk-mcp-headless` =
  `run_stdio_with_transport` (reconnect no-op). 12 lib-тестов
  протокольного цикла (initialize-эхо; tools/list 36; roundtrip
  text+structuredContent; CP3-гейт graph_apply мини-эталон №1 oracle
  ±1 %; CP5-гейт ρ-лестница 0.417/0.833/2.229; −32601/−32700/isError/
  batch/notification-тишина; персистентность). Нюансы: аргументы с
  переносами строк — только через serde_json (сырые \n в format! дали
  control-character в конверте); массивные результаты (nodes_list) —
  text-контент, structuredContent только для объектов (FR-034).
  Гейты: workspace 1089/0 (1077 + 12); wasip1 12/12 в wasmtime; check
  wasm32-unknown-unknown ✓; ручная wasmtime-сессия: initialize →
  tools/list 36 → tools/call canvas_info, exit 0 по EOF.
- 2026-09-19 — **MW1 реализован** (сессия агента; сабагент MW1 + доводка
  ребейса): крейт `canvas-scene` — SceneState модельного слоя (без
  UI-полей) + viewport-зеркало `Viewport{x,y,zoom}` + `mcp_dispatch`
  (27 инструментов) + measure-слой (уровень 1 в крейте, уровень 2 —
  инъекция `install_measured_reserve` из canvas-render) + 50
  перенесённых MCP-тестов (имена/ассерты без изменений) + паритет-тест
  констант. canvas-app: UI-поля ввода — поля App; on_mcp_wake — синк
  зеркала. Ребейс на параллельную волну CP4/CP6 (83d43a9): конфликт
  main.rs разрешён, вью-типы SpillView/WhatIfNode перенесены из
  canvas-render в canvas-scene/view.rs (реэкспорт, слои core → scene →
  render → app). Гейты на итоговом дереве (поверх MW2): fmt/clippy
  чисто; workspace 1077/0; wasip1 canvas-scene 53/53 в wasmtime.
