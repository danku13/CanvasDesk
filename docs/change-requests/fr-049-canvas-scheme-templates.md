...
- **Статус:** выполнено (v1 2026-09-21; контент v2 2026-09-22 — описания нод, множественные связи, трассируемость фич; MCP v2 2026-09-22 — Q5 закрыт: schemes_list/schemes_apply)
...
- `2026-09-22` — агент (запрос владельца «подтянуть функционал MCP под
  обновления и полностью проверить работу MCP»): **Q5 закрыт —
  MCP-инструменты галереи v2**: `schemes_list` (реестр — те же 6 пакетов,
  что видит пользователь в галерее, RU-первично, в text-контенте FR-034)
  и `schemes_apply` (вставка как «Открыть»: ремап id, один undo-шаг,
  пересчёт, bbox, flow активного состояния; неизвестный id — isError без
  undo-шага) в `mcp_dispatch`/`tools/list` (36 → 39 инструментов, +
  `lineage` FR-048 X2 — см. SPEC §13). Инвариант «MCP-видимость = UI»
  закреплён для инструментов чтения: `flow_recalc`/`analyze_bottlenecks`/
  `lineage`/flow-поле `graph_apply`+`schemes_apply` считают АКТИВНОЕ
  what-if состояние (СВЕЖИМ пересчётом с подменами — ленивые мутации
  CR-012 не искажают отчёт; ранее flow/analysis считали БАЗУ — при
  активном сценарии агент видел не то, что пользователь) + авто-строки
  FR-050 Р-4 (`autoRows`) в ответах. Тесты: +4 canvas-scene (реестр,
  вставка/undo/оракулы, lineage-дерево/негативные, паритет what-if),
  e2e-сессия mcp_wasm_e2e.py расширена (шаги 4a–4d + негативы),
  inspector 39. Гейты: fmt/clippy/нативные (454+15+12+85)/wasip1/mcp-wasm
  (e2e сошлась) — зелёные.
...
- `2026-09-22` — агент (запрос владельца — переделка сценариев): контент v2
  «шаблон = витрина фич» (PRD-0008 §7.2.1). (1) Формат пакета расширен:
  `SchemeNode.label` (заголовки групп, стандарт JSON Canvas),
  `SchemeEdge.fromLine/fromOutput/toParam` (FR-025/FR-029-адресация;
  валидатор — адресация только у value-рёбер, `fromLine XOR fromOutput`,
  непустые имена; инстансер переносит поля — автотесты).
  (2) Все 6 схем переписаны: заголовок-объект + пояснение в КАЖДОЙ ноде
  (первая строка — проза для адресов «Объект.Поле»), пучки ≥2 рёбер одной
  пары (main stage FR-042), ноды с ≥3 value-связями (fan-in/fan-out),
  `toParam`-проливание в 5 схемах, `fromLine`-построчные истоки в бюджете,
  `utilization`/`mm1` + Percent-анализ FR-016 в capacity-service (Warn +
  Overload), цепочки 2–4 хопа с ветвлением (PRD-0007-готовность), hint с
  приглашением D5 в каждой. (3) Фикс ядра: `inbound_slots_with_lines`
  теперь принимает `NamedOutputs` — построчные бейджи приёмников
  `fromOutput`-рёбер больше не краснеют «вход отсутствует» (регресс-тест
  `inbound_slots_resolve_named_outputs`; вызов из `scene.rs` синхронизирован).
  (4) Тесты: 8 инвариантов контента + обновлённые оракулы (canvas-core 310,
  scene +12; гейты fmt/clippy/test зелёные). (5) Доки: PRD-0008
  §7.2/§7.2.1/§8/§16, SPEC §5.3, scheme-gallery.md, ACCEPTANCE.md
  (FR-049.4/FR-049.9 обновлены, FR-049.10 добавлен) — синхронизированы.
  Бюджет G5: 32.4 КБ ≤ 100 КБ.
- `2026-09-22` (2) — агент (решения FR-050 Н6 + Р-6, раунд 3): владелец
  подтвердил перевод схем с позиционных входов на адресную модель;
  `toParam`-адресация рёбер уже выполнена контентом v2 (запись выше) —
  остаток Н6: перевод формул на именованные ссылки «Объект.Поле» (Р-6,
  этап F дорожной карты FR-050); позиционные входы остаются
  легаси-совместимыми (ADR-0003).

- `2026-09-22` — агент (аудит CJM по запросу владельца «найди то, что
  проходит формальный чек, но ломает CJM»): (1) вердикты 4 схем звали
  «Выделите итог — цепочка расчёта покажет...», а UI цепочки (PRD-0007
  X2) не реализован (0 ссылок `lineage` в canvas-app) — приглашения
  переписаны под живые фичи (правка чисел, сценарии, пучок-веер);
  (2) хинт intro-whatif учил несуществующему «выделите строку» с
  перепутанным порядком — двойной клик по строке ВНЕ режима открывает
  правку базы, нарушая обещание «файл не меняется»; хинт исправлен
  (режим кнопкой внизу → двойной клик по строке); (3) 4-й чип категории
  («Бизнес», unit-economics) молча срезался раскладкой (120×5=560 >
  inner 536 при PANEL_W 560) — ширина чипа 120→108, добавлен инвариант
  `chips_all_categories_fit` (1280/1024/800); (4) входные ноды были
  красным пресетом «1» — конфликт с красной рамкой перегрузки FR-016:
  входы переведены на «5» (циан), пресет «1» в схемах запрещён тестом
  `schemes_avoid_red_node_fills` (§7.2.1.7 PRD); (5) жаргон D3
  («проливание», «M/M/1», «именованные выходы») вычищен из
  description_ru/en, формулировки уточнены («почти две трети» при
  0.625, «итог проливается в доллары на месяц» → внятная фраза);
  добавлены регресс-тесты пользовательских путей `scheme_cjm_tests`
  (canvas-app): геометрия схем после авто-роста высот с реальным
  шейпингом (наложения/вместимость групп — чисто), D6-сценарий с
  подменой строки маркетинга (1500→1800/18000→21600, база цела),
  D5-правка входа с перегрузкой mm1 (человекочитаемая диагностика).

- `2026-09-25` — агент (валидация и расширение готовых схем по запросу
  владельца «валидируй готовые схемы, доработай их и приложи еще схемы»):
  (1) валидация: все 6 схем v1.1.0 зелёные по всем гейтам (реестр 8/8,
  oracle+инварианты canvas-scene 30/30, CJM canvas-app 3/3, бюджет G5
  53.6/100 КБ) — вычислительных дефектов не найдено. (2) Доработка:
  вердикты unit-economics / project-budget / renovation-estimate получили
  контрольные числа по образцу capacity-service (1.8 / бюджет 16925 /
  смета со скидкой 1305), версии → 1.2.0. (3) Новые 4 схемы (каталог
  6 → 10, 96/100 КБ) по направлениям шаблонного аудита:
  `support-staffing` (Эрланг C + mmc + littles_law + utilization(c),
  ρ=0.8 без перегрузки на дефолтах), `investment-case` (npv с нулевым
  годом, irr на зеркальном потоке `0 $ - capex`, cagr на деньгах,
  relay-строка capex в NPV для чистой цепочки 3 хопа), `cohort-launch`
  (cohort_ltv 6 аргументов, ценность волны, здоровье/эффект),
  `runway` (sum ×4, чистый расход, рунвей 54 мес); все — с EN-контентом,
  fromOutput/toParam-адресацией, пучками и value-оракулами. (4) Гейты:
  +4 value-оракула; новый инвариант `hint_and_verdict_notes_stay_silent`
  (пресет «4» — hint/вердикты/try — обязан молчать ЦЕЛИКОМ во всех
  схемах: проза с числами не вычисляется). (5) Фикс ядра FR-071:
  `refine_by_swaps` обменивал y-позиции дословно — при разных высотах
  соседей рвал накопительный стек ROW_GAP (нахлёсты cohort-launch
  plan×retention); обмен теперь пере-стекует колонку от текущего верха
  (`restacked_column`, общая модель стоимости `column_state_cost`) +
  новая стадия `refine_by_insertions` (перенос ноды на другую строку
  колонки — добирает диагональные каналы, capacity-service 1→0
  пересечений); регресс-тест `swap_refine_keeps_row_gaps_with_unequal_heights`.
  (6) Синхронизация устаревших гейтов шаблонного аудита: счётчики 45 → 61
  (merged 62), lb-версия 1.2.1, категории backend 14 / network 6;
  галерея при 10 схемах прокручивается (`layout_clamps_to_small_viewport`
  переформулирован под clamp_scroll). (7) Доки: PRD-0008 §7.2 (каталог
  10 схем), scheme-gallery.md §5, ACCEPTANCE.md — синхронизированы.
  Гейты: canvas-core 400 / canvas-scene / canvas-app — зелёные,
  fmt/clippy -D warnings чисто.
