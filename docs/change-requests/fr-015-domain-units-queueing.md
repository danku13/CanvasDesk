...
- **Статус:** выполнено (v1 — минимальный mm1/mmc, без `Value::Struct`)
...
- `2026-09-16` — агент: реализация v1 по решению владельца («минимальный
  ...
  W = W_q + 1/μ, Erlang-C через рекуррентную Erlang-B — без факториалов,
  устойчива при c ≤ 1000); `utilization(λ, μ[, c])` → `Percent` (0..1,
  без ограничения сверху); `littles_law(λ, W)` → `Count`; `erlang_c(λ, μ, c)`
  → `Percent` (при ρ ≥ 1 — 1.0, SLA-ответ). Перегрузка ρ ≥ 1 в mm1/mmc —
  новый `EvalError::Overload` (красная строка на карточке). Файл —
  `expr/queueing.rs` (вместо полного `units.rs`: таблица единиц и
  conversions уже в FR-013, дублировать нечего); `EvalError` лишился `Eq`
  (f64 в Overload, PartialEq сохранён). Оценка «P_wait ≈ 0.21» из исходного
  документа неточна: формула даёт 0.29976 для (1000 rps, 600 rps, 3) —
  тесты зафиксированы по формуле. 17 golden-тестов (`tests/expr_queueing.rs`),
  включая стык с FR-014 (`mm1($in, …)`). Struct-вывод `{utilization,
  queue_length, wait_time}` — v2 (в план не возвращать до запроса владельца).
