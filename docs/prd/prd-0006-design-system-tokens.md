...
- `2026-09-21` — агент: **этап D4 (F-8) закрыт — FR-047 «Темы-пресеты как данные» (статус «выполнено»)**: Q3 закрыт решением владельца (состав — 7 пресетов: Nord, Dracula, Catppuccin Mocha/Latte, Solarized Light, Tokyo Night, Gruvbox Dark); пресет = JSON `design/tokens/themes/*.json` (36 слотов) + строка реестра `canvas_core::theme_presets::PRESETS` — G2 замерен (добавление пресета не трогает рендер, ≤ 30 мин); разбор+валидация+кэш OnceLock в canvas-core (wasm-безопасно), `ThemeColors::from_settings` — единая точка (13 точек app.rs), dropdown в модалке FR-039 (отклонение от AC-3.2 «карточки» — задокументировано в FR-047), `Settings.theme_preset` (round-trip, serde-default совместимость); контраст-машина G3 покрывает каждый пресет; F-9 (web CSS-переменные + persist) — остаётся на D4b; D5 (демо-гейт, ручная приёмка) — следующий этап.
