...
- **Статус:** в работе (D1–D3 выполнены FR-046 v1; D4/F-8 выполнено FR-047; D5 — демо-гейт)
...
- **Q3.** Состав встроенных пресетов F-8 (какие 2–3 палитры) — предложение: Nord, Catppuccin Mocha, Solarized Light. Решает: владелец (по демо). **Закрыт 2026-09-21 (FR-047):** владелец запросил 5–7 пресетов из расширенного перечня; принято 7 — Nord, Dracula, Catppuccin Mocha, Catppuccin Latte, Solarized Light, Tokyo Night, Gruvbox Dark (Monokai/GitHub/VSCode Dark Modern/Solarized Dark — пул «позже», добавляются одной строкой реестра).
...
- [x] G3: контраст-тесты покрывают все новые слоты на обеих темах (и пресетах, если F-8 принят); известные исключения задокументированы в FR-046. *(FR-047: пресеты покрыты теми же порогами — графика ≥3:1, тексты ≥4.5:1)*
...
- [x] F-8: пресет добавлен как данные, рендер-код не менялся (дифф-проверка). *(FR-047: 7 пресетов — JSON + строка реестра; маппинг и рендер универсальны)*
...
- `2026-09-21` — агент: статус → `в работе (FR-046 создан)`; создан реализационный FR-046 (`docs/change-requests/fr-046-design-tokens.md`, этап D0 дорожной карты §13): решения Р-1..Р-10 (формат/размещение токенов, слоты v2, severity/minimap как компонентные палитры, единый акцент, прокидывание темы, пресеты, web), план T-046.1..5, 10 инвариантов; номера вопросов Q1–Q6 остаются открытыми до демо-гейтов соответствующих этапов.
- `2026-09-21` — агент: **этапы D1–D3 закрыты (FR-046 v1, статус «выполнено (v1)»)**: D1 примитивы (`design/tokens/*.json` + `canvas_core::tokens` + 4 паритет-теста), D2 ThemeColors v2 + миграция ~25 бродячих констант + контраст-тесты F-5 (исключения задокументированы в FR-046), D3 единый акцент G4 (включая виджет-мост; санкционированное изменение #3B82F6→#659CF7) + метрики G5 (алиасы токенов) + токен-линт F-7. D4 (пресеты F-8, web F-9) и D5 (демо-гейт CJM, ручная приёмка) — следующий этап; G1/G3/G4/G5 замерены, зафиксированы в ACCEPTANCE.md; G2 — после D4.
- `2026-09-21` — агент: **этап D4 (F-8) закрыт — FR-047 «Темы-пресеты как данные» (статус «выполнено»)**: Q3 закрыт решением владельца (состав — 7 пресетов: Nord, Dracula, Catppuccin Mocha/Latte, Solarized Light, Tokyo Night, Gruvbox Dark); пресет = JSON `design/tokens/themes/*.json` (37 слотов, включая stage_dim FR-042) + строка реестра `canvas_core::theme_presets::PRESETS` — G2 замерен (добавление пресета не трогает рендер, ≤ 30 мин); разбор+валидация+кэш OnceLock в canvas-core (wasm-безопасно), `ThemeColors::from_settings` — единая точка (13 точек app.rs), dropdown в модалке FR-039 (отклонение от AC-3.2 «карточки» — задокументировано в FR-047), `Settings.theme_preset` (round-trip, serde-default совместимость); контраст-машина G3 покрывает каждый пресет; F-9 (web CSS-переменные + persist) — остаётся на D4b; D5 (демо-гейт, ручная приёмка) — следующий этап.
