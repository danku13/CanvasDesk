...
- **Статус:** в работе (U0–U3 и U5 выполнены — FR-051/FR-052/FR-053/FR-054; U4 — в работе по приказу владельца «Продолжай U4», FR-055: kit v1 F-8, DebugOverlay F-10/G6, витрина, перенос пилотов)
...
| V-3 retained-дерево + taffy в PoC | отложен (v2) | Переписывание 20+ поверхностей несоразмерно; избыточность для 8–10 панелей; интерфейс примитивов V-5 держит дверь открытой. Количественное подтверждение и триггеры эскалации — **ADR-0013** (2026-09-23: taffy 0.14 flexbox-only ≈ 108.5 КБ wasm = весь G7-остаток волны, grid ≈ 376 КБ; усиление кита без зависимости — FR-062) |
...
- [x] G1: демо-поверхность добавлена правками ровно в реестре + 1 функции контента (дифф-проверка; 0 правок вводных цепочек/renderer) — доказано интеграцией X2-окна проверки цепочки (FR-052) и choice_menu (FR-050 этап C).
- [x] G2: pick-матрица автотестом: Block/Capture-поверхности перехватывают клик под собой; PassThrough/Passive — нет (ui_registry-тесты + кадровый assert_backdrop_is линта).
- [x] G3: все существующие z-тесты (zorder.rs:185–412, zorder_smoke, stage-тест) зелёные на merge.
- [x] G4: layout-линт: 3 окна (1280×800, 1024×640, 800×560) × RU/EN — 0 пересечений интерактивных rect'ов одного слоя, 0 переполнений, 0 выходов за вьюпорт (сквозной CI-линт `app::ui_layout_lint`, F-11; нашёл и погасил 3 реальных налезания — см. ретро).
- [x] G5: в мигрированных панелях 0 ручных клампов/срезов (grep-аудит `take(`/`break`-клампы/`truncate_chars`; эвристики `CHAR_W_FACTOR` docs и `chars·7.5` палитры удалены).
- [x] G6: F9-оверлей показывает слои/rect'ы/имя под курсором; пересечения подсвечены; работает в wasm — выполнен на U4 (FR-055): DebugOverlay — полоса UiLayer::Debug того же ScreenBand-пути (web == натив, wasm-гейты зелёные), headless-тесты модели.
- [x] G7: 0 новых runtime-зависимостей; wasm-гейты зелёные. Замер прироста бина ≤100 КБ закрыт FR-056 (scissor-клиппинг, 2026-09-23): trunk release до/после (оба без wasm-opt) — wasm 10 849 931 → 10 853 951 байт (**+4 020 б ≈ 3,9 КБ ≤ 100 КБ**); canvas-ui внешних крейтов не имеет.
- [x] G8: ≥ 6 поверхностей на реестре+примитивах (grep-аудит отсутствия литеральной rect-математики в их модулях): поиск, настройки, docs, палитра шаблонов, what-if бар, галерея схем.
...
### 16.1 Ретроспектива PoC (U5)

**Что сработало:**
- **Реестр — единственный диспетчер** оправдал себя на всех этапах: добавление
  поверхности = декларация + hit-rect + arm ввода (доказано живыми интеграциями
  X2 PRD-0007 и choice_menu FR-050 поверх слитого U2 — 0 правок их кода).
- **Один источник геометрии** (layout-функции для отрисовки и hit-rect'ов)
  сделал налезания тестируемыми: сквозной линт U5 поймал 3 реальных дефекта
  shipped-сборки (завышение hit-rect'ов замыканием-конвертером; панель хоткеев
  × полоса палитры; модаль настроек × полоса/карточка того же слоя) — по
  критерию D1 «налезание из мистики → два rect'а с именами».
- **Измеренный текст** (TextMeasurer) устранил класс «эвристика ширины»
  (CR-015) на 6 поверхностях: переносы docs, ширины чипов палитры, подписи
  what-if/галереи считаются тем же шейпингом, что и рисуются.
- **Поэтапность (Q3-a)** удержала ветки зелёными при параллельных волнах
  (FR-050/PRD-0007/MCP v2 вливались без конфликтов каркаса).

**Честные уступки:**
- hit-rect форматы раскладок неоднородны (xyxy у search_ui) — контракт
  задокументирован, унификация отложена (риск правки потребителей выше пользы);
- линт канонических состояний не комбинирует фичи (what-if + док палитры и
  т.п.): их совместный порядок — нормализованные дельты U2, доброкачественные
  «popup над панелью» перекрытия допустимы семантикой egui-подобного pick;
- клавиатурный роутер вскрыл регресс U2 (док палитры в фокусе терял клавиши
  при видимой палитре выделения) — исправлен проходом по скоуп-стеку;
  эквивалентность легаси-head зафиксирована тестом.
- G6 (DebugOverlay) и замер wasm-прироста — за не заказанным этапом U4.

## 16. История изменений

- `2026-09-23` — агент: **FR-060 выполнен** (миграция волна 2 — последние hand-rolled поверхности): `autolink_ui` — окно через `kit::modal` (min-маржа «viewport−20» — мёртвый код, устранена с parity-тестом), кламп прокрутки `ScrollState`, строки групп — `kit::list_rows` (окно видимости кита ≡ прежней попарной проверке краёв; строка с 0 видимых px на кромке исключается — попутно убрано пересечение невидимой hit-зоны с футером), ✕ — `kit::stack` (30×30 дословно: `icon_button` 26 ≠ прежних 30 — числа сильнее перечня); `palette.rs` — бар и колонки через `kit::dropdown_menu` (якорь-строка 6 = `PAL_ANCHOR_GAP` 10 − зазор кита 4; якорь-зона колонки = полоса бара ±1: зазоры 3 = −1+4 — прежние числа дословно), строки колонок — `list_rows`; flip бара у нижнего края (прежний кламп перекрывал выделение) — системная политика кита; `explain_ui` — `kit::modal` со слотом-инсетом `WIN_MARGIN` (центр и клампы сохранены — parity-тест), крошки — `take_while` вместо break-клампа, обход предков — match-страж (G5); app.rs: **диалог подтверждения — измеренная геометрия** (`kit::modal` + `kit::button_size`: ширина/высота от замера текста — фикс класса дефектов «h=150», ellipsis вместо молчаливого клипа, кегли/пады дословно константами), пункты меню и хоткеи — `list_rows` + `WidgetState` (брейк-кламп хоткеев «ниже кромки −2» устранён — частичные строки клипует scissor FR-056), отрисовка `autolink_frame`/`palette_overlay` — Painter + WidgetState (панели с тенью — квады: params.w вне PaintItem); G5-аудит трёх модулей — 0 `take(n)`/`break`/`truncate_chars` (`Option::take` — перенос состояния, не срез); G4-линт-матрица: +3 состояния (`lint_autolink_review_open`/`lint_explain_open`/`lint_palette_selected` через `test_viewport`); **финальный замер wasm волны: +1 229 Б ≈ 1,2 КБ ≤ 100 КБ** (raw cdylib release, main vs ветка одной процедурой); гейты fmt/clippy/test (workspace 1732)/wasm/mcp-wasm --check зелёные; доки: CR-060 ✅, index, ui-kit §8, worklog.
- `2026-09-23` — агент: **FR-062 выполнен** (усиление кита — layout v2, этапы A/B/C по ADR-0013): F-13 `MeasuredItem`+`Row::lay_out_measured` (TextMeasurer внутри раскладки; кламп ширины — только явный `max_w`, молчаливого среза нет — G5), F-14 `Child.grow`/`Child::flexible`+`MainAlign::End` (SqueezeTail приоритетен; grow=0 — байт-в-байт), F-15 `RowPolicy::Wrap` (число видимых строк задаёт высота слота, перелив за нижний край не маскируется), F-16 `grid_cells`, F-17 `kit::focus_order`+`FocusRing::retain_order` (только добавление), F-18 геометрические golden-снапшоты (grid/stack, Switch/Card — без шрифтов); витрина: секции measured/flex/wrap/grid/focus + i18n RU/EN, Tab/Shift+Tab — `App::gallery_focus_step` (кольцо в контент-координатах, retain_order); контракты F-13/F-15 уточнены при реализации (Changelog FR-062); taffy не подключён (ADR-0013). Гейты: canvas-ui 144 (+24), canvas-app lib 322, fmt/clippy/wasm-check — зелёные.
- `2026-09-23` — агент: **ADR-0013** (по запросу владельца «почему не taffy, какие выигрыши против актуального плана, как усилить кит»): реконструкция решения V-3 + внешнее исследование taffy 0.14.0 (MIT, MSRV 1.71, Flexbox/Grid/Block, retained-дерево TaffyTree; потребители Servo/Bevy/Zed/Slint) + **замер wasm по методологии G7**: flexbox-only ≈ 108.5 КБ (= весь G7-остаток волны), flexbox+grid ≈ 376 КБ (gzip ≈ 103 КБ) — против +4 020 Б scissor (FR-056). Вердикт: актуальный план не менять; главные выигрыши taffy (auto-размер, flex-факторы, wrap, равная 2D-сетка) закрываются собственными примитивами — постановка **FR-062** (F-13 measured-дети, F-14 flex-факторы, F-15 Wrap, F-16 grid_cells, F-17 фокус-связка, F-18 геометрические снапшоты; ≤15 КБ кумулятив, 0 зависимостей); триггеры эскалации T1–T4 + протокол (§7.4 V-3 дополнен ссылкой).
- `2026-09-23` — агент: **FR-056 выполнен** (волна 2 кита, сливается первым из волны): scissor-клиппинг рендера F-5 — `ScreenBand.clip: UiRect` (из `SurfaceFrame.clip` кадра реестра), чистая конверсия `band_scissor_rect` (ceil лево/верх / floor право/низ + кламп к вьюпорту — scissor никогда не расширяет видимое; пустое пересечение — полоса не рисуется), исполнение — один `set_scissor_rect` на полосу-бакет (R-1) с возвратом к полному вьюпорту, тексты полосы — `TextBounds`-клип (scissor на текст не тратится); 8 новых тестов (конверсия ×5, TextBounds ×3), G3 z-тесты зелёные без правок, 0 новых зависимостей; клипы поверхностей сегодня = вьюпорт (сужение per-surface — миграции FR-059/060, их G5-аудит опирается на рендер-клип); **замер G7-остатка закрыт: прирост wasm-бина +4 020 б ≈ 3,9 КБ ≤ 100 КБ** (trunk release до/после, оба без wasm-opt); гейты fmt/clippy/test/wasm/mcp-wasm зелёные; доки: ui-kit.md §6 (scissor-политика), surface-registry.md §2 п.5, CR fr-056 (статус ✅).
- `2026-09-23` — агент: волна 2, **FR-057 выполнен** (kit-core): draw-слой и машина состояний виджета переехали в крейт — `canvas_ui::paint` (Painter/PaintItem — данные без wgpu, G7; конвертация в инстансы рендера — забота потребителя) и `canvas_ui::widget` (WidgetState: hover/pressed/selected/disabled/focused → KitState по матрице Disabled > Pressed > Hovered > Selected > Normal + ребро клика «press был внутри → release внутри»); `keyboard::FocusRing` — Tab-фокус контента скоупа (только добавление); `KitDraw` (kit_ui.rs) — тонкая обёртка над Painter (методы/поведение 1:1, тест эквивалентности quads/texts — 0 визуального скачка), `cursor_state`/`dropdown_item_state` — deprecated-делегаты; **0 правок app.rs/renderer** (дифф). Гейты: canvas-ui 83/83 (22 новых TDD-теста), canvas-app lib 318/318, fmt/clippy/wasm-check — зелёные. Доки: ui-kit.md §7.1; потребители — FR-058/059/060.
- `2026-09-23` — агент: волна 2, **FR-059 выполнен** (миграция волна 1 + витрина v2): `hints_ui` — popup через `kit::dropdown_menu` (якорь-строка каретки 16 + `DROPDOWN_GAP` 4 — прежний flip «−20» дословно) + `list_rows`/`ScrollState`; `flowmap_ui` — `kit::stack` + `list_rows`/`ScrollState`, кап «… ещё N» с break-клампом удалён (окно 12 строк — прежняя геометрия, все строки доступны скроллом, бегунок `scroll_bar`), hit-тесты — модельные индексы; `calc_panel_ui` — `kit::stack` + `list_rows`/`ScrollState` на обе колонки, срез «… ещё N» удалён (кап высоты — ограничение размера), ширины текстов — измеренные (замена «6.3·символ»); отрисовка трёх поверхностей — через Painter (конвертации `paint_items_to_band`/`paint_items_to_stage` — конвенции полос/стейджа сохранены), состояния строк — WidgetState; витрина `kit_gallery` — секции v2 (TextField ×3 с кареткой/placeholder, Switch ×4, Card, список+скролл с бегунком, Icon-глифы ×4) + прокрутка контента (шапка фиксирована, offset 0 — прежняя раскладка); `cursor_state`/`dropdown_item_state` — `#[deprecated]`; G5-аудит трёх модулей — 0 `take(`/`break`/`truncate_chars`; G4-линт-матрица зелёная (витрина покрыта); гейты: canvas-ui 120, canvas-app lib 322 (+9 интеграционных), fmt/clippy/wasm-check — зелёные.
- `2026-09-22` — агент: этап **U4 выполнен** (FR-055): kit v1 (F-8) в `canvas-ui::kit` — 8 компонентов контракта слотов (Panel/Button×варианты/IconButton/Dropdown «якорь+flip»/Chip/Toast/Tooltip «+delay»/Modal) + `anim` (BoolAnim, dt-детерминизм); DebugOverlay (F-10) — F9/`?ui=debug`, рамки слоёв/имя под курсором/подсветка пересечений — G6 закрыт; витрина `kit_gallery` («?» → «О интерфейсе», Q5-a) — компоненты × состояния × RU/EN × темы, кнопка темы — реальный kit-контрол; хром пилотов (контейнер/пилюля what-if, «✕» галереи) — через kit-стили (I-1); витрина в G4-линте (кит покрыт линтом); scissor/замер wasm — G7-остаток.
- `2026-09-22` — агент: этап **U5 выполнен** (FR-054): миграция 4 поверхностей (поиск, настройки, docs — измеренный текст вместо эвристик, палитра шаблонов — измеренные чипы/SqueezeTail), лестница on_key → KeyboardRouter (Q4-a, NUMI не тронуты), сквозной G4-линт в CI (F-11) — поймал и погасил 3 реальных налезания (hit-rect замыкание, хоткеи × полоса, настройки × полоса/карточка), `docs/ui-kit.md` + `docs/interface-objects/surface-registry.md`; онбординг не тронут (решение владельца); гейты fmt/clippy/test 1509/wasm/mcp-wasm зелёные; U4 остаётся вне PoC-этапов.
- `2026-09-22` — агент: этап **U5 начат** (FR-054) — приказ владельца «Продолжай U5» (+ решение: онбординг не дорабатывать, пользовательских изменений нет): миграция 4 поверхностей (поиск, настройки, help/docs, палитра шаблонов) на примитивы/TextMeasurer, лестница on_key → KeyboardRouter (Q4-a), layout-линты в CI (F-11), `docs/ui-kit.md`, interface-objects (US-5), ретро. U4 (кит F-8, DebugOverlay F-10, витрина, scissor) владельцем не заказан — G6 остаётся за U4.
