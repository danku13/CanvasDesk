...
- **Статус:** выполнено (PoC: E1–E4 реализованы, реализация — `FR-042` v1; стресс-замер G4 — ручная приёмка)
- **Тип:** PRD (документ требований продукта; реализация оформляется отдельным `FR-042` по шаблону `docs/change-requests/cr-template.md`; структура — единый шаблон `docs/prd/TEMPLATE.md`)
- **Приоритет:** важно
...
| **Input-коннект** | Один входящий слот приёмника от данного источника: PoC-семантика — 1 ребро = 1 коннект; вес пучка = число его рёбер, направленных в приёмник (расширение на `fromLine`/`toParam` — §11-Q1) |
...
- AC-2.3: остальной канвас затемняется (см. §7.5 про фон); клик по затемнённому фону закрывает main stage.
...
Персона CJM — **инженер моделирования** с плотной моделью (50+ нод, десятки value- и control-связей); канал — desktop. CJM зафиксирован для PoC-границы и пересматривается после первых юзабилити-прогонов (§13, E5).

### 6.1 Краткий CJM (шаги)

| # | Этап | Действие пользователя | Поверхность |
|---|---|---|---|
| 1 | Обзор модели | Открывает плотный канвас, ищет нужное соединение | канвас (LOD-0) |
| 2 | Наведение | Находит пучок по толщине и бейджу ×N | канвас (LOD-0) |
| 3 | Детализация | Клик по агрегированной линии | main stage |
| 4 | Разбор соединения | Читает рёбра пучка: kind, fromLine, toParam, значения | main stage |
| 5 | Возврат к работе | Esc или клик по затемнённому фону | main stage |
| 6 | Правка топологии | Меняет связи, наблюдает пересчёт веса и агрегацию | канвас |
...
|---|---|---|---|---|---|---|
| 1. Обзор | Пан/зум, визуальный поиск пары нод | «Что тут вообще соединено с чем?» | перегрузка (−2) | Пучки параллельных линий, перекрытые лейблы | D1: не находит нужную связь за разумное время → уходит в файл/экспорт → митигация: G1/F-2 (агрегация снижает шум) | Толщина линии как «тепловая карта» связности узлов |
| 2. Наведение | Hover на линиях пучка | «Вот толстая — на неё много входов» | интерес (0→+1) | Мелкая зона клика у тонких линий | D2: клики «в пустоту» без реакции → привыкает, что линии не кликабельны → митигация: F-5 (допуск от толщины) + hover-отклик | Подсказка «клик — детали соединения» при hover |
| 3. Детализация | Клик по агрегированной линии | «Покажи, что здесь происходит» | ожидание (+1) | Страх «куда делись мои линии» | D3: непонятно, что произошло, экран «сломался» → панический Esc → митигация: бейдж ×N, подпись режима, подсказка «Esc — закрыть» (F-6) | Плавная анимация перехода (кандидат v2) |
| 4. Разбор | Читает рёбра пучка, hover-подсветка | «Вот кто льётся в $qps, вот построчный исток» | ясность (+2) | В LOD-0 лейблы перекрывались — здесь решено | D4: детали всё равно нечитабельны → закрывает и читает `.canvas` текстом → митигация: AC-2.2 (детальный рендер: стороны, лейблы, fromLine/toParam) | Подсветка value-цепочки upstream (кандидат v2) |
| 5. Возврат | Esc / клик по фону | «Вернусь к общей картине» | облегчение (+1) | Страх потерять контекст и выделение | D5: непредсказуемый способ выхода → раздражение → митигация: F-8 (два выхода) + подсказка в панели | Сохранение выделенного ребра после выхода (AC-3.2) |
| 6. Правка | Shift+drag нового ребра в пучок; удаление рёбер | «Бейдж сам обновился — удобно» | контроль (+1) | — | D6: пучок «распадается» на отдельные линии после правки → шум вернулся → митигация: F-1 (инкрементальный пересчёт кэша) | Показ дельты веса при drag (v2) |

### 6.3 Трассировка точек отвала

Каждая точка отвала CJM обязана иметь митигацию в требованиях (§8) или рисках (§12): D1 → G1/F-2; D2 → F-5; D3 → F-6; D4 → AC-2.2; D5 → F-8; D6 → F-1. Конфликт жестов «клик по линии» — дополнительно в §12 (первая строка таблицы рисков).

---

## 7. Решение: уровни детализации
...
### 7.2 Композиция main stage (≤ 70% вьюпорта)
...
> **Примечание (реализация 2026-09-22/23, FR-044):** композиция stage уточнена —
> пилюли веера с подложками лейн-стопкой в коридоре между колонками подписей
> (Р-1), панель «Как считается» у приёмника с группами «Переменные»/«Расчёт»
> (Р-4) и подсветкой зависимостей формула⇄переменные⇄рёбра (Р-5), подписи
> портов-слотов на карточках (лейбл выхода + значение у истока, путь у
> приёмника, control — «управление»/«to: …») (Р-2/Р-3-а), эшелоны переполнения
> стопки пилюль Full→Compact→Scroll (Q2) и анимация перехода подсветки
> (Q3, токен `focus_fade_ms`). Детали и инварианты —
> `docs/change-requests/fr-044-mainstage-fan-readability-qualified-refs.md`.

### 7.3 Вычисление пучков и весов (кэш, а не per-frame)
...
Вес пучка (PoC): `weight = число рёбер пары, для которых эта нода — приёмник` (см. §4). Семантика «всё, что льётся в мои input-слоты» (включая `fromLine`-строки и `toParam`-спиллы из `inbound_values`, `flow.rs:579`) — расширение v2 (§11-Q1).

### 7.4 Рендер агрегированной линии
...
### 7.6 Интеграция с существующими механизмами
...
| F-13 | Настройка «агрегация связей: вкл/выкл» (`Settings`, по умолчанию вкл) | should | Переключатель в панели настроек; off — прежний рендер связей (без агрегации) |
| F-14 | Расширенный вес: учёт `fromLine`-строк и `toParam`-спиллов в весе | could | §11-Q1 |
...
- **Не делаем настоящий blur в PoC.** Затемнение — альфа-оверлей; blur-пасс — отдельное решение (§11-Q4).
...
- **Не меняем MCP-контракт.** `edges_list`/`edge_get` (FR-032) отдают рёбра как раньше; агрегированное представление — только визуальное (вопрос `bundles_list` — §11-Q5).
...
|---|---|---|
| E0 | FR-042 по шаблону `cr-template.md` (фиксация решений §11-Q1/Q2/Q3) | Разрешение на реализацию |
...
- `docs/change-requests/fr-042-edge-lod-main-stage.md` — реализационный FR (создан 2026-09-20, этап E0 закрыт; решения §11 зафиксированы в FR).
...
- `2026-09-20` — агент: перенос в каталог `docs/prd/` (переименован в `prd-0002-edge-lod-main-stage.md`); добавлен обязательный раздел CJM (§6: краткий + детальный + трассировка точек отвала); нумерация разделов приведена к единому шаблону `docs/prd/TEMPLATE.md`; приёмка F-13 заменена на проверяемую формулировку.
- `2026-09-21` — агент: **E1–E4 выполнены** (FR-042 v1; приоритет владельца — «LOD политика и связи»). Индекс пучков + геометрия stage (E1), LOD-0 агрегация + бейдж ×N + hit-test-допуск (E2), main stage с виртуальной камерой-композицией, модальность, затемнение, Esc/фон-выход, валидация среза (E3), минимикарта-дедуп + настройка F-13 + i18n + T23-OR-семантика (E4). Детали — changelog FR-042; приёмка — ACCEPTANCE §FR-042; остаток — стресс-замер G4 (ручная приёмка). E5 закрыт обновлением точек входа (edge.md, SPEC, user-docs, ACCEPTANCE).
- `2026-09-20` — агент: **E0 выполнен** — создан реализационный FR-042 (`docs/change-requests/fr-042-edge-lod-main-stage.md`, по запросу владельца «глубоко проанализировать концепцию main stage и прописать как FR»): глубокий анализ рендер-/вводного конвейеров (актуальные строки main после FR-038/039/040), решения открытых вопросов §11-Q1…Q6 (Q1 вес = число рёбер; Q2 два пучка для A→B/B→A; Q3 порог N ≥ 2; Q4 фон — альфа-аппроксимация; Q5 MCP вне скоупа; Q6 оверлеи взаимоисключительны), ключевое техническое решение — второй проход с виртуальной камерой + срез Canvas, 10 инвариантов тестируемости. Статус PRD остаётся «в анализе» до реализации E1–E5.
