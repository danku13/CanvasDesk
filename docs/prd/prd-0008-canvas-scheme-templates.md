...
- **Связанные документы:** FR-028 (онбординг-карусель, зарезервированное действие v2), FR-018/FR-019/FR-020 (шаблоны одиночных нод), FR-029 (value-порты `fromOutput`/`toParam`), FR-025 (`fromLine`), FR-017 (what-if), FR-031 (меню «?»), FR-006 (undo), FR-016 (узкие места), PRD-0002/FR-042 (main stage), PRD-0007 (цепочка расчёта цифры), PRD-0003 (всеядный импорт графов), PRD-0006 (design-токены), `docs/SPEC.md` §5.1 (расширения формата), §6.3 (бюджеты)
- **Создан:** 2026-09-21
- **Обновлён:** 2026-09-22
...
| 2. Выбор | открывает галерею | «О, тут есть готовое» | любопытство (+1) | боязнь сложного UI | D2: галерея перегружена/непонятны категории → закрывает | ≤ 4 категорий (v2, все чипы видны без среза — тест `chips_all_categories_fit`), карточки с одним предложением смысла |
...
Каждая точка отвала CJM имеет митигацию в требованиях или рисках: D1 → F-6 (empty-state) + G1; D2 → F-4 (галерея с категориями) + §7.2 (4 категории v2 — чипы без среза, инвариант-тест; срез прятал категорию «Бизнес»); D3 → F-5 (описания схем на языке задачи, i18n F-7; жаргон движка — «проливание», «M/M/1», «именованные выходы» — вычищен из описаний галереи аудитом 2026-09-22); D4 → F-3 (один undo-шаг, ремап id) + G3; D5 → F-5 (подсказки внутри схем набора — обязательное требование к контенту) + G2 (oracle-тесты гарантируют живость; хинт учит фактическому механизму — правка числа двойным кликом); D6 → F-5 (схема what-if демо) + US-2 AC-2.2 (хинт объясняет порядок: сначала режим кнопкой внизу, затем двойной клик по строке); D7 → §10 (non-goal «смена документа») + US-3 AC-3.2.
...
| Галерея | `canvas-app`: `scheme_gallery_ui.rs` по образцу `TemplatePanel` (`template_ui.rs:103`) — категории-чипы (4 в v2, ширина чипа 108 — все видны без среза, тест `chips_all_categories_fit`), поиск, карточки (имя, описание 1-2 строки, категория, счётчик нод/рёбер, кнопка «Открыть»); цвета — слоты `ThemeColors` v2, без новых констант (правило PRD-0006) |
...
**Каталог (10 схем, 4 категории)** — контент v2 (переделка по запросу
владельца 2026-09-22): каждая схема несёт краткие описания в КАЖДОЙ ноде
(первая строка — имя-объект для адресов «Объект.Поле»), пучок ≥2 рёбер
одной упорядоченной пары (клик → main stage, FR-042), ноды с множественными
связями (fan-in/fan-out ≥3), адресацию значений (`fromOutput`/`fromLine`/
`toParam` — «проливание»), цепочку глубиной 3-4 хопа с ветвлением (готовность
к дереву происхождения PRD-0007) и подсказку-приглашение D5. Инварианты
контента фиксированы автотестами `scheme_apply` (описания, пучки, глубина,
отсутствие красных строк, трассируемость фич). Расширение 6 → 10 (аудит
схем 2026-09-25): по направлениям шаблонного каталога — backend/network →
контакт-центр (erlang_c/mmc/littles_law), финансы UE → инвесткейс
(npv/irr/cagr), PA → когорта (cohort_ltv), planning → рунвей (sum).

| id | Категория | Суть | Демонстрирует |
|---|---|---|---|
| `intro-calculations` | Онбординг | «Живые расчёты»: спрос × сессии → нагрузка → доля лимита; 6 нод, 4 ребра | Numi-строки, value-рёбра, именованные выходы, пучок ×2 → main stage, пересчёт при правке |
| `intro-whatif` | Онбординг | «Сценарии»: статьи бюджета → итог → годовой бюджет через параметры; 6 нод | FR-017: подмена строки, дельта-бейджи, toParam-проливание в `$spend`/`$months` |
| `capacity-service` | Архитектура | «Ёмкость сервиса»: спрос → rps → утилизация/латентность M/M/1, пик ×3 → перегрузка; 10 нод | mm1/utilization, Percent → узкие места FR-016 (Warn + Overload), пучок ×2, проливание, цепочка 3 хопа |
| `project-budget` | Планирование | «Бюджет проекта»: статьи в именованных группах → подытоги → резерв 15% → итог → поток по стадиям; 14 нод | группы с заголовками, fan-in ×4, fromLine по строкам графика, два пучка, toParam-резерв |
| `unit-economics` | Бизнес | «Юнит-экономика»: цена/COGS → маржа → LTV → CAC:LTV, окупаемость; 9 нод | производные метрики, money/money, пучок ×2, окупаемость через `$cac`/`$margin`, цепочка 3 хопа |
| `renovation-estimate` | Планирование | «Смета ремонта»: площади × ставка (фан-аут) + материалы − скидка; 11 нод | единицы ($ за метр), фан-аут ставки ×2, пучок ×2, итог со скидкой через параметры |
| `support-staffing` | Архитектура | «Штат поддержки»: обращения/час → поток и производительность → загрузка/Эрланг C/M/M/c/Литтл; 11 нод | erlang_c, mmc, littles_law, utilization(c), скалярные $-параметры в аргументах функций, пучок ×2, hub с 3 функциями от одних входов |
| `investment-case` | Бизнес | «Инвесткейс»: потоки 4 лет → PV → NPV → индекс прибыльности; IRR и CAGR рядом; 10 нод | npv (нулевой год), irr (поток со знаком минус), cagr на деньгах, пучок ×4, relay-строка capex, цепочка 3 хопа |
| `cohort-launch` | Бизнес | «Когорта запуска»: удержание d1/d7/d30 → LTV → ценность волны → здоровье и эффект; 10 нод | cohort_ltv (6 аргументов, retention-кривая), пучок ×4, Money/scalar арифметика, toParam-проливание в health-пару |
| `runway` | Планирование | «Рунвей»: статьи → sum → чистый расход → годовая дыра и рунвей; 8 нод | sum ×4, Money/скаляр деление, двойное проливание ($burn/$revenue, $balance/$net), цепочка 3 хопа |

Наблюдение по категориям: фактически категорий четыре (онбординг,
архитектура, планирование, бизнес) — G2 «≥ 3» выполнен с запасом. Аудит
CJM 2026-09-22: при ширине чипа 120 пятый чип молча срезался (категория
«Бизнес» и её схема были недостижимы через фильтр) — ширина чипа
снижена до 108 (все «Все» + 4 категории в ряду, инвариант-тест
`chips_all_categories_fit` на 1280/1024/800).

### 7.2.1 Требования к контенту схем (v2, «шаблон = витрина фич»)

Схема — витрина функциональности, а не только расчёт: пользователь должен
происследовать на её базе не менее 3-4 особенностей продукта. Обязательные
элементы каждой схемы (инварианты автотестов):

1. **Описания нод**: первая строка текста — короткое имя-объект (заголовок
   карточки, основа квалифицированных адресов «Объект.Поле» в main stage),
   вторая строка — пояснение, что за значение и в каких единицах.
2. **Множественные связи**: ≥1 нода с ≥3 инцидентными value-рёбрами
   (fan-in/fan-out) и ≥1 пучок из ≥2 рёбер одной упорядоченной пары A→B
   с адресацией истока — клик по пучку открывает main stage с веером
   значений на строках.
3. **Адресация и проливание**: именованные выходы (`fromOutput` —
   присваивания `имя = значение`), построчные истоки (`fromLine`),
   параметры приёмника (`toParam` → `$имя`) — кроме вводной
   `intro-calculations`, которая учит позиционным слотам `$1..$N` по
   одному механизму за раз.
4. **Глубина под объяснение**: цепочка значений ≥2 хопов у онбординг-схем,
   ≥3 у остальных — будущее дерево происхождения цифр (PRD-0007) получит
   ветвление, а не одну пару.
5. **Живость без красного**: ни одна строка/итог вставленной схемы не
   даёт ошибок; формулы с `$параметр` пишутся авто-строкой (значение — в
   полосе результата D), присваивания с `$параметр` запрещены (красный
   бейдж построчного пересчёта без проливания).
6. **Приглашение D5**: нода-проза `hint` в каждой схеме содержит
   приглашение «поменяйте число — пересчитается» и указание на пучок
   main stage. Аудит CJM 2026-09-22: приглашения обязаны описывать
   ФАКТИЧЕСКИЕ механизмы продукта — мёртвые ссылки на нереализованный
   UI (например «цепочка расчёта покажет происхождение» до X2 PRD-0007)
   дают пользователю ощущение «сломанной» схемы; вердикты приглашают к
   живым фичам: правка чисел, сценарии what-if (порядок: режим кнопкой
   внизу → двойной клик по строке), пучок-веер main stage.
7. **Цветовая семантика**: пресет «1» (красный) в заливках нод запрещён
   — красный занят рамкой перегрузки FR-016 (тревога); входы — «5»
   (циан), расчёты — «2» (оранж), итоги — «6» (фио), hint/вердикты —
   «4» (зелёный), группы — «3» (жёлтый). Инвариант — тест
   `schemes_avoid_red_node_fills`.
8. **Язык задачи в описаниях галереи** (D3): жаргон движка («проливание»,
   «M/M/1», «именованные выходы») не попадает в `description_ru/en`;
   технические термины допустимы внутри схем (глубина изучения), но
   поясняются словами («модель M/M/1», «приходит по связи в параметр»).
...
| F-5 | Стартовый набор: 6 схем (§7.2) с oracle-тестами значений и подсказками-приглашениями внутри; v2 (§7.2.1): описания в каждой ноде (заголовок-объект + пояснение), ≥1 пучок ≥2 рёбер одной пары (main stage), ≥1 нода с ≥3 value-связями, адресация `fromOutput`/`fromLine`/`toParam`, глубина цепочки 2–4 хопа, живость без красного | must | G2; тест перебора схем + инварианты контента `scheme_apply` |
...
| Q5 | MCP-инструменты `schemes_list`/`schemes_apply` — включать в PoC или отложить в v2? | (a) v2 (предложение); (b) сразу — агентная петля «покажи демо» | скоуп FR, реестр API | владелец — **закрыто: v2 реализовано 2026-09-22 (запрос «подтянуть MCP»), + lineage FR-048** |
...
- `2026-09-22` — агент (сессия владельца): переделка контента по запросу владельца («описания в ноды, множественные связи, трассируемость 3–4 фич на шаблон»): §7.2 переписана (состав v2, 4 категории), добавлена §7.2.1 «шаблон = витрина фич» (6 инвариантов контента, автотесты); формат пакета расширен — `label` групп (JSON Canvas), адресация рёбер `fromLine`/`fromOutput`/`toParam` (FR-025/FR-029, декларировалась в §7.1 ранее); фикс ядра `inbound_slots_with_lines` (именованные выходы в построчных бейджах, регресс-тест); F-5 актуализирован; FR-049, SPEC §5.3, scheme-gallery.md, ACCEPTANCE.md, worklog синхронизированы.
- `2026-09-22` — агент (аудит CJM по запросу владельца «найди то, что проходит формальный чек, но ломает CJM»): найдены и закрыты 5 классов: (1) мёртвые приглашения — 4 вердикта звали «цепочка расчёта покажет...», а UI цепочки (PRD-0007 X2) не реализован (0 ссылок lineage в canvas-app) → вердикты переписаны под живые фичи; (2) хинт what-if учил несуществующему «выделите строку» и перепутанным порядком (двойной клик ВНЕ режима = правка базы — нарушал обещание «файл не меняется») → порядок и механизм исправлены; (3) 4-й чип категории («Бизнес») молча срезался (120×5 = 560 > inner 536) → ширина чипа 108 + тест `chips_all_categories_fit`; (4) красные заливки входов конфликтовали с красной рамкой перегрузки FR-016 → входы переведены на циан («5»), пресет «1» запрещён в схемах тестом `schemes_avoid_red_node_fills` (§7.2.1.7); (5) жаргон D3 и фактические неточности в текстах («проливание»/«M/M/1»/«именованные выходы» в описаниях галереи, «две трети» при 0.625, сломанная фраза «проливается в доллары на месяц») → вычищены. Добавлены регресс-тесты путей пользователя (canvas-app `scheme_cjm_tests`: геометрия после авто-роста с реальным шейпингом — наложения/вместимость групп; D6-сценарий с подменой строки; D5-правка входа с перегрузкой). §6.3 (D2/D3/D5/D6 митигации), §7.2, §7.2.1, §16 актуализированы.
- `2026-09-22` — агент (запрос владельца «подтянуть функционал MCP под обновления и полностью проверить работу MCP»): Q5 закрыт — инструменты `schemes_list`/`schemes_apply` (галерея агенту, SPEC §13) + `lineage` (FR-048 X2, паритет с окном проверки); инвариант «MCP-видимость = UI» для flow_recalc/analyze_bottlenecks/lineage/flow-полей (активное what-if состояние + авто-строки FR-050 Р-4, свежий пересчёт); 36 → 39 инструментов, e2e-сессия расширена; FR-049.12, SPEC §13, worklog — синхронизированы.
