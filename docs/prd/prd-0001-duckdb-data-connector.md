...
- **Статус:** в анализе
- **Тип:** PRD (документ требований продукта; реализация оформляется отдельным `FR-041` по шаблону `docs/change-requests/cr-template.md`; структура — единый шаблон `docs/prd/TEMPLATE.md`)
- **Приоритет:** важно
...
**Персона 3 — ИИ-агент (MCP, CR-013).** Агент собирает модели автоматически (`canvas-scene/src/mcp.rs`); программная поверхность коннектора позволит ему в будущем самому подтягивать вводные (не входит в PoC — см. §10, фиксируется как точка роста).
...
**Acceptance criteria:**
- AC-5.1: нода-источник показывает: тип источника (файл/БД), путь/строку подключения (без пароля — см. §9.4), время последнего расчёта агрегатов.
...
Персона CJM — **аналитик моделирования** (§4, Персона 1); канал — desktop. CJM зафиксирован для PoC-границы и пересматривается после первых юзабилити-прогонов (§13, D4).

### 6.1 Краткий CJM (шаги)

| # | Этап | Действие пользователя | Поверхность |
|---|---|---|---|
| 1 | Осознание потребности | Фиксирует, что модель на ручных вводных не отвечает на вопрос бизнеса | вне продукта |
| 2 | Подключение источника | Перетаскивает файл данных или задаёт строку подключения к БД | drag-drop → нода-источник |
| 3 | Ориентация в схеме | Открывает инспектор, выбирает таблицу и колонку | панель-инспектор |
| 4 | Получение агрегатов | Читает count/min/max/avg/p50/p90/p95, сопоставляет с гипотезой | панель-инспектор |
| 5 | Перенос в модель | «В канвас» → снапшот-нода → value-ребро в параметр шаблона | канвас |
| 6 | Проверка и ревизия | Запускает модель; при необходимости «пересчитать» | канвас / панель-инспектор |
...
|---|---|---|---|---|---|---|
| 1. Осознание | Формулирует вопрос модели, понимает, что вводные выдуманы | «Числа с потолка — ответ неубедителен» | тревога (−1) | Не знает, какие данные вообще доступны и в каком виде | — (этап вне продукта) | Подсказка онбординга: «вводные можно взять из файла/БД» |
| 2. Подключение | Drag-drop файла; ввод DSN | «Кину файл на канвас — пусть само» | ожидание (0) | Непонятен формат строки подключения; некуда ввести пароль | D1: DSN без очевидного поля секрета → бросает затею; D2: формат не поддержан (например, .xlsx) → уходит с файлом | Валидация источника при подключении с внятной ошибкой; явный список поддержанных форматов |
| 3. Ориентация | Листает таблицы и колонки инспектора | «Какая из сорока таблиц моя?» | растерянность (−1) при большом списке | Длинные имена, нет поиска по схеме | D3: панель не открылась или «зависла» без индикации → закрывает продукт | Поиск/фильтр по таблицам и колонкам (кандидат v2) |
| 4. Агрегаты | Выбирает колонку, читает агрегаты | «p95 = 480 мс, а я закладывал 200» | инсайт (+1) | Неясна свежесть данных и что именно посчитано | D4: запрос дольше 5 с без индикатора → переключается на другое дело | Сравнение агрегатов соседних колонок (кандидат v2) |
| 5. Перенос | Кнопка «в канвас», проводка value-ребра | «Пусть проводка будет за меня» | удовлетворение (+2) при ≤ 3 действиях | Ручная проводка рёбер и правка формул | D5: снапшот без ссылки на источник → недоверие к числу | Авто-проливание в выделенную ноду (§11-Q5) |
| 6. Проверка | Запускает модель, крутит сценарии «а если» | «А если p95 вырастет вдвое?» | уверенность (+2) | Протухшие агрегаты дают ложные выводы | D6: модель «врёт» из-за устаревших данных → теряет доверие к продукту | Связка со сценариями what-if (FR-017) |

### 6.3 Трассировка точек отвала

Каждая точка отвала CJM обязана иметь митигацию в требованиях (§8) или рисках (§12) — иначе она декоративная: D1 → §11-Q4 (секрет подключения); D2 → F-1–F-3 (явный список форматов в диалоге подключения); D3 → F-10 (индикация/отмена); D4 → G3 + F-10; D5 → AC-5.1 (источник + время на ноде-источнике); D6 → F-7 (пересчёт, метка свежести).
...
### 7.2 Точка интеграции: провайдер по образцу `providers.rs`
...
### 7.3 Почему не другие варианты интеграции
...
4. **Безопасность.** Пароли/токены в конфиге подключения не сериализуются в `.canvas` (только DSN без секрета, секрет — в per-session вводе или системном хранилище — открытый вопрос §11-Q4). Read-only: DuckDB открывается в режиме без записи; продукт ничего не пишет в источник.
...
- **Q2.** Форма ноды-источника: отдельный тип ноды (`canvasdesk.datasource` на `NodeKind::Unknown`-совместимом типе) или расширение существующей файловой ноды? Решение — при оформлении FR-041 (влияет на `model.rs` и `docs/interface-objects/node.md`).
...
|---|---|---|
| Сборка `libduckdb-sys` (C++, ~мегабайты бинаря, cmake) удлиняет CI и рвёт матрицу M7 | Сборка/дистрибуция | Изоляция в крейте `canvas-data` за feature; гейт сборки в CI отдельной джобой; план B — внешний процесс `duckdb` CLI (§7.3) |
...
| Пользователь подключает источник с PII/секретами | Безопасность | Read-only, секреты не сериализуются (§9.4), DSN маскируется в UI |
...
|---|---|---|
| D0 | ADR-0013 (зависимость DuckDB, граница ADR-0008) + FR-041 по шаблону `cr-template.md` | Разрешение на реализацию |
...
- `docs/change-requests/fr-041-duckdb-data-connector.md` — реализационный FR (создаётся на D0).
...
- `2026-09-20` — агент: перенос в каталог `docs/prd/` (переименован в `prd-0001-duckdb-data-connector.md`); добавлен обязательный раздел CJM (§6: краткий + детальный + трассировка точек отвала); нумерация разделов приведена к единому шаблону `docs/prd/TEMPLATE.md`.
