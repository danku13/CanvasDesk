...
| Режим защиты | модальное окно поверх канваса (§6.4): панель прячется на время защиты (паттерн диалога ревью), снапшот сохраняется; внутри — тот же граф нод с ветками, укрупнённый (решение владельца, 2026-09-22 — У9 закрыт, прототип v3 `docs/prototypes/ux-defense-mode.html`); другие оверлеи недоступны |
...
- `docs/prototypes/ux-review-dialog.html` (У7), `docs/prototypes/ux-defense-mode.html` (У9 — закрыт, v3: модальное окно с деревом-графом) — интерактивные HTML-прототипы UX-решений Q8; решение по У7 — после просмотра.
