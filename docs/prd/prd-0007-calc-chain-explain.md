...
| 2. Ревью автосвязи (аналитик) | Диалог по бейджу: предложения с процентом совпадения; приём пачкой или поштучно | «Принял пачку — и знаю, как откатить одним шагом» | контроль (+1) | Усталость от длинного списка (12+ предложений) | D10: список перегружен → массовое «Принять все» по ошибке → митигация: откат пачки с подтверждением и подсветкой отменяемого (AC-5.3); группировка/сортировка (У7, Q8 — переформулировано); возврат отклонённых до закрытия (У8, Q8 — обоснование дано) | Приоритизация предложений по уверенности (v2) |
...
| Режим защиты | под-состояние панели (§6.4): панель остаётся видимой, дерево укрупняется (У9, Q8 — пояснено: из D11/шага 6 CJM); другие оверлеи недоступны |

Синхронизация канвас↔дерево: дерево→канвас — подсветка узлов/рёбер (AC-3.1); канвас→дерево — клик по подсвеченной ноде выделяет и подводит соответствующий узел дерева (У2, Q8 — переформулировано, ожидает решения).
...
- **Q8. Раунд 3 — UX/CJM (2026-09-21, «детально спроектировать CJM»).** Детализация §6 выполнена (акты/14 шагов/D8–D13/§6.4–6.7). Ответы владельца (раунд 3, вторая партия): **У1 — да** — фолбэк-триггер: клик по цифре открывает панель наряду с hover-«?» (AC-1.1, §6.4, §6.6, D8); **У3 — принято в формулировке владельца «если открыта панель — чип нужен»**: чип возникает при любом изменении модели при открытой панели, без исключений — включая подмену листа и Apply из панели/бара; канвас при подмене живёт сразу, оверлей обновляется кликом по чипу (AC-3.3/AC-4.2 скорректированы); **У4 — да** — чип: закреплённая полоса сверху панели; **У6 — да** — повторный «?» на другую цифру: перестройка на новый корень; **У10 — да** — main stage закрывает панель, возврат через «?» — мгновенно из сессионного кэша. Переформулированы с пояснениями и повторно вынесены владельцу (Q8 открыт): **У2** — обратная синхронизация канвас→дерево; **У5** — затемнение атомарно с готовым деревом (в Loading канвас не затемняется); **У7** — ревью-диалог: группировка предложений по паре нод + сортировка по имени, счётчик в бейдже; **У8** — «Отклонить все» обратимо до закрытия диалога; **У9** — режим защиты: панель остаётся видимой (дерево укрупняется) — см. D11/шаг 6 CJM.
...
- Раунд 3 (сессия 2026-09-21, чат, две партии) — запрос владельца «детально спроектировать CJM»: детализация §6 (три акта, 14 шагов, D8–D13, машина состояний §6.4, модальности §6.5, краевые траектории §6.6, хронометраж §6.7); вопросы У1–У10 — Q8. Ответы владельца: У1/У3/У4/У6/У10 — приняты (фолбэк-клик по цифре AC-1.1; чип при любом изменении при открытой панели без исключений AC-3.3/AC-4.2; чип-полоса сверху; перестройка на новый корень; кэш при main stage); У2/У5/У7/У8/У9 — переформулированы с пояснениями, повторно вынесены.
