...
**Acceptance criteria:**
- AC-6.1: тумблер «Режим защиты» доступен из открытой explain-панели; запуск — одним действием (кнопка в панели или хоткей, без захода в настройки — решение владельца, раунд да/нет 2026-09-21) и открывает модальное окно защиты поверх канваса (решение владельца, 2026-09-22 — У9 закрыт); хоткей — решение §11-Q4 (синк `HOTKEYS` + `user-docs/hotkeys.md`).
- AC-6.2: типографика дерева в модальном окне защиты увеличивается (шаг ×1.5 от базовой, значения из токенов PRD-0006); служебные строки (проза, `NumiLineKind::Prose`) в дереве скрываются; представление дерева не меняется — тот же граф нод с ветками, что и в explain-панели (решение владельца по итерациям прототипа У9, 2026-09-22), укрупнение применяется ко всему графу (узлы и ветки).
...
- AC-6.4: выход — Esc, ✕ или клик по затемнённому фону; после выхода обычный вид модели восстановлен полностью (подсветка, зум, панель возвращается).
...
| 2. Ревью автосвязи (аналитик) | Диалог по бейджу: предложения с процентом совпадения; приём пачкой или поштучно | «Принял пачку — и знаю, как откатить одним шагом» | контроль (+1) | Усталость от длинного списка (12+ предложений) | D10: список перегружен → массовое «Принять все» по ошибке → митигация: откат пачки с подтверждением и подсветкой отменяемого (AC-5.3); группировка/сортировка (У7 — под вопросом, решение по HTML-прототипу `docs/prototypes/ux-review-dialog.html`); возврат отклонённых до закрытия (У8 — да, AC-5.2) | Приоритизация предложений по уверенности (v2) |
...
Состояния: **Closed** — панели нет; **Hover** — «?» виден только под курсором (решение владельца); клик по самой цифре — альтернативный триггер (фолбэк У1 — да); **Loading** — панель открыта сразу с «честным лоадером»: ротация 10–15 подписей этапов построения (AC-1.2, У5 — да); **Ready (fresh)** — панель и подсветка рендерят кэшированный снапшот; **Stale** — модель изменилась, снапшот помечен чипом, данные не перерисовываются сами (AC-3.3/F-5); **Defense** — режим защиты поверх Ready (AC-6.1–6.4): модальное окно с тем же деревом-графом нод с ветками, укрупнённым и с пошаговым раскрытием уровней; панель на время защиты прячется (снапшот сохраняется) — решение владельца, 2026-09-22, У9 закрыт.

Правила переходов (детализация UX; пометки: «да» — решение владельца, Q8 — вопрос ещё открыт):
...
- модальное окно защиты открывается поверх Ready одним действием; панель на время защиты прячется (паттерн диалога ревью), снапшот сохраняется; выход — Esc/✕/клик по фону, панель возвращается (решение владельца, 2026-09-22 — У9 закрыт).
...
| Режим защиты | модальное окно поверх канваса (§6.4): панель прячется на время защиты (паттерн диалога ревью), снапшот сохраняется; внутри — тот же граф нод с ветками, укрупнённый (решение владельца, 2026-09-22 — У9 закрыт, прототип v3.1 `docs/prototypes/ux-defense-mode.html`); другие оверлеи недоступны |
...
| Режим защиты | Модальное окно в `canvas-app` (не сериализуется): крупная типографика из токенов PRD-0006, пошаговое раскрытие уровней, скрытие `NumiLineKind::Prose`-строк дерева, представление дерева не меняется (тот же граф нод с ветками), панель прячется на время защиты; хоткей — с синком `HOTKEYS`/`user-docs/hotkeys.md` |
...
| F-8 | Режим защиты: запуск одним действием — модальное окно поверх канваса (кнопка в панели или хоткей — решение владельца), типографика ×1.5 из токенов, пошаговое раскрытие уровней, скрытие прозаических строк, представление дерева не меняется (тот же граф нод с ветками), выход Esc/✕/клик по фону с полным восстановлением вида | should | AC-6.1–AC-6.4 |
...
| Режим защиты скрывает нужное (комитет не видит контекст) | UX | Модальное окно с панелью за ним — выход возвращает панель и подсветку без следов (AC-6.4); скрытие только прозаических строк (AC-6.2); демо-точка X2 |
...
| X4 | Автосвязь F-7: детектор + диалог ревью (группировка/сортировка — по решению У7 после просмотра прототипа) + батч-создание + настройка | US-5, G4 |
| X5 | Режим защиты F-8 (модальное окно — решение владельца, У9 закрыт 2026-09-22; представление — дерево-граф нод с ветками) | US-6 |
...
- `docs/prototypes/ux-review-dialog.html` (У7), `docs/prototypes/ux-defense-mode.html` (У9 — закрыт, v3.1: модальное окно с деревом-графом; фикс запуска окна — в v3 окно фактически не открывалось, проверено в headless-браузере) — интерактивные HTML-прототипы UX-решений Q8; решение по У7 — после просмотра.
...
- [ ] Режим защиты: вход одним действием — модальное окно поверх канваса с тем же деревом-графом нод с ветками (укрупнённое), пошаговое раскрытие уровней (клик/пробел), выход Esc/✕/клик по фону без следов, панель возвращается (AC-6.1–AC-6.4, У9 закрыт).
