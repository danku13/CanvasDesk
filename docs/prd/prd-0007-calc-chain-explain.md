...
- **Q7. Раунд да/нет (2026-09-21) — пункты, ожидающие подтверждения владельца:** (а) кнопки «принять все / отклонить все» в диалоге ревью автосвязи; (б) перепроверка предложений автосвязи после переименования строк; (в) индикатор покрытия цепочек прямо на канвасе; (г) инвариант F-5 «панель и подсветка из одного снапшота»; (д) запуск детектора автосвязи только явной командой (AC-5.5); (е) создание подтверждённых связей одним undo-батом (AC-5.3); (ж) сериализация UI-состояния панели (раскрытые узлы) в бандл — предложение «нет», состояние производное; (з) запрет lineage-циклов через `creates_value_cycle` (AC-5.4); (и) режим защиты без изменений модели — только слой рендера (F-8).
