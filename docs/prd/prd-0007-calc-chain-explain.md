...
- **Статус:** в работе (X0–X4 выполнены 2026-09-22 — FR-048, ядро `lineage.rs`, окно проверки/триггер/подсветка/лоадер/кэш; what-if из дерева — подмена листа/дельты/сценарии; автосвязь F-7 — детектор/бейдж/ревью/undo-бат/тумблер; демо-гейт Q6; далее X5 — режим защиты)
