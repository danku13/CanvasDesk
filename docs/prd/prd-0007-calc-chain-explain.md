...
- AC-6.2: типографика дерева и подсвеченных цифр канваса увеличивается (шаг ×1.5 от базовой, значения из токенов PRD-0006); служебные строки (проза, `NumiLineKind::Prose`) в дереве скрываются; представление дерева при входе в защиту не меняется — тот же граф нод с ветками, что и в обычном режиме (решение владельца по фидбэку к прототипу У9, 2026-09-22), укрупнение применяется ко всему графу (узлы и ветки).
...
Состояния: **Closed** — панели нет; **Hover** — «?» виден только под курсором (решение владельца); клик по самой цифре — альтернативный триггер (фолбэк У1 — да); **Loading** — панель открыта сразу с «честным лоадером»: ротация 10–15 подписей этапов построения (AC-1.2, У5 — да); **Ready (fresh)** — панель и подсветка рендерят кэшированный снапшот; **Stale** — модель изменилась, снапшот помечен чипом, данные не перерисовываются сами (AC-3.3/F-5); **Defense** — режим защиты поверх Ready (AC-6.1–6.4): то же дерево-граф нод с ветками, укрупнённое и с пошаговым раскрытием уровней (решение владельца по фидбэку к прототипу v2, 2026-09-22); видимость панели (У9) — решение по HTML-прототипу.
...
| Режим защиты | под-состояние панели (§6.4): дерево — тот же граф нод с ветками, укрупнённый (решение владельца, 2026-09-22); видимость панели (У9) — под вопросом, решение по прототипу v2 `docs/prototypes/ux-defense-mode.html`; другие оверлеи недоступны |
...
| Режим защиты | Состояние в `canvas-app` (не сериализуется): крупная типографика из токенов PRD-0006, пошаговое раскрытие уровней, скрытие `NumiLineKind::Prose`-строк дерева, представление дерева не меняется (тот же граф нод с ветками); хоткей — с синком `HOTKEYS`/`user-docs/hotkeys.md` |
...
| F-8 | Режим защиты: запуск одним действием (кнопка в панели или хоткей — решение владельца), типографика ×1.5 из токенов, пошаговое раскрытие уровней, скрытие прозаических строк, представление дерева не меняется (тот же граф нод с ветками), выход Esc с полным восстановлением вида | should | AC-6.1–AC-6.4 |
...
| X5 | Режим защиты F-8 (видимость панели — по решению У9 после просмотра прототипа v2; представление — дерево-граф нод с ветками, решение владельца 2026-09-22) | US-6 |
...
- `docs/prototypes/ux-review-dialog.html` (У7/У8), `docs/prototypes/ux-defense-mode.html` (У9, v2 — дерево-граф нод с ветками) — интерактивные HTML-прототипы UX-решений Q8; решения владельца — после просмотра.
...
- [ ] Режим защиты: вход одним действием, то же дерево-граф нод с ветками укрупнённое, пошаговое раскрытие уровней (клик/пробел), выход Esc без следов (AC-6.1–AC-6.4).
...
- Раунд 3 (сессия 2026-09-21/22, чат, три партии) — запрос владельца «детально спроектировать CJM»: детализация §6 (три акта, 14 шагов, D8–D13, машина состояний §6.4, модальности §6.5, краевые траектории §6.6, хронометраж §6.7); вопросы У1–У10 — Q8. Итог: У1/У2/У3/У4/У6/У8/У10 — да; У5 — да с «честным лоадером» (AC-1.2); У7/У9 — решение по HTML-прототипам `docs/prototypes/ux-review-dialog.html` и `ux-defense-mode.html`; итерация 2026-09-22 — фидбэк владельца по У9 («дерево нод с ветками») → прототип v2 с единым дерево-графом.
