...
**Acceptance criteria:**
- AC-1.1: триггер «?» доступен у полосы результата D (итог ноды) и у каждого построчного результата Numi-листа; появляется только по наведению курсора на цифру (hover-only — решение владельца, раунд да/нет 2026-09-21), постоянно видимым не показывается; клик открывает explain-панель.
- AC-1.2: панель открывает дерево, корень которого — выбранная цифра с текущим значением; построение асинхронное (решение владельца — максимальная производительность): панель открывается сразу с индикатором готовности, UI не блокируется; дерево готово ≤ 3 с на модели 200 нод (G1).
...
- AC-3.3: дерево и подсветка строятся из одного снапшота модели — рассинхрон панель↔подсветка невозможен (инвариант F-5). Кэширование оверлея (решение владельца, финал раунда 2): данные оверлея кэшируются — панель и подсветка рендерятся из кэшированного снапшота и не перестраиваются сами; при любом изменении модели при открытой панели — правки на канвасе (панель — оверлей, канвас доступен), правки MCP-агентом (FR-005/FR-032/FR-033), внешняя перезапись файла (watcher), Apply сценария — появляется оповещение-чип «Данные изменены — требуется обновление оверлея»; клик по чипу перестраивает оверлей из нового снапшота; если корневая цифра перестала существовать, обновление закрывает панель с сообщением, повторное открытие — через «?».
...
- AC-5.2: результаты — в диалоге ревью: список предложений «исток → приёмник (строка/слот)» с процентом совпадения имён; каждое предложение принимается или отклоняется индивидуально; пакетные действия «Принять все» / «Отклонить все» (решение владельца, раунд 2); ничего не создаётся без подтверждения (D1).
...
**Acceptance criteria:**
- AC-6.1: тумблер «Режим защиты» доступен из открытой explain-панели; запуск — одним действием (кнопка в панели или хоткей, без захода в настройки — решение владельца, раунд да/нет 2026-09-21); хоткей — решение §11-Q4 (синк `HOTKEYS` + `user-docs/hotkeys.md`).
...
| 2. Ревью автосвязи (аналитик) | Диалог по бейджу: предложения с процентом совпадения; приём пачкой или поштучно | «Принял пачку — и знаю, как откатить одним шагом» | контроль (+1) | Усталость от длинного списка (12+ предложений) | D10: список перегружен → массовое «Принять все» по ошибке → митигация: откат пачки с подтверждением и подсветкой отменяемого (AC-5.3); группировка/сортировка (У7, Q8); возврат отклонённых до закрытия (У8, Q8) | Приоритизация предложений по уверенности (v2) |
...
| 5. Вопрос комитета (CFO → CPO) | Наведение на цифру — появляется «?» | «Сейчас покажу» | собранность (0) | Память не держит 5 таблиц; все смотрят на экран | D8: «?» не нашли в момент вопроса (hover неочевиден) → пауза, доверие падает → митигация: покрытие 100% (F-1/G2), онбординг-шаг (§14), фолбэк-клик по цифре (У1, Q8) | История недавних объяснений (v2) |
| 6. Открытие цепочки (CPO) | Клик «?» → индикатор построения → дерево + синхронная подсветка + затемнение | «Вот она, вся цепочка — и на канвасе тоже» | контроль (+1) | Раньше: скриншот из Excel, статичная картинка | D3: построение дольше 3 с → пауза в разговоре → митигация: G1/G5 (≤ 100 мс), асинхронность (AC-1.2), инкрементальное раскрытие (§12). D11: проектор блёклит подсветку/шрифт → задние ряды не читают → митигация: контраст ≥ 3:1 (AC-3.4), ×1.5 в режиме защиты (AC-6.2), проверка на проекторе в X2 | Предзагрузка дерева по hover (v2) |
...
Каждая точка отвала CJM обязана иметь митигацию в требованиях (§8) или рисках (§12); митигации, ожидающие решений владельца (Q8), помечены. D1 → AC-5.2/AC-5.3/G4/§12/§10; D2 → F-1/G2/AC-1.1; D3 → G1/G5/AC-1.2/§12 (инкрементальные ветви); D4 → F-2/AC-2.1/AC-2.4/AC-2.5/F-12; D5 → F-6/AC-4.1/AC-4.3; D6 → AC-4.2; D7 → AC-4.3/G6; D8 → F-1/G2/§14 (онбординг)/У1 (Q8, ожидает решения); D9 → AC-3.3/F-5; D10 → AC-5.3/У7/У8 (Q8, ожидает решения)/G4; D11 → AC-3.4/AC-6.2/§12 (проектор); D12 → AC-3.3/§9.2/Q7-ж; D13 → AC-2.2/§10 (граница ответственности функции — репетиция, шаг 4).
...
stateDiagram-v2
    ...
    Ready --> Stale: внешнее изменение модели — канвас/MCP/файл/Apply → чип «Данные изменены»
    ...
    Ready --> Defense: тумблер «Режим защиты» одним действием (AC-6.1)
    Defense --> Ready: Esc — полное восстановление вида (AC-6.4)
```

Состояния: **Closed** — панели нет; **Hover** — «?» виден только под курсором (решение владельца); **Loading** — панель открыта сразу с индикатором построения (AC-1.2); **Ready (fresh)** — панель и подсветка рендерят кэшированный снапшот; **Stale** — модель изменилась, снапшот помечен чипом, данные не перерисовываются сами (AC-3.3/F-5); **Defense** — режим защиты поверх Ready (AC-6.1–6.4, панель остаётся видимой — У9, Q8).

Правила переходов (детализация UX, предложения — Q8):

- чип «Данные изменены» — закреплённая полоса сверху панели, не перекрывает дерево (У4); клик по чипу — единственный путь из Stale в Ready;
- чип возникает только для изменений, инициированных вне панели (правки на канвасе, MCP-агент, перезапись файла, Apply из бара); подмена листа и Apply, инициированные из панели, обновляют оверлей сразу (У3 — согласуется с AC-4.2);
- в Loading канвас не затемняется — затемнение и подсветка появляются атомарно с готовым деревом (У5, один снапшот F-5);
- повторный «?» на другую цифру в Ready/Stale — перестройка на новый корень: одна панель — один корень (У6);
...
|---|---|
| Открытие main stage (FR-042) | панель закрывается, снапшот остаётся в сессионном кэше; возврат через «?» — из кэша ≤ 1 с (У10, Q8) |
...
| Режим защиты | под-состояние панели (§6.4): панель остаётся видимой, дерево укрупняется (У9, Q8); другие оверлеи недоступны |

Синхронизация канвас↔дерево: дерево→канвас — подсветка узлов/рёбер (AC-3.1); канвас→дерево — клик по подсвеченной ноде выделяет и подводит соответствующий узел дерева (У2, Q8 — ожидает решения).
...
| Тач-устройство (нет hover) | фолбэк: клик по цифре открывает панель | У1 (Q8, ожидает решения) |
...
| Повторный «?» на другую цифру | перестройка на новый корень, старый снапшот заменяется | У6 (Q8, ожидает решения) |
...
| Explain-панель | Screen-space панель-оверлей справа (дерево, хлебные крошки, действия листов); данные оверлея — сессионный кеш в памяти (снапшот `LineageTree` + чип устаревания, AC-3.3), UI-состояние панели не сериализуется (Q7-ж, §7.3); взаимоисключимость с main stage / wheel / what-if баром — реестр модальностей по паттерну Q6 FR-042 (F-10) |
...
| F-8 | Режим защиты: запуск одним действием (кнопка в панели или хоткей — решение владельца), типографика ×1.5 из токенов, пошаговое раскрытие уровней, скрытие прозаических строк, выход Esc с полным восстановлением вида | should | AC-6.1–AC-6.4 |
...
- **Q8. Раунд 3 — UX/CJM (2026-09-21, «детально спроектировать CJM»).** Детализация §6 выполнена (акты/14 шагов/D8–D13/§6.4–6.7); ожидают решений владельца (да/нет, предложения в §6): **У1** — фолбэк-триггер: клик по цифре открывает панель наряду с hover-«?» (единственный способ на таче); **У2** — обратная синхронизация: клик по подсвеченной ноде на канвасе выделяет узел в дереве; **У3** — чип «Данные изменены» только для внешних изменений, подмена листа и Apply из панели/бара обновляют оверлей сразу; **У4** — размещение чипа: закреплённая полоса сверху панели; **У5** — канвас не затемняется до готовности снапшота (затемнение атомарно с деревом); **У6** — повторный «?» на другую цифру: перестройка на новый корень (одна панель — один корень); **У7** — ревью-диалог: группировка предложений по паре нод + сортировка по имени переменной, счётчик в бейдже; **У8** — «Отклонить все» обратимо до закрытия диалога («Вернуть все»); **У9** — режим защиты: панель остаётся видимой (дерево укрупняется), не скрывается; **У10** — открытие main stage закрывает панель, возврат через «?» — мгновенно из сессионного кэша.
...
| Триггер «?» не обнаружен в момент вопроса (hover-only неочевиден) | Adoption (D8) | Покрытие 100% (F-1/G2), онбординг-шаг «объясни цифру» (§14, вопрос владельцу), фолбэк-клик по цифре (У1, Q8) |
...
|---|---|---|
| X0 | FR-048 по шаблону `cr-template.md` (фиксация решений Q1–Q7); ADR не требуется (новых зависимостей и правил архитектуры нет) | Разрешение на реализацию |
| X1 | `lineage.rs`: модель `LineageTree` + `build_lineage` (DAG/спиллы/unmapped/циклы/строки) + тесты §9.4 | Чистое ядро, доступное агенту |
| X2 | Триггер «?» F-1 + explain-панель + комбо-подсветка F-4/F-5 (токены, i18n, настройка лимита глубины в FR-039) | **Демо-точка владельцу (Q6)**: G1/G2 замер на демо-сценарии |
| X3 | What-if из дерева F-6: подмены, дельты, сценарии, сравнение, возврат | US-4, G3 |
| X4 | Автосвязь F-7: детектор + диалог ревью + батч-создание + настройка | US-5, G4 |
| X5 | Режим защиты F-8 | US-6 |
...
- [ ] Автосвязь на эталоне «5 таблиц»: recall ≥ 80% / precision ≥ 90% по ревью; фон с дебаунсом; «Принять все/Отклонить все»; создание пачкой одним undo-шагом, откат пачки — с подтверждением и подсветкой (G4, US-5).
...
- [ ] Кэш оверлея: правка модели при открытой панели (канвас/MCP/перезапись файла/Apply) показывает чип «Данные изменены», клик обновляет оверлей из нового снапшота; удаление корня закрывает панель с сообщением (AC-3.3, F-5).
...
- [ ] Все точки входа §14 обновлены; вопросы онбординга и Q1–Q7 закрыты в FR.
...
- Раунд 3 (сессия 2026-09-21, чат) — запрос владельца «детально спроектировать CJM»: детализация §6 (три акта, 14 шагов, D8–D13, машина состояний §6.4, модальности §6.5, краевые траектории §6.6, хронометраж §6.7); вопросы У1–У10 — Q8 (ожидают решений).
