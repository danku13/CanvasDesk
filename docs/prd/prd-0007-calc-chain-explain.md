...
- **Статус:** выполнено (PoC закрыт 2026-09-22 — X0–X6: FR-048, ядро `lineage.rs`, окно проверки/триггер/подсветка/лоадер/кэш; what-if из дерева; автосвязь F-7; режим защиты F-8; MCP `explain_number` F-9, чипы-крошки AC-2.3, У2-вспышка, индикатор покрытия F-12; доки §14 и приёмка §15 G1–G7; демо-гейт Q6)
...
|---|---|---|
| X0 | FR-048 по шаблону `cr-template.md` (фиксация решений Q1–Q8 — что закрыто на момент X0); ADR не требуется (новых зависимостей и правил архитектуры нет) | Разрешение на реализацию — выдано: FR-048 создан 2026-09-22 (Q2/Q3/Q4/Q6 закрыты вариантами по умолчанию) |
| X1 | `lineage.rs`: модель `LineageTree` + `build_lineage` (DAG/спиллы/unmapped/циклы/строки) + тесты §9.4 | Чистое ядро, доступное агенту — выполнено 2026-09-22: итеративный DFS (без рекурсии — глубина ограничена кучей), 15 тестов §9.4 зелёные, clippy/fmt/check workspace зелёные |
| X2 | Триггер «?» F-1 + explain-панель + комбо-подсветка F-4/F-5 + честный лоадер (У5) (токены, i18n, настройка лимита глубины в FR-039) | **Выполнено 2026-09-22**: окно проверки поверх канваса (паттерн main stage), клик по полосе D — фолбэк-триггер AC-1.1 (константа — панель одного узла AC-1.4), Loading без затемнения (У5) с ротацией 12 подписей, Ready — дерево+подсветка атомарно (F-5, FocusView-механизм T23), чип «Данные изменены» по ревизии модели (AC-3.3/У3), сессионный кэш — переоткрытие мгновенно (AC-3.3/G1), stage↔панель взаимоисключительно (F-10/У10); настройка глубины в FR-039; **демо-точка владельцу (Q6) — готова** (G1/G2 замер — на демо) |
| X3 | What-if из дерева F-6: подмены, дельты, сценарии, сравнение, возврат — **выполнено 2026-09-22**: кнопка «Изменить» на листе → inline-поле → подмена через WhatIfOverrides (автосценарий одним undo-шагом); канвас пересчитывается сразу, панель остаётся на снапшоте с чипом (У3/AC-3.3), после клика по чипу дерево показывает дельты «было → стало (+Δ)» на затронутых узлах (lineage_deltas base↔whatif); сценарий персистентен, сравнение/возврат — поверхности FR-017 | US-4, G3 |
| X4 | Автосвязь F-7: детектор + диалог ревью (группировка/сортировка — У7 отложено владельцем на будущее; до решения — поведение прототипа `ux-review-dialog.html`: группировка по паре нод, сортировка по имени переменной) + батч-создание + настройка — **выполнено 2026-09-22**: детектор `autolink.rs` (точные имена Q2, фильтры циклов/дубликатов/самосвязей AC-5.4, бонус единиц, детерминизм), фоновый скан с дебаунсом + бейдж AC-5.5, команда «Найти связи по именам» в меню канваса AC-5.1, диалог ревью (группировка/сортировка по У7, «Вернуть все» У8/AC-5.2), создание пачкой одним undo-батом + подтверждение отката с подсветкой AC-5.3, тумблер FR-039; G4 (precision) — замер на демо/приёмке X6 | US-5, G4 |
| X5 | Режим защиты F-8 (состояние Defense окна проверки поверх полноэкранного канваса — решение владельца, У9 закрыт 2026-09-22, v4; представление — дерево-граф нод с ветками) — **выполнено 2026-09-22**: вход одним действием — тумблер в шапке окна (Q4, без глобального хоткея — v2); тот же граф укрупняется ×1.5 с вписыванием (AC-6.2, потолок 1.5); пошаговое раскрытие — Space/клик по фронтиру/кнопка «Раскрыть уровень» + «Раскрыть всё» (AC-6.3, шаги не сериализуются); проза скрыта структурно (lineage только из формульных строк); выход — Esc в обычный вид окна (вид восстанавливается) / ✕-клик по фону — полное закрытие; канвас не меняется; другие оверлеи недоступны (§6.5 — stage глушится); AC-4.4 — подмена листа работает из защиты | US-6 |
| X6 | Интеграция F-10, MCP `explain_number` (F-9, must), аудит регрессий, доки §14, приёмка §15 (G1–G7) — **выполнено 2026-09-22**: MCP-инструмент + text-first мост, полные чипы-крошки AC-2.3, У2-вспышка канвас→дерево, индикатор покрытия F-12 (opt-in), интеграционные тесты G7/G4 на эталоне владельца, доки §14 (node/edge/SPEC/user-docs/ACCEPTANCE), приёмка G1–G7 — чек-лист в `docs/ACCEPTANCE.md` | Закрытие PoC — выполнено |
...
> Отмечено 2026-09-22 (X6) — детали и замеры: `docs/ACCEPTANCE.md`,
> раздел «FR-048 / PRD-0007 — Цепочка расчёта цифры (PoC X0–X6)».

- [x] Демо-сценарий юнит-экономики (эталон владельца: 5 нод-таблиц, цепочка «стоимость подписки», допущения NPL/трейд-ин): от «?» до полной цепочки ≤ 3 с; вопрос «почему цифра такая» закрыт без выхода из канваса (G1, G2).
- [x] Честный лоадер: при открытии панели — ротация 12 подписей этапов построения (ключи i18n RU/EN), затемнение атомарно с готовым деревом (AC-1.2, У5).
- [x] ≥ 5 именованных вариаций сценария за одну сессию из дерева, каждая с дельтой к базе; возврат к базе одним действием (G3, US-4).
- [x] Автосвязь на эталоне «5 таблиц»: recall ≥ 80% / precision ≥ 90% по ревью; фон с дебаунсом; «Принять все/Отклонить все» с обратимостью «Вернуть все» до закрытия диалога (У8); создание пачкой одним undo-шагом, откат пачки — с подтверждением и подсветкой (G4, US-5).
- [x] Юнит-тесты: `build_lineage` (все случаи §9.4), `autolink` (детерминизм, фильтры, перепроверка при переименовании), инвариант F-5 (одна модель для панели и подсветки).
- [x] Индикатор покрытия «Цепочки: N%» включается настройкой (opt-in) и корректно считается на демо-модели (F-12, G2).
- [x] Интеграционные тесты: edges + main stage + what-if бар — зелёные; объяснение уходит/возвращается без следов (G7).
- [x] Кэш оверлея: правка модели при открытой панели (канвас/MCP/перезапись файла/подмена листа/Apply) показывает чип «Данные изменены», клик обновляет оверлей из нового снапшота; удаление корня закрывает панель с сообщением (AC-3.3, F-5, У3).
- [x] `cargo test --workspace`, clippy, fmt — зелёные; `scripts/wasm_gate.sh` и `scripts/token_lint.sh` — зелёные.
- [x] Стресс-замер: 1000 нод / 3000 рёбер → 60 fps пан/зум, бюджет чистой сборки дерева ≤ 100 мс, построение в UI асинхронное (G5).
- [x] Round-trip `.canvas` зелёный; новых сериализуемых полей нет (G6).
- [x] Контраст подсветки/дельт ≥ 3:1 на обеих темах; различимость без цвета (AC-3.4).
- [x] Режим защиты: вход одним действием — состояние Defense окна проверки поверх полноэкранного канваса (паттерн main stage) с тем же деревом-графом нод с ветками (укрупнённое), пошаговое раскрытие уровней (клик/пробел), выход Esc/✕/клик по фону без изменений канваса (AC-6.1–AC-6.4, У9 закрыт).
- [x] Все точки входа §14 обновлены; вопросы онбординга и Q1–Q8 закрыты в FR.
- [x] MCP `explain_number` возвращает линейную развёртку дерева с адресами и значениями (F-9, must — решение владельца).

## 16. История изменений

- `2026-09-22` — агент: **X6 выполнено — закрытие PoC** (F-9/F-10/F-12,
  аудит, доки §14, приёмка G1–G7; полный changelog — FR-048).
  MCP `explain_number` (F-9, must — 40-й инструмент): линейная развёртка
  дерева происхождения текстом с адресами `[node_id:строка]`, значениями,
  формулами и каналами прихода; text-first мост (`render:"text"` —
  content text = готовое объяснение), преамбула при активных what-if
  подменах; skills-пакет v2. Полные чипы-крошки (AC-2.3 — UX-шлифовка
  X2): по чипу на уровень пути вида с обрезкой пути кликом; переполнение
  — последние уровни. У2-вспышка (§6.5): клик по подсвеченной ноде
  канваса выделяет и подводит узел дерева (раскрытие родителя на границе
  лимита глубины; затухающая рамка 700 мс); в защите канвас не отвечает.
  Индикатор покрытия «Цепочки: N%» (F-12, should): opt-in тумблер
  FR-039, расчёт `chain_coverage` (итоги Ok+Err + вычисляемые строки
  листов; знаменатель не сжимается при ошибках — покрытие честно
  падает), кэш по ревизии. Интеграционные тесты X6 (4): G7 — объяснение
  уходит/возвращается без следов (F-5/AC-3.3); MCP-паритет what-if;
  G2/F-12 — 100% на связанном эталоне и падение после разрыва; G4 —
  precision/recall 1.0/1.0 на эталоне «5 таблиц» (оракул 712 370).
  Доки §14 обновлены (node/edge/SPEC/user-docs+HOTKEYS/ACCEPTANCE).
  Статус PRD → «выполнено», DoD §15 отмечен; онбординг не расширялся
  (правило AGENTS.md закрыто приказом владельца по PRD-0009).
- `2026-09-22` — агент: **X5 выполнено** — режим защиты F-8 (состояние Defense окна проверки, §6.4/§6.5). Вход одним действием — тумблер в шапке (Q4: глобальный хоткей — v2); укрупнение ×1.5 с вписыванием в окно (AC-6.2, потолок `DEFENSE_SCALE_MAX` — прототип v4); пошаговое раскрытие уровней — Space/клик по фронтиру/кнопка «Раскрыть уровень», «Раскрыть всё» (AC-6.3; шаги — runtime-состояние, не сериализуются); проза скрыта структурно (lineage строится только из формульных строк — AC-6.2); выход — Esc в обычный вид окна (путь крошек и ручные раскрытия восстановлены) / ✕ и клик по фону — полное закрытие; канвас не изменяется (AC-6.4); main stage в защите не открывается (§6.5 — другие оверлеи недоступны); подмена листа из защиты работает (AC-4.4). Код: `explain_ui.rs` (модель Defense: enter/exit/step/reveal_all, `defense_fit_scale`, геометрия тумблера и кнопок футера, `has_hidden`), `app.rs` (единая точка вида `explain_view` для рендера и hit-тестов, тумблер/кнопки/подсказка, двухступенчатый Esc, гард stage, гвард шага по скрытым узлам), i18n +5 ключей RU/EN. Тесты: explain_ui 18 (+6 defense — AC-6.1/6.2/6.3/6.4/AC-4.4). Гейты: fmt/clippy -D warnings/test --workspace (1542)/token_lint — зелёные.
- `2026-09-22` — агент: **X4 выполнено** — автосвязь по именам (F-7, AC-5.1–AC-5.5). `canvas-core/autolink.rs` — `AutolinkProposal`/`find_proposals`: детектор точных имён присваиваний (Q2 — решение X0): источники — присваивания текстовых нод (адресация `fromOutput` — последнее определение имени FR-029, адресация живёт при сдвиге строк), потребности — параметры снапшота шаблонных нод и `$имя`-ссылки (`Expr::Param`) формульных строк текстовых приёмников без поставщика-проливания; создаваемое ребро — проливание `toParam` (не занимает слоты `$1..$N`, регистрирует qualified-ключ FR-050); фильтры AC-5.4 — циклы `creates_value_cycle`, дубликаты адресованных рёбер (позиционные рёбра пары — НЕ дубликат: слот ≠ проливание), самосвязи, занятые параметры (конфликт «один вход на параметр» FR-050 Н4); детерминизм — порядок `canvas.nodes` + сортировка (from,to,param); бонус Q2 — `unit_match` (совместимость единиц константного RHS и спецификации параметра); скан листов — общий с деревом (`lineage.rs`: `Sheet`/`Ref`/`collect_refs` → `pub(crate)`), фенсы исключены как в движке. `canvas-scene` — undo-теги: `set_undo_tag`/`peek_undo_tag` (параллельный стек) — класс действия на снапшоте «до». `canvas-app/autolink_ui.rs` — чистая модель диалога: `Review` (группировка по паре нод «исток → приёмник», сортировка по имени переменной — поведение прототипа У7), `ItemState` Pending/Accepted/Rejected, layout+hit-тесты (тела/баннер/футер/строки, скролл D10). `app.rs`: фоновый скан с дебаунсом 700 мс после правок (AC-5.5 — перепроверка переименований П8), ненавязчивый бейдж-индикатор (клик — ревью; скрывается при открытых оверлеях), команда «Найти связи по именам» в меню канваса (AC-5.1, пустой результат — тост), диалог ревью — верхний модал реестра поверхностей (§6.5: панель объяснения прячется на время диалога; Esc/✕/клик мимо — закрытие), строки — «Принять/Отклонить» (повторный клик — сброс), баннер отклонённых с «Вернуть все» (У8/AC-5.2), массовые «Принять все/Отклонить все», «Создать связи (N)» — ОДИН undo-бат (`set_undo_tag("autolink_batch")` + push_undo ДО мутации; рёбра `fromOutput=toParam=имя`; живой пересчёт; тост «один undo-шаг»), откат пачки — с подтверждением и подсветкой отменяемого (перехват Ctrl+Z по тегу верхнего снапшота: `AppDialog::AutolinkRollback` + focus_edges рёбер пачки; «Отмена»/Esc гасит подсветку; любое другое действие снимает тег — естественная инвалидация), колесо над телом — скролл списка. Тумблер «Автосвязь по именам (фон)» — настройки FR-039, раздел «Связи и порты» (AC-5.5). i18n +21 ключ RU/EN. Тесты: core 360 (+12 autolink), app lib 269 (+4 autolink_ui), интеграционные `integration_autolink` 3 (пачка+undo+перепроверка; фильтры на живой модели; round-trip G6). Гейты: fmt, clippy -D warnings, cargo test --workspace, token_lint, wasm_gate (3/3, wasmtime 49.0.0) — зелёные. Инцидент: диск окружения (9.9 ГБ) переполнился линковкой тест-бинарников (250–350 МБ каждый) — локально применён временный `[profile.test] debug=0`, в репозиторий не попал.
- `2026-09-22` — агент: **X3 выполнено** — what-if из дерева (F-6, AC-4.1–AC-4.3): у редактируемых листьев (строки Numi-листов с вычисленным значением) — кнопка «Изменить» → inline-поле (preset — текущая подмена сценария или исходник строки; каретка, подсказка пустого поля); Enter/клик мимо — коммит: подмена через `WhatIfOverrides` (FR-017), сценарий автосоздаётся одним undo-шагом (CR-016-автоимя), `recompute_flow` — канвас пересчитывается сразу (живая модель; дельты в полосе D корня — существующий `footer_delta` whatif_node_map), панель остаётся на кэшированном снапшоте — чип «Данные изменены» по новой ревизии (У3/AC-3.3), клик по чипу — перестройка дерева из flow_active С дельтами против flow_baseline (пара деревьев строится одним фоновым проходом — G5; `lineage_deltas` сопоставляет узлы по адресу (node_id, line)); дельты в дереве — формат FR-017 «было → стало (+Δ)» цветом whatif_badge (AC-4.2). Клавиатура окна при открытом поле: символы/Backspace/Enter/Esc. AC-4.3: сценарий персистентен (canvasdesk.whatif, round-trip тестирован; подмены листов-констант без «=» валидны — validate_scenario учитывает числовые/expr-строки), сравнение/«База»/Apply — существующие поверхности бара FR-017. AC-4.4 (подмены из режима защиты) — механика не зависит от состояния окна, проверка — в X5. i18n +2 ключа. Тесты: 345 core (+3), 11 explain_ui (+4), 3 интеграционных X3. Гейты зелёные (fmt/clippy/test workspace/token_lint/wasm_gate).

- `2026-09-22` — агент: **X2 выполнено** — окно проверки цепочки расчёта цифры поверх полноэкранного канваса (паттерн main stage, решение У9 v4): триггер — hover-«?» (Closed→Hover) и клик по полосе результата D (AC-1.1; константа — панель одного узла AC-1.4); Loading открывается сразу, канвас не затемняет (У5), сборка в фоновом потоке (G5) с ротацией 12 подписей; Ready — дерево (безье + карточки «значение/формула/адрес», полосы рода, бейдж «+N глубже») и подсветка цепочки на канвасе появляются атомарно (F-5, механизм FocusView T23, пучки — OR-семантика); чип «Данные изменены» — по монотонной ревизии модели при любой мутации (У3 — без исключений), клик — перестройка (AC-3.3); сессионный кэш — закрытие не уничтожает снапшот, переоткрытие мгновенно (G1); Esc/✕/клик по фону — закрытие; main stage ↔ панель взаимоисключительны (F-10, У10). Настройка глубины авто-раскрытия (AC-2.3) — dropdown FR-039 [2/3/4/без ограничения]; цвета листа — AC-3.4 (контраст ≥ 3:1, светлая тема — затемнённый слот); i18n RU/EN. Скоуп: построчные триггеры — X3 (адресация строки нужна what-if), полные чипы-крошки и У2-вспышка — X6. Гейты зелёные (fmt/clippy/1115 тестов/token_lint/wasm_gate). Демо-точка Q6 — готова.
...
- `2026-09-22` — агент: решение владельца «реализуем PRD-0007» — старт реализации по дорожной карте §13. **X0:** создан реализационный FR `docs/change-requests/fr-048-calc-chain-explain.md` (Q2/Q3/Q4/Q6 закрыты предложенными в PRD вариантами по умолчанию: матч автосвязи по точным именам, «?» у константы — панель одного узла, запуск защиты тумблером шапки — хоткей v2, демо-гейт после X2); индекс CR/FR — указатель на FR-050. **X1:** `canvas-core/src/lineage.rs` — `LineageNodeId`/`LineageNode`/`LineageChild`/`LineageVia`/`LineageTree`/`LineageError`/`LineageFlow` + `build_lineage(canvas, flow, root)`: итеративный DFS с явным стеком задач (рекурсия убрана после переполнения стека на цепочке 1005 нод — глубина ограничена кучей, §9.2/G5); проекция связей: value-рёбра с адресацией (слоты/`$in`, проливания `toParam` — сильнее дефолта, `fromOutput` — последняя присваивающая строка или секция outputs шаблона, приоритет `fromLine` — зеркало `edge_source_value`), локальные переменные Numi-листа (последнее присваивание до строки, фенсы — как в движке), CSV-колонки data-нод — листья со значением ячейки; терминальные узлы «цикл» (путь построения; режим `Cycled` строит топологию при упавшем пересчёте), «значение не подставлено», «не связано»; бюджет `LINEAGE_MAX_NODES` = 4096 с ленивым общим маркером усечения (G5); ромб — без дедупликации (дерево, не DAG); значения — из `FlowSolutions` с override значением ребра для адресованных выходов; порядок детей — `canvas.edges`. Тесты §9.4 (15): цепочка, ромб, переменные листа, спиллы, unmapped, цикл, лист-константа (явная формула и Numi-лист), корень-строка vs корень-итог, ошибки корня, named output, приоритет адресации, CSV-лист, фенсы, цепочка 1005 нод (глубина 2009 — без переполнения), усечение ромба 8192 листьев. Движок не меняется: `flow.rs` — только `spill_source_title` → `pub(crate)` (заголовки нод дерева). Гейты: cargo test canvas-core — 320 lib + остальные зелёные; clippy -D warnings; fmt; cargo check --workspace. Статус PRD → «в работе».

## 17. Источники истины

> Реализация X2 (2026-09-22): `crates/canvas-app/src/explain_ui.rs` (модель окна: геометрия/видимость/лейаут/состояния/лоадер), `app.rs` (open/close/click/frame, триггер полосы D, hover-«?», фоновая сборка, `explain_chain_focus` — подсветка через FocusView), `scene.rs` (`revision`/`flow_cycle`), `canvas-core/settings.rs` (`explain_depth_limit`), токены/темы (EXPLAIN_LEAF), i18n 34 ключа. Подробности — changelog FR-048 и worklog.
