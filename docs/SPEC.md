...
`text`/`group`, цвет — только пресеты `1..6`, value-рёбра `flowKind:
"value"`). Формат `.canvas` не расширяется: инстансер
(`canvas-scene::scheme_apply`) ремапит id и вставляет стандартные ноды/рёбра;
валидатор реестра (`canvas-core::schemes`) — лимиты ≤ 200 нод / ≤ 400 рёбер,
обязательные поля. Схемы компилируются в бинарник (`include_dir`,
