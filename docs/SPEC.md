...
| Меню помощи и документация | Кнопка «?» (кластер ⚙/тема): «Документация ▸» — 7 разделов во встроенном просмотрщике (правый док: колесо — прокрутка, внутренние ссылки — переход, × / Esc / клик вне — закрыть); «Пройти онбординг» (FR-027/FR-028) |
...
После каждого релиза агент ОБЯЗАН создать запись в `DEVLOG.md`:
