## 2026-09-26 — feat(scheme): FR-071 v2 — сеточная раскладка нод (grid mode) — ряды выровнены, «прилипание» исключено конструктивно

- **Запрос владельца:** «…оптимизация расстановки нод не даёт результата: ноды прилипают друг к другу и расставляются хаотично; в идеале — приблизительно по сетке, чтобы удобно было визуально считывать».
- **`canvas-core/src/scheme_layout.rs` — v2 (grid mode):** свободные y-позиции v1 (независимое центрирование колонок на родителях + сдвиги ±4×64 px) заменены строгой сеткой: шаг колонки `CELL_W = max_w + LAYER_GAP`, шаг ряда `CELL_H = max_h + ROW_GAP` (ROW_GAP 64→80 — рамки соседних групп больше не соприкасаются, дефект v1 устранён). Позиция ноды — левый-верх ячейки (x = колонка·CELL_W, y = ряд·CELL_H); колонка занимает подряд идущие ряды (base — по среднему ряду центров родителей, clamp ≥ 0); ряды — общая координата сцены (выровнены между колонками и кластерами); аннотации — соседние колонки сетки (левая — со сдвигом потока на колонку вправо). Зазоры гарантированы построением: по вертикали ≥ CELL_H − max_h ≥ ROW_GAP, по горизонтали ≥ LAYER_GAP. Оптимизация пересечений — в единицах сетки: свапы соседних рядов, переносы на другие ряды, сдвиги колонок целыми рядами (±4), НОВОЕ — совместные сдвиги пар колонок (±2×±2) и внешние раунды до фикспойнта (MAX_JOINT_ROUNDS=8): локальный минимум ab-testing («control→effect × significance») устранён, оракул 0 пересечений держится на всех 6 схемах.
- **Тесты:** 13 юнит-тестов scheme_layout (обновлены group_frame_covers_children/standalone_annotations/empty_no_flow под ряды; новые `nodes_snap_to_uniform_grid` — привязка к ячейкам + гарантия зазоров, `rows_align_across_columns` — выравнивание приёмников); oracle-тесты scheme_apply прежние — 0 edge×node, 0 bbox-пересечений, дети в рамках, детерминизм (все зелёные).
- **Диагностика:** пример `canvas-scene/examples/layout_debug.rs` (позиции + детали пересечений по любой схеме: `cargo run --example layout_debug -p canvas-scene -- ab-testing`), витрина `examples/layout_showcase.rs` (+ playwright-сценарий вне репо) — wasm-скриншоты project-budget и ab-testing: колонки/ряды выровнены, группы обрамлены, аннотации по бокам (download/fr071_grid_gridbudget.png, fr071_grid_gridabtest.png). Эмпирика wasm-стенда: wheel-DOWN (dy>0) = зум-аут (уточнение к Task 7).
- **Гейты:** cargo test --workspace (~2150 тестов) зелено; clippy --workspace --all-targets -D warnings чисто; fmt чисто; wasm-гейт canvas-core/canvas-scene (wasm32) чисто (ADR-0011).
- **Доки:** fr-071-scheme-smart-layout.md §История (v2-запись), док-комментарии scheme_apply/scheme_layout.
...
## 2026-09-26 — feat(ui/app): W3.3 финал — пад-семантика Text{pad_x,h} + TextMeasurer::wrap, провода width_of→Text, витрина Row v1 → Table, аудит FR-074 (лид + 3 сабагента)

- **Агент:** Super Z (сессия web-3e2a9c55; запрос владельца: «заменяй и доделывай остаток бэклога»; base ae0a806 → main 12aba65).
- **Этап 1 — фундамент canvas-ui (лид, additive; 4f89415, aa23181):** `MeasuredItem::Text += pad_x` (суммарный горизонтальный пад; бит-в-бит ≡ `Fixed{width_of+pad}`; pad_x: 0.0 — прежняя семантика) — названное в W3.1 условие миграции «width_of → размер ребёнка»; `Text += h: Option<f32>` (явная высота дизайн-константой) — находка гейта: `CHIP_HEIGHT 26` ≠ измеренной `13·1.3=16.9`, без `h` миграция whatif меняла бы визуал (I-1); `TextMeasurer::wrap` — жадный перенос по словам (семантика прецедента `admin_ui::wrap_text`: длинное слово не рвётся, пустой текст → одна пустая строка). +4 оракула: полный rect бит-в-бит Text{pad_x,h}≡Fixed; кнопочная проводка max(min_w,w+pad) — на уровне ВЫХОДА движка (round_px on freeze — первый вариант теста словил разницу 148.0 vs 147.65); wrap ≡ ручному оракулу RU/EN ×4 потолка.
- **Этап 2 — волна 3 сабагентов (git-worktree от aa23181, последовательный merge):**
  - **2-a (b5b5958):** витрина кита — секция Row v1 УДАЛЕНА (решение владельца по открытому вопросу M6 «замена витрины Row на Table»), Table — единственная табличная секция; отрисовка не тронута (row_rows generic, overlays — только комментарий); оракул бит-в-бит «Row ≡ Table на одних данных» сохранён тестом (row-путь kit::row_guides/row_layout — теперь только тест); ключ i18n kit.section.row удалён (RU/EN); пин шага «Table следует за Icons» = SECTION_GAP+18.
  - **2-b (f095b22):** провода → самоизмерение: whatif бар — чипы/кнопки `Fixed{chip_width/btn_width}` → `Text{pad_x, h: Some(CHIP_HEIGHT)}` / `Text{pad_x, min_w: BTN_WIDTH, h}` (rect-тесты зелёные БЕЗ правки значений); template чип категорий → `Text{pad_x: 20}`; `admin_ui::wrap_text` → делегат `TextMeasurer::wrap`. chip_width/btn_width/category_chip_width остались ТОЛЬКО первым проходом (ширина бара/полосы — аналог двухпроходных направляющих).
  - **2-c (2c435e9, аудит — код не менялся):** z-index — НЕ кандидат на текущем band-пути хромировки (Painter::z_group сортирует Painter-журнал; порядок экранной хромировки = UiLayer::DRAW_ORDER + порядок пушей handler.rs; dropdown компенсируется skip строк — кандидат при переводе хромировки на Painter); Grid Auto/minmax — карточки тем: 2 фиксированные карточки на measured-Row (тот же делёж в другом месте + риск дрейфа флоата), template_ui сеток нет; Sticky — заголовки «Как считается» статичны архитектурой, потребителей 0 (sticky-строки внутри списков = новая фича, решение владельца; движок готов). Статус-абзацы 6–8 в §9.3 каталога.
- **Классификация остатка width_of (§9.3.1 закрыт):** замеры контейнеров (dialog_measured_in, панель хоткеев, тултип-боксы), screen-текст (debug_overlay, docs-спаны), канвас-домен (row_grid бейджи/ellipsis) — НЕ проводки «width_of → ребёнок»; docs_ui span-перенос — контентный движок вьюера (href per span + разрыв по глифам) — остаётся, замер фактический.
- **Интеграция:** merge origin/main (FR-028 v2 — параллельная сессия, 14 файлов) без конфликтов; попутный фикс main: tour_signal.rs doc-отступы (clippy -D warnings) + fmt-дрейф (предсуществующие на d4a3313).
- **Гейты (финал):** cargo test --workspace **2089 passed / 0 failed** (база 2072; +4 оракула canvas-ui, +12 переезд/пины волны); clippy --workspace --all-targets -D warnings; fmt --check; wasm_gate.sh --check OK (wasm-target доставлен в среду).
- **Доки:** каталог §9.3.6–8/§9.5 (W3.3 — ИСПОЛНЕНО, вердикты FR-074), FR-068 changelog, ui-kit §4/§5 (pad_x/h/wrap), ACCEPTANCE FR-068.10, worklog.
- **Ручная проверка владельцу:** витрина кита — секция «Таблица (Table)» теперь одна табличная (Row-секции больше нет); whatif-бар/шаблоны/админ-перенос выглядят как прежде (бит-в-бит).
- **Telegram:** план (295) → этап 1 (298) → этап 2 (303) → интеграция (304) → финальное саммари.

---

## 2026-09-26 — §Контракт-7: замер wasm-бандла исполнен (сессия web-49ae985b)

- **Запрос владельца:** «замерь место, если влезет то давай проверим» — снятие отложки §Контракт-7 (W3/W4: «release-сборка не помещается в диск среды»).
- **Среда:** свежая песочница 9.9 ГБ диск (8.5 ГБ свободно на старте) / 3.9 ГБ RAM / 2 ядра; тулчейн с нуля — rustup minimal 1.98.1 stable + wasm32-unknown-unknown + trunk 0.21.14 + binaryen 125 (версия CI) и 133; brotli — python-модуль (CLI в среде нет; `sudo` недоступен). Особенность песочницы: фоновые процессы убиваются между вызовами — cargo гнали синхронными порциями (артефакты переживают, сборка резюмится).
- **Метод:** канонический `web_bundle.sh` (trunk build --release → wasm-bindgen 0.2.127 → wasm-opt -Oz → отчёт) на main **93e553c**; A/B-атрибуция — снапшоты ed0c0c4 (W2), 772ab9c (пре-webgl), eef589c (пост-webgl), ae0a806 собирались тем же пайплайном в идентичной среде; верификация метода — W2-снапшот воспроизвёл задокументированный агентский замер с точностью **0.08%** (9,152,760 vs 9,159,641).
- **Результат (main 93e553c):** бандл **11,729,302 raw (11.18 МиБ) / 2,629,954 brotli (2.51 МиБ) / 4,891,998 gzip (4.66, оценка сверху)**; wasm 11,516,622 raw (wasm-opt −25%) / 2,595,466 brotli; js-glue 141,333; tour.js 48,821; загрузка ~2 с @25 Мбит/с ✓. Бюджет §8.8: **raw ≤8 ПРЕВЫШЕН (+3.18 МиБ), brotli ≤4 OK**, загрузка OK.
- **A/B-атрибуция (wasm post-opt, binaryen 125):** W2 9,152,760 → **+212 КиБ** 48 коммитов фич (W3 Component-слой, Table v2, FR-073, FR-040 v2, FR-028 v2, icons, схемы 6→10) → 9,369,611 → **+2,124,689 (+2.03 МиБ) WebGL2-фолбэк (FR-WASM-02, `wgpu features=["webgl"]`)** → 11,494,300 → **+37 КиБ** FR-074+W4 (taffy −, net) → 11,532,033 (ae0a806) → −15 КиБ W3.3+tour → main. Binaryen 125 vs 133 — 8 КиБ (шум). Ожидание W4 «−376 КиБ от taffy» не реализовалось: webgl-фолбэк, вошедший в main параллельно, перекрыл экономию на порядок.
- **Вердикт/решение владельцу:** превышение raw — по контракту решение владельца. Варианты: (1) пересмотреть raw-бюджет (brotli 2.51 ≤ 4 и загрузка ~2 с в норме — лимит писался до webgl-фолбэка), (2) webgl за opt-in-фичей/ленивым выбором бекенда (−2.03 МиБ), (3) статус-кво с фиксацией в ACCEPTANCE.
- **Попутные находки:** (а) прод-деплой Pages `/app` — 404 на ВСЕ ассеты (приложение в проде недоступно; последняя рабочая приёмка была 2026-09-19) — починка вне скоупа замера; (б) `web_bundle.sh` в средах без brotli-CLI вердиктит «FAIL-compressed» по gzip-оценке, тогда как реальный brotli в бюджете (CI с brotli — корректно); (в) main переписан в линейную историю (59d77cb в предках; tooltip-фикс 0a77d89 и хотфикс W3.2 a11f6c9 в main); (г) за время замера owner запушил 3 коммита (tip 93e553c) — замер выполнен на актуальном tip.
- **Гейты:** сборка release зелёная; код НЕ менялся (замер + доки). Доки: FR-068 (статус/чеклист W4/changelog), ACCEPTANCE FR-068.7, worklog.
---

## 2026-09-26 — feat(core)+feat(app/ui)+refactor(ui/render): волна 3 субагентов — разряды, звёздочка, пересборка поверхностей, аудит шаблонных нод

- **Агент:** Super Z (сессия web-77d836dd; запрос владельца: 4 задачи «сделай сабагентами»; base ae0a806 → коммиты 19012c9, 362fa21, 1cc6ac9 — после ребейза на origin/main 8f3a71e (интеграция с параллельной W3.3)).
- **1-a (core, группировка разрядов):** `format_num` (canvas-core, D-1) — целая часть ≥1000 группируется по 3 цифры NBSP U+00A0 (`1 234 567`); NBSP выбран, т.к. обычный пробел — точка переноса (тултип `wrap_words` исключает NBSP из разбиения — тест); дробная часть/NaN/±∞/6 значащих цифр — как раньше. `format_num_raw` (pub) — машинный формат без групп для текстов, парсящихся обратно: template `instantiate` и MCP `param_set` (canvas-scene/mcp.rs) — NBSP-группа `1 000` разобралась бы как `1 × 0` (это ловили гейт-оракулы CP3/CP5 mini-reference). Тест-ожидания NBSP по canvas-scene/app/render/scene-worker.
- **1-b (core, звёздочка):** `*` как явное умножение УЖЕ работала (лексер `b'*'` → `Tok::Star`, `parse_mul_tail` → `BinOp::Mul`); закреплено регрессионными `star_multiplication_eval/_variables_and_lines` (слитно/пробелы, единицы, приоритет, лево-ассоц., переменная `x`, `line_kind` = Expression, `1000 * 2` → `2 000`); user-docs/calculations.md — `*`, `× · ⋅ ✕ ⨯`, `x`/`х`, неявное.
- **2 (surfaces, «О интерфейсе» + «UI-консоль»):** витрина kit_ui — новые секции component::Row (RowProps/Row::paint), component::Panel (PanelProps/Panel::layout), SqueezeTail (12×72px), MainAlign::SpaceBetween, Column+Spacer (lay_out_measured); икон-глифы = ВСЕ варианты kit::Icon (тест-инвариант); скролл хвоста — детерминированный scan_to. Админпанель admin_ui — хвост матрицы: dropdown закрыт/открыт, tooltip, toast, строки списка Normal/Hovered/Selected + scroll_bar, строки таблицы зебра/фокус/Selected; секция «Наполнение» — карточка ноды с табличным телом FR-061 (хедер + 3 строки, зебра, бейдж у Σ). i18n ~100 ключей RU/EN; отрисовка — kit-адаптер overlays.rs; существующие демо бит-в-бит (I-1).
- **3 (templates, аудит шаблонных нод):** отдельного template-пути НЕТ — карточка байт-равна text-ноде (оракул cards.rs); геометрия таблицы тела УЖЕ kit (RowGuides, measure_row_cells, leader_dash_rects, ROW_BADGE_PAD_H, ellipsis); шелл/полоса/иконка — world-space GPU-путь (FR-068 W3 §9.4 «канвас-домен не мигрирует» — screen-space кит не знает камеры; полная миграция = camera-aware kit, non-goal). Единственный дубликат (зебра-прогон D-5) перенесён: `canvas_ui::row_guides::zebra_run_flags` (+5 тестов кита, оракул `zebra_run_matches_inline_oracle` — 9 сценариев, бит-в-бит). Доки: docs/plans/fr-061-template-node-kit-alignment.md (карта элемент→реализация).
- **Координация волны:** core/surfaces упали по таймауту tool-вызова ПОСЛЕ завершения правок (компилируемых); лид довёл интеграцию: нашёл и закрыл parse-back путь `param_set` (машинный формат), 2 NBSP-ожидания (spill-тултип, explain), clippy (doc-list, len_zero), fmt.
- **Гейты:** workspace 2107 passed / 0 failed (66 наборов); clippy --workspace --all-targets -D warnings; fmt --check; wasm_gate.sh --check OK (wasm32 target доустановлен в среде).
- **Telegram:** план → отчёты 1-a/1-b, 2, 3 → финальное саммари.
- **Интеракция с параллельной W3.3:** витрина Row v1 удалена решением владельца (2-a W3.3) — в пересобранной витрине табличная секция ОДНА (Table, семантика W3.3), а демо component::Row показывает retained-API компонентного слоя (RowProps/Row::paint) — это не возврат Row v1; скан-предикаты тестов согласованы (row_rows == 4 — Table-only; икон-глифы ×8 — полный инвентарь). Ручная проверка: открыть «О интерфейсе» (новые секции в конце) и «UI-консоль» → «Компоненты» (хвост матрицы) / «Наполнение» (карточка с табличным телом).

## 2026-09-26 — feat(core/layout): FR-010 v2 — Sugiyama-lite для авто-раскладки связанных карточек (DAG-корректность + barycenter + минимизация пересечений + radial parent/child)

- **Запрос владельца:** «Надо понять какой метод взять, чтобы доработать v1 функционал расстановки графов, который сейчас работает не очень эффективно и корректно. Ранее мы решали какие формулы и методы расстановки графов использовать и не взяли ни один из них».
- **Анализ v1 (`layout.rs::plan_related_layout`):** 4 дефекта: (1) BFS со знаковой глубиной давал каждой ноде ОДИН уровень (первый визит) — DAG merge-point с двумя путями терял топологию; (2) нет минимизации пересечений рёбер с нодами; (3) нет смысловой группировки (barycenter); (4) Radial делил кольца между родителями и потомками по `|level|` — наложение (зафиксировано в комментарии v1).
- **Метод:** переиспользован движок Sugiyama-lite из FR-071 (`scheme_layout.rs`, сделан 2025-09-25 для схем целиком), адаптированный под seed-anchored окрестность. Конвейер: (1) достижимость от семени (BFS по ненаправленному графу); (2) знаковое longest-path слоение — `forward_level` (longest path ОТ семени по исходящим, Kahn + фолбэк для циклов) и `backward_level` (по входящим), знак = `forward − backward`; направленная достижимость гарантирует: предки не получают forward-уровень, потомки — backward; (3) barycenter (2 свипа — по предкам, по потомкам); (4) позиции: семя-якорь, положительные уровни в сторону роста, отрицательные — обратно, колонки центрируются на среднем центре размещённых предков; (5) минимизация пересечений edge×node (swap соседей + поперечные сдвиги колонок + переносы на другую строку; колонка семени level 0 иммунна к сдвигам); (6) группы едут с детьми (инвариант translate_group, без изменений).
- **Radial v2:** родители (−N) и потомки (+N) разведены по разным полуокружностям — центр 180° (предки, слева) и 0° (потомки, справа), инсет-зазор 0.84π на каждой стороне → гарантированный разрыв на вертикалях (±90°), наложений нет. Угловой barycenter (порядок по среднему углу соседей) для эстетики. Одиночная нода — в центре стороны (раньше — на границе, colliding).
- **Переиспользование:** geometry-хелперы `seg_intersects_rect`, `right_center`/`left_center`, `seg_len`, тип `Rect` из `scheme_layout.rs` сделаны `pub(crate)` — FR-071 не тронут (0 регрессий). Слоение/barycenter/refine написаны inline в `layout.rs` под знаковые уровни и 3 режима (FR-071 работает с беззнаковыми уровнями кластеров).
- **Контракт сохранён:** `plan_related_layout(canvas, seed, mode) -> LayoutPlan`, `LayoutMode::{TreeHorizontal, TreeVertical, Radial}`, константы `LEVEL_GAP`/`SIBLING_GAP`/`RADIAL_RING_STEP`, семя-якорь, группы едут с детьми, ноды без связи не трогаются. App-интеграция (`apply_related_layout` → `PaletteAction::Layout`, палитра выделения, один undo-шаг) — без изменений.
- **Тесты (12 layout, вкл. 10 новых):** tree-H/tree-V chain (топология + 0 bbox-пересечений), radial rings (одно кольцо, 0 пересечений), **radial parent/child separated** (предок слева, потомки справа, 0 наложений — новый тест на баг v1), cycles terminate, invalid/isolated seed, determinism, group moves children once, **dag_merge_point_levels** (a→b→c→d + b→d, d на уровне 3 не 2 — longest-path), **barycenter_groups_siblings** (общий родитель → рядом), **crossing_minimization_diagonal** (длинная диагональ через колонку устраняется, 0 пересечений edge×node).
- **Гейты:** `cargo test -p canvas-core -p canvas-scene -p canvas-app` — зелено (426+ / 43+ / 398+); `cargo clippy --workspace -- -D warnings` — чисто; `cargo fmt --check` — чисто; wasm-гейт `cargo check -p canvas-core -p canvas-scene --target wasm32-unknown-unknown` — чисто (ADR-0011).
- **Документация:** FR-010 changelog обновлён (v2 entry). Точки входа (`node.md §4`, `palette.rs`) без изменений — API 1:1.
- **Коммит/ветка:** `feature/fr-010-v2-sugiyama-layout`.

## 2026-09-26 — fix(stage): панель «Как считается» — контент панели не отрисовывался (двойное смещение координат), найдено при верификации Table v2 M2

- **Агент:** Super Z (сессия web-3e2a9c55; верификация запроса владельца «пришли скрины обновлённой вёрстки»).
- **Симптом:** на wasm-деплое панель «Как считается» (stage) рисует только каркас-фон; заголовки «Переменные/Расчёт», строки (Table v2, M2) и бегунки — за нижней границей stage.
- **Диагностика:** локальный стенд (trunk-сборка + agent-browser, синтетический dblclick по пучку «Спрос → Интенсивность ×2» — stage открывается двойным кликом по линии пучка, EDGE_HIT_TOLERANCE 6 px); tracing в кадре: vars_viewport y=1191 при вьюпорте 900 — двойное смещение panel.rect.
- **Причина:** площади `CalcPanelLayout` — stage-локальные, а отрисовка прибавляла их к `rect.x/y + panel.rect` (смещение панели дважды). Предсуществующий дефект с 994e68e (FR-044); M2 сохранил его по I-1 (оракул T7 — самосогласован двух путей на одной базе); hit-тесты input.rs были на верной базе — расхождение не проявлялось (панель невидима → ни кликов, ни колеса).
- **Фикс (stage.rs):** заголовки/вьюпорты колонок/бегунки — от начала stage (`ox/oy = rect.x/rect.y`); `paint_calc_panel_rows` — параметр «начало stage»; колесо над колонками — `rel = cursor − rect.x/y` (паритет с input.rs). Оракул T7 переведён на ту же базу (равенство Table ≡ ручной цикл сохранено).
- **Гейты:** cargo test --workspace **2089/0**; clippy --workspace --all-targets -D warnings; fmt --check; wasm_gate.sh --check — зелёные. Визуальная верификация в браузере: панель отрисовывается полностью (строки vars с value-точками, формулы с ƒ-маркером), направляющие выровнены.
- **Доки:** fr-068-table-v2.md §11 (постскриптум: урок I-1 — оракулы равенства не ловят общую ошибку базы координат; нужна скриншот-проверка реального рендера).


## 2026-09-26 — feat(render): FR-075 волны 0.5+W1 — шелл карточек по вёрстке prototype-unified (чип категории, полоса «ИТОГ», порты в z-сегментах)

- **Запрос владельца:** «Нужен не только паритет, но и ui с учётом примера верстки прототипа» (сессия web-77d836dd; продолжение deep dive FR-075).
- **Референс:** docs/prototypes/ — prototype-unified.html (канон, M.HEADER=34/M.STRIP=32/радиус 10, drawNode/drawStrip/drawPorts/drawDataRow), prototype-mainstage-anatomy.html (анатомия A–E), ux-node-body-fill.html (DOM .strip-d/.chip).
- **Карточка:** CARD_CORNER_RADIUS 8→10 (зеркала dimensions.json — тест I-5); тень offsetY 3 / smoothstep(−5,7) / 0.25, margin 16/zoom (cards.wgsl); волосяной контур card_edge на каждой карточке (theme.card_edge #30323c/#d5dae3, шейдер 1.5·a≈1.28 px); внешнее кольцо выделения selection_ring_instance (x−3.5..+7, R+3.5, accent ≈1.2 px; не рисуется при кольце серьёзности FR-016).
- **Полоса категории → чип:** template_band_instance + TEMPLATE_BAND_H удалены; чип header_chip_instance (пилюля x+10/y+8/метка+14/16, corners 8) + ChipKind/chip_fill (template → template.color — преемственность цвета; РАСЧЁТ/ЗАМЕТКА/ФАЙЛ — chip_calc/chip_note/chip_file; палитра chips прототипа); метка 10 SEMIBOLD uppercase, текст #14161c (тёмный на любой теме); шейпинг в кэше ноды (CachedChip/CachedTitle.chip, TextSystem::chip; дрейф метки — перешейп одного буфера без сброса кэша); заголовок после чипа (title_left_offset, title_clip_width + chip_w), буква расширения — после чипа. Иконка роли FR-018 — v-центр шапки (template_icon_rect y+9). В снапшоте canvasdesk.template категории нет — метка «ШАБЛОН», категория читается по цвету (deferred: реестр в кэш).
- **Зоны:** header_separator_instance / result_strip_line_instance — zone_top #2a2b33/#e3e8f0, высота 1/effective_zoom (1 физ. px на любом зуме). Полоса «ИТОГ»: CARD_RESULT_STRIP_H=32 (зеркала JSON), result_strip_instance — тинт strip_tint rgba(49,184,166,.06)/.07, corners [0,0,R,R]; result_footer_y = центр полосы (порт футера line=None следует автоматически); значение прижато к направляющей чисел (row_guides.value_right; фолбэк — правый пад), цвет result_value #31b8a6/#1f8f80 (Σ-строка — тот же тон).
- **Таблица:** зебра — светлые прогонки по прототипу: theme.zebra_fill rgba(255,255,255,.04)/rgba(20,24,40,.03) для BodyQuadKind::RowBg (search_row_fill остался поиску — прежняя заливка затемняла строки).
- **Порты (W1):** построчные порты/якоря строятся в z-сегменте своей ноды (после тамбнейла, до карточек перекрывающих нод) — порт фоновой ноды не поверх карточек переднего плана; инстансы в общем буфере (диапазоны карточек сегмента покрывают — plan_tail_ranges не тронут); culling бесплатно (полный обход canvas.nodes из мир-хвоста удалён); LOD-гейт zoom ≥ 0.6 (anatomy::LOD_L0_MAX_ZOOM); стиль точек — row_port_dot: fill ROW_PORT_FILL #31b8a6 α0.5 + кольцо ROW_PORT_RING #0f2c29 (drawRowPort прототипа; is_final остался в данных); hover/compat-акценты прежние. Мир-хвост — только интеракционные аффордансы (порты сторон T8, хэндлы CR-002, draft).
- **Main stage:** app/stage.rs — band → чип (метка «ШАБЛОН», замер TextMeasurer + measure_font_system, SEMIBOLD, font(10)); screen-текст метки #14161c.
- **Слоты тем:** theme.rs — card_edge, zone_top, strip_tint, result_value, chip_calc/note/file, zebra_fill (обе темы); theme_presets.rs — вывод тех же слотов для JSON-пресетов (тона сетки/акцент/link; is_light_bg-ветка).
- **Тесты:** новые result_strip_geometry, selection_ring_geometry, row_ports_use_prototype_style; обновлены оракул I-шаблон (чип/иконка — декор), result_strip_template_only_and_footer_parity (зона D 32), param/line-port стили, title_clip_width(+chip_w), json_dimensions_match_rust_mirror (corner_radius 10 + card.result_strip).
- **Доки:** FR-075 (§4.1а волна 0.5, §4.1б W1, §4.6 deferred — R-3 якоря авто-строк/статус-бейджи шапки/title 13/ROW 24/силуэт L0/В-C кромки, история, источники), index-cr-fr.md, docs/architecture/shell-world-space.md (состав шелла: чип/линии/полоса/кольцо; тень; порядок кадра с портами в сегментах).
- **Гейты:** cargo test --workspace — 66 наборов, 0 failed; clippy --workspace --all-targets -D warnings; fmt --check; naga-валидация 7/7 WGSL; wasm-check (canvas-web, wasm32).
- **Deferred/не тронуто:** W2 SVG-иконки мира, W3 ClipRect, W4 KitState-состояния, W5 rotate; метрики текста прототипа (title 13/ROW 24/PROSE 12.5) — глубокий риппл, отдельное решение; силуэт L0; В-C кромки-зоны (UX-решение владельца).


---

## 2026-09-26 — fix(render/scene+app+web): FR-075 — верификация витриной wasm (10 вариантов карточек, 1 скриншот) и 3 найденных/исправленных дефекта

- **Запрос владельца:** «Покажи на 1 скриншоте с wasm до 10 вариантов наполнения и вида карточки, заодно проверь что ты всё правильно отредактировал».
- **Стенд:** песочница с нуля (rustup 1.98.1 + wasm-bindgen 0.2.127 prebuilt, Chromium 1243 + WebGPU/SwiftShader под Xvfb — рецепт docs/WASM-TESTING.md). Сцена витрины — новый генератор `crates/canvas-scene/examples/shell_showcase.rs` (Registry::builtin + fit_template_node_height + ТОЧНЫЙ шейдер высот через install_measured_reserve), доставка в браузер — запись JSON в OPFS до старта приложения (перехват загрузки wasm в playwright-сценарии) + `?canvas=showcase`.
- **Дефект 1 — резерв футера:** уровень-1 (canvas-scene/measure.rs) и точный шейпер (canvas-app/support.rs) резервировали `RESULT_LINE_HEIGHT + 2 = 18 px` — модель ДО FR-075; полоса «ИТОГ» — 32 px → дефицит 14 px, последний ряд тела заходил под полосу (воспроизведён на шаблоне ab-test: visitors_b под «Итог 0.004651 %»). Фикс: резерв = `CARD_RESULT_STRIP_H` в обоих уровнях (+тест оценки).
- **Дефект 2 — загрузочная подгон без описаний (I-2):** `SceneState::with_storage` считает первичный пересчёт ДО App с пустым `template_descs` (наполнялся только при импорте шаблона), рендер описание манифеста показывает; хеш-гейт «первая встреча — доверяем высоте» не догонял. Фикс: `App::new` → `sync_template_descs()` + новый pub `SceneState::refit_after_template_descs()` (growth/усадка по хеш-гейту; появление описания меняет content_height_hash).
- **Дефект 3 — web-обёртка:** (а) в index.html два синтаксических обрыва inline-инициализации тура — pageerror «missing ) after argument list», блок умирал: меню тура не строилось, host-hook не ставился; (б) после починки вскрылся слой глубже — неактивный dim-оверлей тура создавался с `pointer-events:auto` (highlight.ts: конструктор до первого paint) → НЕВИДИМЫЙ щит fullscreen, все клики страницы глотались (канвас мёртв: выделение/ drag недоступны). Фиксы: восстановлены обрывы (tour.run(scenarios[k]) + хвост блока); оверлей создаётся с `pointer-events:none` (paint() возвращает auto при активном шаге — семантика неактивного тура сохранена).
- **Витрина (скриншот):** 10 вариантов — 6 шаблонных нод разных категорий/цветов/иконок (cdn, api-gateway, queue-kafka, ab-test, lb, http-endpoint; чип по цвету манифеста, иконка роли, зебра, Σ, полоса «ИТОГ» с значением на направляющей, бирюзовые построчные порты), РАСЧЁТ с единицами и группировкой разрядов, ЗАМЕТКА, ФАЙЛ, сломанная формула (красный diag в футере), пара с перекрытием (порты фоновой lb не просвечивают сквозь http — W1). Сетка 1280×800 при zoom 1 с учётом хрома.
- **Гейты:** cargo test --workspace 2143 passed / 0 failed; clippy --workspace --all-targets -D warnings; fmt --check; wasm_gate.sh --check OK; страница без pageerror; скриншоты в target/wasm_ui + download владельца.
- **Доки:** FR-075 §7 (история), данный worklog.


---

## 2026-09-26 — fix(render/scene/core): LOD текста (зум-инвариантный кламп описания) + паника bundles.rs (воркер-окно перестройки пучков)

- **Репорт владельца (два дефекта):** (1) «неправильно работает логика LOD в отношении текста на ноде, пример шаблон стоимости инфраструктуры в месяц, в ней примерно 5 строк текста и весь текст виден только при максимальном приближении; чем дальше отдаляешься, тем больше обрезаются строки. Текст не должен обрезаться при масштабировании»; (2) паника `crates\canvas-core\src\bundles.rs:358:37: index out of bounds: the len is 19 but the index is 19` (release, desktop).
- **LOD текста — корень:** `with_body_stack` (canvas-render/text.rs) передавал в `clamp_desc_text` ширину `body_width × zoom_px` при фиксированных метриках клампа (BODY_FONT_SIZE/BODY_LINE_HEIGHT без × zoom) → бюджет «≤ 2 строки» (TABLE_DESC_CLAMP_LINES) зум-зависим: на максимальном зуме в кламп проходил весь текст описания (зона в world-px раздувалась относительно `measure_body_height`, которая всегда zoom 1 — рассинхрон I-2 → клип тела резал строки), при отдалении — всё меньше слов. Фикс: кламп меряет перенос по world-ширине `body_width` — контент зоны и высота стека одинаковы на любом зуме (перенос клампа совпадает с рендером блока — пропорциональный скейл); полный текст — через аффорданс «⋯ целиком ▾» (D-8 runtime v1). Тест `desc_zone_zoom_invariant` (zoom 0.5/1/4 — идентичные высоты; на баге падает 88≠108).
- **Паника — корень:** воркер-ветка FR-064 в `SceneState::recompute_flow` (desktop) делает ранний return ДО хвоста, где FR-042 перестраивает `EdgeBundleIndex` — до прихода `FlowReady` кадр рендерится с пучками прежнего канваса; удаление ребра (20→19) → `dominant_edge` читал `canvas.edges[19]` вне диапазона. Фикс 1: синхронная перестройка `self.bundles` в воркер-ветке (O(edges), propagate по-прежнему у воркера). Фикс 2: `dominant_edge` — `edges.get(index)`, устаревший индекс пропускается (страховка от любого непредвиденного пути). Тесты: `worker_path_rebuilds_bundles_synchronously` (canvas-scene, воркер с тривиальным вычислителем; на баге падает), `dominant_edge_stale_index_does_not_panic` (canvas-core).
- **Верификация wasm (L2):** стенд по docs/WASM-TESTING.md; новый генератор `crates/canvas-scene/examples/lod_probe.rs` (одна нода com.canvasdesk.infra-cost, точный шейпер + refit_after_template_descs: 252→318). Скриншоты zoom ≈0.5/1.0/1.8 (Ctrl+колесо — колесо без модификаторов это пан, SPEC §8; камера не персистится — по загрузке на состояние): контент зоны описания и все ряды таблицы идентичны на всех зумах, ИТОГ 390 руб на месте; backend=BrowserWebGpu, 0 pageerror. Склейка — download/lod_zoom_invariance_wasm.png.
- **Гейты:** cargo test --workspace — 0 failed (+3 новых теста); clippy --workspace --all-targets -D warnings; fmt --check; wasm-стенд L2.
- **Доки:** fr-061 changelog (зум-инвариантность клампа), fr-042 changelog (воркер-ветка + страховка dominant_edge), данный worklog.
