...
## 2026-09-26 — feat(schemes): шаблон схемы «A/B-тест» — каталог 10 → 11 (запрос владельца)

- **Агент:** Super Z (сессия web-45286787; запрос владельца: «нужен шаблон схемы по расчёту a/b тестов… выведи для апрува, далее собирай»; + «в этот же бэклог возьми переработку поверхности меню screen templates»).
- **Анализ базы:** формат схем FR-049 (`schemes.rs`: белый список text/group, пресеты 1..6, лимиты, валидатор XOR-адресации), инварианты каталога `scheme_apply.rs` (пучки ≥2/хабы ≥3/цепочки ≥3 хопов/немые проза-ноды/без красного), умная раскладка FR-071 (координаты пакета — только seed-подсказки). Ограничения движка для A/B-математики: нет √/pow, stats-функции FR-063 за фичей `stats` и в дефолтной сборке НЕ включены (workspace default = [] — нормальные Quantile/CDF недоступны) → значимость считается через z²: тест «|z| > zcrit» ⟺ «z² > zcrit²» — эквивалентно без √; размер выборки — упрощённая формула долей n = 2·p̄·(1−p̄)·(zα+zβ)²/Δ² (расхождение с полной пул-формулой 0.004% на дефолтах); z-константы (1.96/0.84) с таблицей замен в вердикте (α 10/5/1% → 1.64/1.96/2.58; мощность 70/80/90% → 0.52/0.84/1.28).
- **Схема `com.canvasdesk.scheme.ab-test`** (категория «Бизнес», 10 нод / 19 value-рёбер, RU+EN): ветка планирования Трафик(visitors_day/share/rate_day) + Гипотеза(conv_a/lift/zcrit/zpower) → Цель(conv_b/delta) → План(pbar/zsum/n_variant/days) → Бюджет(cost ≈ 7 800 $); ветка оценки Замер(users/orders×A/B) → Результаты(rate_a/rate_b/pbar/se2) → Итог(d/uplift/z2/zcrit2/margin ≈ +1.184 — дефолтный замер значим). Проливание toParam демонстрируется на $zcrit (Гипотеза → Итог); хабы: Гипотеза 6 рёбер, План 7, Результаты 7; цепочка Гипотеза→Цель→План→Бюджет = 3 хопа; пучки: Гипотеза→Цель, Гипотеза→План, Замер→Результаты, Результаты→Итог.
- **Оракул** `oracle_ab_test`: построчные значения всех 5 расчётных нод (helper `lines_of` — Some(Ok) в порядке строк, Some(Err) — паника) + узловые итоги; дефолтная история связная: «план сказал ~31 199/вариант и 15.6 дня, замер 20k/вариант дал uplift 10% → z² 5.03 > 3.84».
- **Правки тестов-счётчиков:** `mcp_schemes_list_embedded_registry` 10 → 11 (+ab-test в реестре). CJM-геометрия: первый прогон поймал OVERLAP после авто-роста (длинная строка n_variant растянула План на +114px поверх Итога; results/outcome аналогично) — декларативные высоты подняты до фактических выросших (growth-only рефит не двигает ноды; ROW_GAP=64 сохраняется).
- **Доки:** scheme-gallery.md §5 (каталог 11, строка ab-test с оракулом), ACCEPTANCE FR-049.14, worklog.
- **Бэклог (отдельный запрос владельца):** `docs/change-requests/cr-017-scheme-gallery-surface-rework.md` — переработка поверхности «Шаблоны схем» (нет превью, плотность строк при 11 схемах, потолок 5 чипов категорий; кандидаты C1–C5: превью поверх thumbs-пайплайна, мастер-деталь, секции вместо чипов) + строка в index-cr-fr.md.

### Гейты
- cargo test --workspace: 66 наборов «ok», 0 failed (scheme_apply 34/34 вкл. oracle_ab_test и все инварианты; canvas-app 389/0; scheme_cjm 4/4); clippy --workspace --all-targets -D warnings — 0; fmt --check — чисто.
- wasm-гейт не запускался (в среде нет wasm32-таргета/trunk/binaryen): изменения — ассет-данные + #[cfg(test)]-код + доки, non-test код не тронут — поверхность wasm-сборки не меняется.

### Статусы
- Каталог схем 10 → 11; CR-017 открыт в бэклоге (статус «выявлено»); счётчики тестов синхронизированы.
- **Telegram:** план → дизайн → отчёт сборки → финальное саммари.
- **Открытый вопрос владельцу:** опциональный палитровый шаблон `pa-ab-sample-size` (одна нода для быстрого планирования) НЕ добавлен — решение за владельцем (потребует сдвига счётчика 61 → 62 в templates_schema.rs).
